...
<p align="center">
  ...
  <a href="https://github.com/poushwell/orchesis/actions"><img src="https://img.shields.io/badge/tests-4%2C912%2B-22c55e?style=flat-square" alt="Tests"></a>
  ...
  <a href="https://orchesis.ai/scan"><img src="https://img.shields.io/badge/MCP_checks-113-c084fc?style=flat-square" alt="MCP Checks"></a>
</p>
...
Orchesis sits between your application and the LLM provider. Requests flow
through a configurable pipeline of processing stages — parsing, rate-limit
checks, content filtering, context optimization, policy evaluation, upstream
routing, response post-processing, and session recording. Each stage is a
plugin; the default set covers the common reliability concerns.

Where it helps:

- Long agent chains that degrade silently — the proxy keeps a rolling state
  estimate per session and intervenes before failures cascade.
- Tool-call reliability — failed calls feed the state estimator, triggering
  retries or fallbacks before the chain compounds errors.
- Cost predictability — multi-provider routing picks the cheapest viable
  provider for each request based on observed cache hit rate and health.
- Production observability — every request is recorded; sessions can be
  replayed for debug.

What Orchesis does **not** see: internal reasoning chains inside the model,
encrypted tool payloads, semantic content that looks identical to legitimate
traffic. These remain the customer's responsibility — Orchesis covers
operational reliability, not model alignment.
...
- [Pipeline Reference](https://github.com/poushwell/orchesis/blob/main/docs/PIPELINE.md) — Request processing stages
