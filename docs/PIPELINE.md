...
- Large chain: injection protocol, context termination, context strategy router, context optimizer, cost optimizer, content ranker, context window optimizer, context budget + compression v2, then `context_engine` if enabled.
- **Configurable:** `injection_protocol`, `context_termination`, `context_optimizer`, `cost_optimizer`, `content_ranker`, `context_window_optimizer`, `context_budget`, `context_compression_v2`, `context_engine`, `request_prioritizer`, etc.
