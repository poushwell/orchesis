...
- **Fallback Reasoner** (`par_reasoning`) — applies fallback reasoning when
  observation count is below a configured threshold
- **Quality Control** (`criticality_control`) — feedback controller keeping
  context quality score within target range
- **Adaptive Controller** (`mrac_controller`) — per-agent gain scheduling
- **Critical Agent** (`keystone_agent`) — identifies agents whose failure
  would disproportionately impact overall reliability
- **Efficiency Metric** (`efficiency_metric`) — relative quality measure
  derived from min/max entropy reference points
- **Adversarial Tracker** (`red_queen`) — tracks co-evolution of threat
  patterns and defense responses
- **Content Importance** (`kolmogorov_importance`) — message importance
  scoring based on compressibility and downstream correlation
- **State Classifier** (`core/pipeline_engine`) — classifies request context
  into operational states (active / stable / degraded)
- **Behavior Sync** (`behavior_sync`) — propagates successful behavioral
  patterns between agents with similar operational profiles
- **Message Consistency** (`discourse_coherence`) — weighted consistency
  score across flow, entity tracking, and topic alignment
