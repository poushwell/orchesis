...
def test_store_model_filter() -> None:
    ...
    cache = SemanticCache(SemanticCacheConfig(enabled=True, cacheable_models=["gpt-4"]))
    msgs = [{"role": "user", "content": "hello " * 10}]
...
def test_proxy_cache_hit_skips_upstream() -> None:
    ...
    ctx.parsed_req = type(
        "Parsed", (), {"tool_calls": [], "content_text": "test", "provider": "anthropic"}
    )()
...
def test_proxy_stores_after_upstream() -> None:
    ...
    ctx.resp_status = 200
    ctx.resp_body = (
        b'{"content":[{"type":"text","text":"ok"}],"usage":{"input_tokens":10,"output_tokens":5}}'
    )
