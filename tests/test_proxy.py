...
class _FakeEngine:
    ...
    def evaluate(self, payload: dict):
        ...
        if payload.get("tool") in self.deny_tools:
            return Decision(
                allowed=False,
                reasons=["file_access: blocked for test"],
...
async def test_proxy_denies_blocked_tool() -> None:
    proxy = OrchesisProxy(
        _FakeEngine(deny_tools={"read_file"}), ProxyConfig(upstream_url="http://localhost:9999")
    )
...
def test_proxy_passthrough_mode() -> None:
    engine = _FakeEngine()
    proxy = OrchesisProxy(
        engine, ProxyConfig(upstream_url="http://localhost:3000", intercept_mode="passthrough")
    )
    extracted = proxy._extract_tool_call(
        "POST", "/tools/read_file", {"content-type": "application/json"}, b'{"path":"/tmp"}'
    )
...
def test_proxy_emitted_secret_findings_redact_raw_match() -> None:
    ...
    proxy = OrchesisProxy(
        _FakeEngine(), ProxyConfig(upstream_url="http://localhost:3000"), event_bus=bus
    )
    findings = proxy._scan_response("read_file", b"OPENAI_KEY=sk-abcdefghijklmnopqrstuvwxyz123")
    ...
    assert all(
        "raw_match" not in finding for finding in emitted_findings if isinstance(finding, dict)
    )
...
async def test_proxy_large_body_rejected() -> None:
    proxy = OrchesisProxy(
        _FakeEngine(), ProxyConfig(upstream_url="http://localhost:3000", max_body_size=16)
    )
    server = await asyncio.start_server(proxy.handle_request, "127.0.0.1", 0)
    port = server.sockets[0].getsockname()[1]
...
""".strip(),
    ...
    port = _pick_free_port()
    proxy = LLMHTTPProxy(
        policy_path=str(policy), config=HTTPProxyConfig(host="127.0.0.1", port=port)
    )
    proxy.start(blocking=False)
    try:
        body = {
...
""".strip(),
    ...
    try:
        for _ in range(4):
            ...
            assert status == 200
        blocked_status, blocked_headers, blocked_payload = _post_chat_completion(
            port, "Read HEARTBEAT.md"
        )
...
""".strip(),
    ...
    try:
        ...
        status1, _, _ = _post_chat_completion(
            port, "Read HEARTBEAT.md", "sess-a", session_header="X-OpenClaw-Session-Id"
        )
        status2, _, _ = _post_chat_completion(
            port, "Read HEARTBEAT.md", "sess-b", session_header="X-OpenClaw-Session-Id"
        )
        status3, _, _ = _post_chat_completion(
            port, "Read HEARTBEAT.md", "sess-a", session_header="X-OpenClaw-Session-Id"
        )
        status4, _, _ = _post_chat_completion(
            port, "Read HEARTBEAT.md", "sess-b", session_header="X-OpenClaw-Session-Id"
        )
        status5, _, _ = _post_chat_completion(
            port, "Read HEARTBEAT.md", "sess-a", session_header="X-OpenClaw-Session-Id"
        )
        assert status1 == 200
        assert status2 == 200
...
def test_get_upstream_blocks_metadata_ip(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    upstream = proxy._get_upstream(
        "openai", {"X-Orchesis-Upstream": "http://169.254.169.254/latest"}
    )  # noqa: SLF001
    assert upstream == "https://api.openai.com"
...
def test_get_upstream_allows_public_https(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "socket.getaddrinfo",
        lambda *args, **kwargs: [
            (socket.AF_INET, socket.SOCK_STREAM, 0, "", ("104.18.33.45", 443))
        ],
    )
...
def test_get_upstream_allows_private_when_enabled(
...
def test_llm_proxy_scans_request_content_for_secrets_and_blocks() -> None:
    ...
    try:
        body = {
            "model": "gpt-4o",
            "messages": [{"role": "user", "content": "my key sk-abcdefghijklmnopqrstuvwxyz123"}],
        }
        req = UrlRequest(
            f"http://127.0.0.1:{port}/v1/chat/completions",
            data=json.dumps(body).encode("utf-8"),
...
def test_loop_warn_adds_dashboard_event() -> None:
    ...
    try:
        proxy._loop_detector = SimpleNamespace(  # noqa: SLF001
            check_request=lambda _req: SimpleNamespace(
                action="warn", reason="Exact loop threshold exceeded"
            )
        )
...
def test_cascade_cache_hit_still_runs_loop_detection() -> None:
    ...
    try:
        ...
        proxy._phase_loop_detection = lambda ctx: (
            setattr(ctx, "loop_warning_header", "loop warn") or True
        )  # type: ignore[method-assign]  # noqa: E501, SLF001
