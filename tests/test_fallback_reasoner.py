...
def test_abductive_mode_below_threshold() -> None:
    reasoner = FallbackReasoner({"abductive_threshold": 4})
...
def test_causal_mode_above_threshold() -> None:
    reasoner = FallbackReasoner({"abductive_threshold": 2})
    reasoner.observe({"event": "a"})
...
def test_confidence_grows_with_observations() -> None:
    reasoner = FallbackReasoner({"abductive_threshold": 8})
...
def test_t5_applies_flag() -> None:
    reasoner = FallbackReasoner({"abductive_threshold": 3})
...
def test_api_abduce_endpoint(tmp_path: Path, monkeypatch) -> None:
    ...
    client.post("/api/v1/reasoning/observe", json={"agent_id": "a1", "event": "deny"}, headers=headers)
    response = client.post(
        "/api/v1/reasoning/abduce",
...
def test_api_causal_graph_endpoint(tmp_path: Path, monkeypatch) -> None:
    ...
    client.post("/api/v1/reasoning/observe", json={"agent_id": "a1", "event": "deny"}, headers=headers)
    response = client.get("/api/v1/reasoning/causal-graph", headers=headers)
    assert response.status_code == 200
    payload = response.json()
