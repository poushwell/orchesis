...
def test_get_stats() -> None:
    with patch("urllib.request.urlopen") as mock:
        mock.return_value.read.return_value = json.dumps({"requests": 10, "blocked": 0}).encode(
            "utf-8"
        )
        mock.return_value.__enter__ = lambda s: s
        mock.return_value.__exit__ = lambda s, *a: None
...
def test_list_sessions() -> None:
    with patch("urllib.request.urlopen") as mock:
        mock.return_value.read.return_value = json.dumps({"sessions": [{"id": "s1"}]}).encode(
            "utf-8"
        )
        mock.return_value.__enter__ = lambda s: s
        mock.return_value.__exit__ = lambda s, *a: None
...
def test_export_session() -> None:
    with patch("urllib.request.urlopen") as mock:
        mock.return_value.read.return_value = json.dumps({"air": {"version": "1.0"}}).encode(
            "utf-8"
        )
        mock.return_value.__enter__ = lambda s: s
        mock.return_value.__exit__ = lambda s, *a: None
...
def test_list_flow_sessions() -> None:
    with patch("urllib.request.urlopen") as mock:
        mock.return_value.read.return_value = json.dumps({"sessions": [{"id": "f1"}]}).encode(
            "utf-8"
        )
        mock.return_value.__enter__ = lambda s: s
        mock.return_value.__exit__ = lambda s, *a: None
...
def test_list_experiments() -> None:
    with patch("urllib.request.urlopen") as mock:
        mock.return_value.read.return_value = json.dumps({"experiments": [{"id": "e1"}]}).encode(
            "utf-8"
        )
        mock.return_value.__enter__ = lambda s: s
        mock.return_value.__exit__ = lambda s, *a: None
...
def test_create_experiment() -> None:
    with patch("urllib.request.urlopen") as mock:
        ...
        result = client.create_experiment(
            "Test", [{"name": "a", "weight": 0.5}, {"name": "b", "weight": 0.5}]
        )
        assert result["experiment_id"] == "exp-1"
        call_args = mock.call_args
        req = call_args[0][0]
...
def test_stop_experiment() -> None:
    with patch("urllib.request.urlopen") as mock:
        mock.return_value.read.return_value = json.dumps({"variants": [], "winner": None}).encode(
            "utf-8"
        )
        mock.return_value.__enter__ = lambda s: s
        mock.return_value.__exit__ = lambda s, *a: None
...
def test_get_results() -> None:
    with patch("urllib.request.urlopen") as mock:
        mock.return_value.read.return_value = json.dumps(
            {"variants": [{"name": "a", "success_rate": 0.0}]}
        ).encode("utf-8")
        mock.return_value.__enter__ = lambda s: s
        mock.return_value.__exit__ = lambda s, *a: None
        client = OrchesisClient("http://localhost:8080")
...
def test_get_outcomes() -> None:
    with patch("urllib.request.urlopen") as mock:
        mock.return_value.read.return_value = json.dumps({"success": 10, "failure": 2}).encode(
            "utf-8"
        )
        mock.return_value.__enter__ = lambda s: s
        mock.return_value.__exit__ = lambda s, *a: None
