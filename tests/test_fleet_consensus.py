...
def test_quorum_forms_at_n_star() -> None:
    sensor = FleetConsensus({"n_star": 3, "similarity": 0.7})
...
def test_similar_tasks_grouped() -> None:
    sensor = FleetConsensus({"n_star": 3, "similarity": 0.75})
    for agent, fp in (
...
def test_dissimilar_tasks_not_grouped() -> None:
    sensor = FleetConsensus({"n_star": 3, "similarity": 0.8})
...
def test_api_quorum_status(tmp_path: Path, monkeypatch) -> None:
    ...
    for i in range(16):
        resp = client.post(
            ...
            json={
                "agent_id": f"agent-{i}",
                "task_fingerprint": f"summarize security report {i % 3}",
            },
            headers=headers,
        )
