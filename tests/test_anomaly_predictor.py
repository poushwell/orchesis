...
def test_prediction_history_tracked() -> None:
    ...
    predictor.record_prediction(
        "agent-z", predictor.predict([{"score": 0.2}, {"score": 0.5}, {"score": 0.7}])
    )
...
async def test_api_predict_endpoint(tmp_path: Path) -> None:
    ...
    async with await _client(app) as client:
        res = await client.post(
            ...
            json={
                "agent_id": "agent-a",
                "recent_metrics": [{"score": 0.2}, {"score": 0.6}, {"score": 0.9}],
            },
            headers=_auth(),
        )
...
async def test_api_warning_endpoint(tmp_path: Path) -> None:
    ...
    _write_events(
        decisions_log,
        [_event("agent-w", 0.2), _event("agent-w", 0.7), _event("agent-w", 0.9, "L2")],
    )
    async with await _client(app) as client:
