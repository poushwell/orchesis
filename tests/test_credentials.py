...
def test_injector_matches_glob_pattern(monkeypatch) -> None:
    ...
    injected, _aliases = injector.inject(
        {"tool_name": "github_list_repos", "params": {}, "headers": {}}
    )
...
def test_param_injection_replaces_value(monkeypatch) -> None:
    ...
    injected, _aliases = injector.inject(
        {"tool_name": "query_database", "params": {"password": "alias"}, "headers": {}}
    )
...
def test_multiple_credentials_injected_same_tool(monkeypatch) -> None:
    ...
    injected, aliases = injector.inject(
        {"tool_name": "query_database", "params": {}, "headers": {}}
    )
...
def test_credentials_cli_list_hides_values() -> None:
    ...
    with runner.isolated_filesystem():
        ...
        listed = runner.invoke(
            main, ["credentials", "list", "--vault-path", ".orchesis/credentials.enc"]
        )
...
def test_credentials_cli_remove() -> None:
    ...
    with runner.isolated_filesystem():
        ...
        removed = runner.invoke(
            main,
            ["credentials", "remove", "openai_key", "--vault-path", ".orchesis/credentials.enc"],
        )
        assert removed.exit_code == 0
        test_result = runner.invoke(
            main, ["credentials", "test", "openai_key", "--vault-path", ".orchesis/credentials.enc"]
        )
...
def test_credentials_cli_env_mapping(monkeypatch) -> None:
    ...
    with runner.isolated_filesystem():
        ...
        test_result = runner.invoke(
            main, ["credentials", "test", "openai_key", "--policy", "missing.yaml"]
        )
        assert test_result.exit_code != 0
...
def test_injector_keeps_existing_headers(monkeypatch) -> None:
    ...
    injected, _ = injector.inject(
        {"tool_name": "web_search", "params": {}, "headers": {"X-Trace": "1"}}
    )
