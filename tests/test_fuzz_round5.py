...
def test_secret_scanner_null_bytes() -> None:
    scanner = SecretScanner()
    text = (b"\x00" * 100).decode("utf-8", errors="replace")
    findings = scanner.scan_text(text)
    assert isinstance(findings, list)
...
def test_secret_scanner_mixed_binary() -> None:
    scanner = SecretScanner()
    text = b"api_key=\xff\xff\x00test".decode("utf-8", errors="replace")
    findings = scanner.scan_text(text)
    assert isinstance(findings, list)
...
def test_pii_detector_0x8a_repetition() -> None:
    ...
    payload = b"\x8a" * 100 + b"123-45-6789"
...
def test_pii_detector_orphaned_continuation() -> None:
    detector = PiiDetector()
    text = (b"\xec\xb7\x8d" * 50).decode("utf-8", errors="replace")
    findings = detector.scan_text(text)
    assert isinstance(findings, list)
