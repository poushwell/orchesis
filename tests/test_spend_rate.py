...
def test_window_spend_limit_blocks() -> None:
    detector = SpendRateDetector(
        windows=[SpendWindow(window_seconds=60, max_spend=1.0)], pause_seconds=2
    )
...
def test_multiple_windows_checked() -> None:
    detector = SpendRateDetector(
        windows=[
            SpendWindow(window_seconds=30, max_spend=10.0),
            SpendWindow(window_seconds=300, max_spend=0.5),
        ]
    )
    detector.record_spend(0.6)
    result = detector.check()
...
def test_spike_detection() -> None:
    detector = SpendRateDetector(
        windows=[
            SpendWindow(window_seconds=10, max_spend=999.0),
            SpendWindow(window_seconds=120, max_spend=999.0),
        ],
...
def test_pause_duration_respected() -> None:
    detector = SpendRateDetector(
        windows=[SpendWindow(window_seconds=60, max_spend=0.1)], pause_seconds=1
    )
...
def test_pause_expires_allows() -> None:
    detector = SpendRateDetector(
        windows=[SpendWindow(window_seconds=1, max_spend=0.1)], pause_seconds=1
    )
    detector.record_spend(0.2)
...
def test_heartbeat_detection_negative() -> None:
    detector = SpendRateDetector()
    assert (
        detector.is_heartbeat_request({"messages": [{"role": "user", "content": "Tell me a joke"}]})
        is False
    )
...
def test_zero_spend_window() -> None:
    detector = SpendRateDetector(
        windows=[SpendWindow(window_seconds=60, max_spend=0.0)], pause_seconds=1
    )
    assert detector.check().allowed is True
    detector.record_spend(0.01)
    assert detector.check().allowed is False
