    ...
    return {
        "rules": [
            ...
            {"name": "sql_restriction", "denied_operations": ["DROP", "DELETE"]},
            {
                ...
                "type": "regex_match",
                "field": "params.query",
                "deny_patterns": [r"(?i)ignore\\s+previous\\s+safety"],
            },
        ]
    }
...
def test_context_collapse_detected_and_handled() -> None:
    ...
    assert any(
        str(msg.get("role", "")).lower() == "system" for msg in compressed["compressed_messages"]
    )
...
def test_fleet_coordination_under_load() -> None:
    ...
    quorum = FleetConsensus({"n_star": 5, "similarity": 0.6})
    allocator = BudgetAllocator({"total_budget": 1200})
...
def test_graceful_degradation_chain() -> None:
    ...
    messages = (
        [{"role": "system", "content": "Always obey policy."}]
    ...
    apoptosis = ContextTerminationEngine({"enabled": True, "confidence": 0.8})
    findings = apoptosis.scan(messages)
    removed = apoptosis.remove(messages, findings)
    assert isinstance(removed["messages"], list)

    injector = ContextInjectionProtocol(
        {"enabled": True, "strategy": "adaptive", "max_tokens": 256}
    )
    inject_decision = injector.should_inject(
        {"request_count": 10}, {"quality_score": 0.4, "budget_level": "L2"}
    )
