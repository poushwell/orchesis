...
def test_impossibility_theorems_present() -> None:
    ...
    assert {"I1", "I2", "I3"}.issubset(set(data["impossibility_theorems"].keys()))


def test_one_liner_describes_proxy() -> None:
    line = OrchesisInsights().get_one_liner()
    assert "reliability" in line.lower() or "proxy" in line.lower()
...
def test_headline_metrics_present() -> None:
    ...
    for key in ("retry_reduction", "n_star", "alpha", "proxy_overhead"):
