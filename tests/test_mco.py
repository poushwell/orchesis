...
from orchesis.mco import ModelStats, MCO
...
def test_beta_sample_deterministic_seed(tmp_path) -> None:
    r1 = MCO(_cfg(tmp_path, seed=42))
    r2 = MCO(_cfg(tmp_path, seed=42))
    try:
...
def test_select_model_prefers_successful(tmp_path) -> None:
    ...
    try:
        ...
        for _ in range(40):
            ...
            r.record_outcome(
                "expensive", cat, {"success": False, "latency_ms": 400, "cost_usd": 0.01}
            )
        d = r.select_model(_req("hello"))
...
def test_select_model_avoids_failing(tmp_path) -> None:
    ...
    try:
        ...
        for _ in range(50):
            r.record_outcome(
                "expensive", cat, {"success": False, "latency_ms": 500, "cost_usd": 0.02}
            )
        d = r.select_model(_req("hello"))
        assert d.selected_model == "cheap"
    finally:
...
def test_select_model_forced_exploration(tmp_path) -> None:
    r = MCO(_cfg(tmp_path, min_exploration_rate=1.0, initial_exploration_rate=1.0))
    try:
        d = r.select_model(_req("hello"))
...
def test_select_model_objective_cost_prefers_cheap(tmp_path) -> None:
    r = MCO(_cfg(tmp_path, objective="cost"))
    try:
        cat = r.classify_request(_req("hello"))
        r._stats[cat] = {
            "cheap": ModelStats(
                model="cheap", successes=10, failures=2, avg_cost_usd=0.001, avg_latency_ms=200
            ),
            "expensive": ModelStats(
                model="expensive", successes=10, failures=2, avg_cost_usd=0.05, avg_latency_ms=200
            ),
        }
...
def test_select_model_objective_quality_prefers_best(tmp_path) -> None:
    r = MCO(_cfg(tmp_path, objective="quality"))
    try:
        cat = r.classify_request(_req("hello"))
        r._stats[cat] = {
...
def test_select_model_objective_speed_prefers_fast(tmp_path) -> None:
    r = MCO(_cfg(tmp_path, objective="speed"))
    try:
        cat = r.classify_request(_req("hello"))
        r._stats[cat] = {
            "cheap": ModelStats(
                model="cheap", successes=10, failures=2, avg_cost_usd=0.01, avg_latency_ms=100
            ),
            "expensive": ModelStats(
                model="expensive", successes=10, failures=2, avg_cost_usd=0.01, avg_latency_ms=2000
            ),
        }
        r.sample_beta = lambda a, b: 0.8  # type: ignore[method-assign]
        d = r.select_model(_req("hello"))
...
def test_select_model_objective_balanced(tmp_path) -> None:
    r = MCO(_cfg(tmp_path, objective="balanced"))
    try:
        d = r.select_model(_req("hello"))
        assert d.selected_model in {"cheap", "expensive"}
    finally:
...
def test_quality_score_failure(tmp_path) -> None:
    ...
    try:
        q = r.compute_quality_score(
            {"model": "cheap", "success": False, "error_type": "upstream_error"}
        )
        assert q == 0.0
    finally:
        r.stop()
...
def test_quality_score_slow_but_successful(tmp_path) -> None:
    ...
    try:
        ...
        for _ in range(5):
            r.record_outcome("cheap", c, {"success": True, "latency_ms": 100, "cost_usd": 0.001})
        q = r.compute_quality_score(
            {"model": "cheap", "success": True, "latency_ms": 2000, "cost_usd": 0.001}
        )
        assert 0.0 <= q <= 1.0
    finally:
        r.stop()
...
def test_quality_score_with_loop_detected(tmp_path) -> None:
    ...
    try:
        q = r.compute_quality_score(
            {"model": "cheap", "success": True, "loop_detected": True, "injection_detected": False}
        )
        assert q <= 1.0
    finally:
        r.stop()
...
def test_quality_score_clamped_0_1(tmp_path) -> None:
    ...
    try:
        q = r.compute_quality_score(
            {"model": "cheap", "success": True, "error_type": "context_length"}
        )
        assert 0.0 <= q <= 1.0
    finally:
        r.stop()
...
def test_learning_converges_to_best_model(tmp_path) -> None:
    r = MCO(_cfg(tmp_path, min_exploration_rate=0.02, initial_exploration_rate=0.2))
    try:
        req = _req("hello")
        ...
        for _ in range(100):
            ...
            r.record_outcome(
                ...
                {
                    "success": success,
                    "latency_ms": 80 if success else 500,
                    "cost_usd": 0.001 if success else 0.02,
                },
            )
...
def test_learning_adapts_when_model_degrades(tmp_path) -> None:
    ...
    try:
        ...
        for _ in range(80):
            ...
            success = d.selected_model != "cheap"
            r.record_outcome(
                d.selected_model, cat, {"success": success, "latency_ms": 120, "cost_usd": 0.002}
            )
...
def test_learning_exploration_decreases_over_time(tmp_path) -> None:
    r = MCO(
        _cfg(
            tmp_path, min_exploration_rate=0.0, initial_exploration_rate=0.8, exploration_decay=0.9
        )
    )
    try:
...
def test_load_missing_file_no_crash(tmp_path) -> None:
    r = MCO(_cfg(tmp_path, save_path=str(tmp_path / "missing.json")))
    try:
...
def test_load_corrupt_file_resets(tmp_path) -> None:
    ...
    r = MCO(_cfg(tmp_path, save_path=str(p)))
    try:
        assert r._stats == {}
    finally:
        r.stop()
...
def test_auto_save_worker_runs(tmp_path) -> None:
    ...
    r = MCO(_cfg(tmp_path, save_path=str(p), save_interval_seconds=1))
    try:
        c = r.classify_request(_req("hello"))
        r.record_outcome("cheap", c, {"success": True})
        time.sleep(1.2)
...
def test_single_model_always_selected(tmp_path) -> None:
    cfg = _cfg(
        tmp_path, models=[{"name": "only", "cost_per_1k_input": 0.001, "cost_per_1k_output": 0.002}]
    )
    r = MCO(cfg)
    try:
        d = r.select_model(_req("hello"))
    ...
    def worker():
        try:
            for _ in range(50):
                d = r.select_model(_req("hello"))
                r.record_outcome(
                    d.selected_model, cat, {"success": True, "latency_ms": 100, "cost_usd": 0.001}
                )
