...
def test_scorecard_computed() -> None:
    ...
    card = scorer.compute(
        ...
        decisions_log=[
            ...
            _event(event_id="2", agent_id="agent-a"),
        ],
    )
...
def test_leaderboard_ranked() -> None:
    ...
    assert (
        leaderboard[0]["overall_score"]
        >= leaderboard[1]["overall_score"]
        >= leaderboard[2]["overall_score"]
    )
...
def test_api_scorecard_endpoint(tmp_path: Path, monkeypatch) -> None:
    ...
    response = client.get(
        "/api/v1/scorecard/agent-a?period=30d", headers={"Authorization": "Bearer test-token"}
    )
...
def test_api_leaderboard_endpoint(tmp_path: Path, monkeypatch) -> None:
    ...
    client = TestClient(app)
    response = client.get(
        "/api/v1/scorecard/leaderboard", headers={"Authorization": "Bearer test-token"}
    )
    assert response.status_code == 200
    payload = response.json()
