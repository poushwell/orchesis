...
def test_update_check_parses_pypi(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "orchesis.cli.urlopen", lambda *_args, **_kwargs: _FakeResp({"info": {"version": "9.9.9"}})
    )
    runner = CliRunner()
...
def test_update_shows_current_version(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: _FakeResp({"info": {"version": str(__version__)}}),
    )
    runner = CliRunner()
    result = runner.invoke(main, ["update", "--check"])
    assert result.exit_code == 0
...
def test_update_detects_newer_version(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "orchesis.cli.urlopen", lambda *_args, **_kwargs: _FakeResp({"info": {"version": "99.0.0"}})
    )
    runner = CliRunner()
    result = runner.invoke(main, ["update", "--check"])
    assert result.exit_code == 0
...
def test_update_handles_network_error(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "orchesis.cli.urlopen", lambda *_args, **_kwargs: (_ for _ in ()).throw(URLError("down"))
    )
    runner = CliRunner()
    result = runner.invoke(main, ["update", "--check"])
    assert result.exit_code == 0
