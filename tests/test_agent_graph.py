...
async def test_api_graph_endpoint(tmp_path: Path) -> None:
    ...
    _write_event(
        decisions_log, agent_id="agent_a", to_agent="agent_b", interaction_type="context_share"
    )
    _write_event(
        decisions_log, agent_id="agent_a", to_agent="agent_b", interaction_type="tool_call"
    )
