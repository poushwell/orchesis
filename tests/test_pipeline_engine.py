...
from orchesis.core.pipeline_engine import AgentState


def test_agent_state_has_ghost_fields() -> None:
    state = AgentState()

    assert state.slope_quality == 0.0
    assert state.ews_tau == 0.0
    assert state.state_lifecycle == "nominal"
...
def test_cognitive_load_property() -> None:
    state = AgentState(state_x=[0.2, 0.7, 0.9])
