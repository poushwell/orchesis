...
def test_tool_chain_simple_loop() -> None:
    ...
    for t in [("read_file",), ("execute",)] * 3:
        history.append(
            StructuralSignature(("user", "assistant"), t, 2, False, True, False, "m", 20)
        )
...
def test_tool_chain_empty_tools() -> None:
    det = StructuralPatternDetector()
    history = [
        StructuralSignature(("u",), tuple(), 1, False, False, False, "m", 5) for _ in range(10)
    ]
    assert det.detect_tool_chain_loops(history) == []
...
def test_role_cycle_simple() -> None:
    det = StructuralPatternDetector({"min_occurrences": 3})
    history = [
        StructuralSignature(("user", "assistant"), tuple(), 2, False, False, False, "m", 10)
        for _ in range(4)
    ]
...
def test_role_cycle_with_tools() -> None:
    ...
    sig = StructuralSignature(
        ("user", "assistant", "tool"), ("read_file",), 3, False, True, True, "m", 20
    )
...
def test_escalation_flat() -> None:
    det = StructuralPatternDetector()
    history = [
        StructuralSignature(("u",), tuple(), 2, False, False, False, "m", 10) for _ in range(5)
    ]
    assert det.detect_escalation_chains(history) == []
