...
def test_full_rate_records_all() -> None:
    sampler = RequestSampler(
        {"rate": 1.0, "strategy": "random", "always_record_blocks": False, "seed": 7}
    )
...
def test_zero_rate_records_none() -> None:
    sampler = RequestSampler(
        {"rate": 0.0, "strategy": "random", "always_record_blocks": False, "seed": 7}
    )
    rows = [sampler.should_record({"decision": "ALLOW"}) for _ in range(100)]
    assert not any(rows)
...
def test_always_record_blocks() -> None:
    sampler = RequestSampler(
        {"rate": 0.0, "strategy": "random", "always_record_blocks": True, "seed": 7}
    )
...
def test_reservoir_sampling_uniform() -> None:
    sampler = RequestSampler(
        {"rate": 0.2, "strategy": "reservoir", "always_record_blocks": False, "seed": 11}
    )
    for _ in range(1000):
...
def test_stats_tracked() -> None:
    sampler = RequestSampler(
        {"rate": 0.5, "strategy": "random", "always_record_blocks": False, "seed": 13}
    )
    for _ in range(20):
        sampler.should_record({"decision": "ALLOW"})
    stats = sampler.get_stats()
...
def test_effective_rate_computed() -> None:
    sampler = RequestSampler(
        {"rate": 0.3, "strategy": "random", "always_record_blocks": False, "seed": 3}
    )
    for _ in range(200):
        sampler.should_record({"decision": "ALLOW"})
    stats = sampler.get_stats()
