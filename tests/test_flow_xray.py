...
@pytest.mark.parametrize(
    "model", ["gpt-4o", "claude-haiku-4", "claude-sonnet-4", "claude-opus-4", "gpt-4o-mini"]
)
def test_record_single_request(model: str) -> None:
    analyzer = _mk_analyzer()
    node_id = analyzer.record_request(
        "s1", model=model, messages=[{"role": "user", "content": "q"}], tools=["read_file"]
    )
...
def test_record_request_response(tokens_in: int, tokens_out: int) -> None:
    analyzer = _mk_analyzer()
    node_id = analyzer.record_request(
        "s1", model="gpt-4o", messages=[{"role": "user", "content": "q"}], tools=[]
    )
    created = analyzer.record_response(
        "s1", node_id, tokens_in, tokens_out, 0.02, 12.5, status="ok"
    )
    assert created == []
    graph = analyzer.get_session_graph("s1")
    assert graph is not None
...
@pytest.mark.parametrize(
    "tool_name", ["read_file", "write_file", "web_search", "http_request", "database_query"]
)
def test_record_tool_calls(tool_name: str) -> None:
    analyzer = _mk_analyzer()
    llm = analyzer.record_request(
        "s1", model="gpt-4o", messages=[{"role": "user", "content": "q"}], tools=[]
    )
    created = analyzer.record_response(
        "s1",
    ...
    assert any(
        e.edge_type == EdgeType.TRIGGERS and e.source_id == llm and e.target_id == created[0]
...
def test_record_tool_result() -> None:
    ...
    assert any(
        e.edge_type == EdgeType.RESPONDS_TO and e.source_id == tool_nodes[0] and e.target_id == rid
...
def test_detect_redundant_tools_exact(repetitions: int) -> None:
    ...
    for _ in range(repetitions):
        analyzer.record_response(
            ...
            0.01,
            10.0,
            tool_calls=[{"name": "read_file", "input": {"path": "/tmp/a"}}],
        )
...
def test_detect_redundant_tools_fuzzy(window: float) -> None:
    ...
    llm = analyzer.record_request("s1", "gpt-4o", [{"role": "user", "content": "q"}], [])
    analyzer.record_response(
        "s1", llm, 10, 10, 0.01, 10.0, tool_calls=[{"name": "web_search", "input": {"q": "x"}}]
    )
    analyzer.record_response(
        "s1", llm, 10, 10, 0.01, 10.0, tool_calls=[{"name": "web_search", "input": {"q": "y"}}]
    )
    analyzer.record_response(
        "s1", llm, 10, 10, 0.01, 10.0, tool_calls=[{"name": "web_search", "input": {"q": "z"}}]
    )
    analysis = analyzer.analyze_session("s1")
    assert analysis is not None
    assert any(p.pattern_type == PatternType.REDUNDANT_TOOL_CALLS for p in analysis.patterns)
...
def test_no_false_positive_different_tools() -> None:
    ...
    llm = analyzer.record_request("s1", "gpt-4o", [{"role": "user", "content": "q"}], [])
    analyzer.record_response(
        "s1", llm, 10, 10, 0.01, 10.0, tool_calls=[{"name": "read_file", "input": {"path": "/a"}}]
    )
    analyzer.record_response(
        "s1", llm, 10, 10, 0.01, 10.0, tool_calls=[{"name": "web_search", "input": {"q": "a"}}]
    )
    analysis = analyzer.analyze_session("s1")
    assert analysis is not None
    assert (
        sum(1 for p in analysis.patterns if p.pattern_type == PatternType.REDUNDANT_TOOL_CALLS) <= 1
    )
...
def test_no_dead_end_when_used() -> None:
    ...
    assert analysis is not None
    assert not any(
        result_id in p.affected_nodes and p.pattern_type == PatternType.DEAD_END
...
def test_detect_tool_chain_patterns(chain: list[str]) -> None:
    ...
    llm = analyzer.record_request("s1", "gpt-4o", [{"role": "user", "content": "q"}], [])
    analyzer.record_response(
        "s1", llm, 10, 10, 0.01, 10.0, tool_calls=[{"name": name, "input": {}} for name in chain]
    )
    analysis = analyzer.analyze_session("s1")
    assert analysis is not None
...
def test_no_false_positive_safe_chain() -> None:
    ...
    analyzer.record_response(
        ...
        0.01,
        10.0,
        tool_calls=[{"name": "read_file", "input": {}}, {"name": "summarize", "input": {}}],
    )
    analysis = analyzer.analyze_session("s1")
    assert analysis is not None
...
def test_security_patterns_disabled() -> None:
    analyzer = _mk_analyzer(
        enable_security_patterns=False, suspicious_tool_chains=[["read_file", "http_request"]]
    )
    llm = analyzer.record_request("s1", "gpt-4o", [{"role": "user", "content": "q"}], [])
    analyzer.record_response(
        "s1",
        ...
        0.01,
        10.0,
        tool_calls=[{"name": "read_file", "input": {}}, {"name": "http_request", "input": {}}],
    )
    analysis = analyzer.analyze_session("s1")
    assert analysis is not None
    assert not any(p.pattern_type == PatternType.TOOL_CHAIN_INJECTION for p in analysis.patterns)
...
def test_analyze_problematic_session() -> None:
    ...
    for _ in range(6):
        _record_llm(
            analyzer, sid="s1", status="error", tokens_in=1000, tokens_out=1, latency_ms=1000
        )
    analysis = analyzer.analyze_session("s1")
    assert analysis is not None
