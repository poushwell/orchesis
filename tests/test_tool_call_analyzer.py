...
def test_api_tool_risk_endpoint(tmp_path: Path, monkeypatch) -> None:
    ...
    decisions.write_text(
        json.dumps(
            _event("evt-1", "sess-x", "read_file", [{"tool": "read_file"}]), ensure_ascii=False
        )
        + "\n",
        encoding="utf-8",
    )
    ...
    risk_response = client.get(
        "/api/v1/tools/risk/bash", headers={"Authorization": "Bearer test-token"}
    )
    assert risk_response.status_code == 200
    assert risk_response.json()["risk_level"] == "critical"

    session_response = client.get(
        "/api/v1/tools/session/sess-x", headers={"Authorization": "Bearer test-token"}
    )
