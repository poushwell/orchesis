...
def test_experiment_results_aggregated(tmp_path: Path) -> None:
    ...
    data = PaperExporter().export_experiment_results(str(results_dir))
...
def test_abstract_generated() -> None:
    text = PaperExporter().generate_abstract()
...
def test_export_all_creates_files(tmp_path: Path) -> None:
    ...
    files = PaperExporter().export_all(str(output_dir))
...
def test_cli_nlce_export_command(tmp_path: Path) -> None:
    ...
    result_all = runner.invoke(main, ["export-paper-data", "--output", str(out_dir)])
    assert result_all.exit_code == 0
    assert "claims_table.md" in result_all.output
    result_section = runner.invoke(
        main,
        ["export-paper-data", "--output", str(out_dir), "--section", "abstract"],
    )
