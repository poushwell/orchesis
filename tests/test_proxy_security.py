...
def test_proxy_scans_request_for_secrets() -> None:
    proxy = OrchesisProxy(
        _Engine(), ProxyConfig(upstream_url="http://localhost:3000"), policy=_policy()
    )
...
async def test_proxy_blocks_critical_secret_in_request() -> None:
    proxy = OrchesisProxy(
        ...
        policy=_policy(block_request=True),
    )
...
def test_proxy_scans_response_for_secrets() -> None:
    ...
    findings = proxy._scan_response_findings(
        "read_file", b"api key sk-abcdefghijklmnopqrstuvwxyz123"
    )
...
def test_proxy_redacts_secrets_in_response() -> None:
    proxy = OrchesisProxy(
        _Engine(), ProxyConfig(upstream_url="http://localhost:3000"), policy=_policy(redact=True)
    )
