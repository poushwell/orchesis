...
def test_baseline_computed_after_observations() -> None:
    ...
    for dim in SessionFP.DIMENSIONS:
...
def test_anomaly_score_high_on_deviation() -> None:
    ...
    for _ in range(10):
        dna.observe(
            _request(content="x" * 20, tools=[]), _decision(duration_ms=100.0, cache_hit=False)
        )
...
def test_anomaly_score_low_on_normal() -> None:
    ...
    for _ in range(10):
        dna.observe(
            _request(content="x" * 80, tools=["read_file"]),
            _decision(duration_ms=200.0, cache_hit=True),
        )
    baseline = dna.compute_baseline()
    current = {
...
def test_store_save_and_retrieve(tmp_path: Path) -> None:
    store = SessionFPStore(storage_path=str(tmp_path / "dna"))
    dna = SessionFP("agent-xyz")
