...
def test_pooled_server_limits_concurrent() -> None:
    ...
    try:
        url = f"http://127.0.0.1:{server.server_address[1]}/"
        req_threads = [
            threading.Thread(target=lambda: urlopen(url, timeout=5).read()) for _ in range(10)
        ]
...
def test_build_synthetic_response() -> None:
    ctx = _RequestContext(
        handler=None, body={"model": "gpt-4o", "id": "x"}, original_model="gpt-4o"
    )
...
def test_dashboard_overview_includes_pool(tmp_path: Path) -> None:
    ...
    try:
        with urlopen(
            f"http://127.0.0.1:{proxy._config.port}/api/dashboard/overview", timeout=5
        ) as resp:
            payload = json.loads(resp.read().decode("utf-8"))
