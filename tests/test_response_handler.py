...
def test_process_response_with_openai_key_blocked() -> None:
    processor = ResponseProcessor(scan_secrets=True)
    parsed = ParsedResponse(
        provider="openai", model="gpt-4o", content_text="sk-abcdefghijklmnopqrstuvwxyz123"
    )
    result = processor.process(parsed)
...
def test_process_response_with_jwt_blocked() -> None:
    ...
    parsed = ParsedResponse(
        provider="openai",
        model="gpt-4o",
        content_text="eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0",
    )
    result = processor.process(parsed)
    assert result["allowed"] is False
...
def test_process_response_with_private_key_blocked() -> None:
    processor = ResponseProcessor(scan_secrets=True)
    parsed = ParsedResponse(
        provider="openai", model="gpt-4o", content_text="-----BEGIN PRIVATE KEY-----\nabc"
    )
    result = processor.process(parsed)
    assert result["allowed"] is False
...
def test_no_crash_when_tracker_none() -> None:
    ...
    parsed = ParsedResponse(
        provider="anthropic", model="claude-sonnet-4", input_tokens=100, output_tokens=100
    )
    result = processor.process(parsed)
...
def test_process_includes_tool_calls_in_output() -> None:
    ...
    parsed = ParsedResponse(
        provider="anthropic", tool_calls=[ToolCall(name="read_file", params={"path": "/tmp/a"})]
    )
    result = processor.process(parsed)
