...
def test_alert_cooldown_rate_limit() -> None:
    manager = AlertManager(
        AlertConfig(enabled=False, cooldown_seconds=60, notify_on=["threat_blocked"])
    )
    manager._config.enabled = True
...
def test_alert_max_per_hour() -> None:
    manager = AlertManager(
        AlertConfig(enabled=False, max_per_hour=1, cooldown_seconds=0, notify_on=["a", "b"])
    )
    manager._config.enabled = True
...
def test_telegram_message_format(monkeypatch: Any) -> None:
    ...
    manager._send_telegram(
        _event(severity=AlertSeverity.CRITICAL, title="Blocked", details="Danger")
    )
...
def test_hourly_counter_reset() -> None:
    manager = AlertManager(
        AlertConfig(enabled=False, max_per_hour=1, cooldown_seconds=0, notify_on=["a"])
    )
    manager._config.enabled = True
