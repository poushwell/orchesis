...
def test_bulk_import_dedup() -> None:
    importer = BulkImporter()
    rows = [
        _incident(incident_id="casura-fixed-1"),
        _incident(incident_id="casura-fixed-1", title="duplicate"),
    ]
    result = importer.import_incidents(rows)
    assert result.imported_count == 1
...
def test_stix_attack_pattern() -> None:
    exporter = StixExporter()
    incident = Incident(
        **_incident(incident_id="stix-2", title="Tool misuse attempt", category="tool_misuse")
    )
