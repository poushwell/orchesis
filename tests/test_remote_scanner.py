...
def test_batch_scan_multiple(monkeypatch) -> None:
    scanner = RemoteSkillScanner()
    monkeypatch.setattr(
        scanner, "scan_clawhub", lambda target: scanner.scan_url("https://example.com/SKILL.md")
    )
    monkeypatch.setattr(
        scanner, "scan_npm_package", lambda target: scanner.scan_url("https://example.com/SKILL.md")
    )
    monkeypatch.setattr(
        scanner, "scan_github", lambda target: scanner.scan_url("https://example.com/SKILL.md")
    )
    monkeypatch.setattr(scanner, "_fetch_text", lambda url: (url, "Safe instructions"))
    reports = scanner.batch_scan(
        ["clawhub:a", "npm:a", "https://github.com/a/b/blob/main/SKILL.md"]
    )
