...
def test_ioc_cli_scan(tmp_path: Path) -> None:
    ...
    path.write_text(
        "gatewayUrl=ws://127.0.0.1:8080/ws token=abcdefghijklmnopqrstuv", encoding="utf-8"
    )
    runner = CliRunner()
...
def test_ioc_dedup_across_variants() -> None:
    ...
    ignore_findings = [
        item for item in inject_findings if "ignore" in str(item.get("matched_pattern", "")).lower()
    ]
