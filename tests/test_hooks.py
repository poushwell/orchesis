...
def test_hook_check_exit_codes(tmp_path: Path) -> None:
    ...
    runner = CliRunner()
    ok = runner.invoke(
        main, ["hook-check", "--tool", "shell", "--input", "echo ok", "--config", str(policy)]
    )
    deny = runner.invoke(
        main, ["hook-check", "--tool", "shell", "--input", "rm -rf /", "--config", str(policy)]
    )
...
def test_hook_log_writes_file(tmp_path: Path) -> None:
    ...
    text = policy.read_text(encoding="utf-8").replace(
        "~/.orchesis/hooks.log", str(log_file).replace("\\", "/")
    )
    policy.write_text(text, encoding="utf-8")
    runner = CliRunner()
    result = runner.invoke(
        main, ["hook-log", "--tool", "shell", "--output", "ok", "--config", str(policy)]
    )
    assert result.exit_code == 0
...
def test_hook_rate_limit_blocks(tmp_path: Path) -> None:
    policy = tmp_path / "policy.yaml"
    policy.write_text(
        _policy_text().replace("max_tool_calls_per_minute: 100", "max_tool_calls_per_minute: 1"),
        encoding="utf-8",
    )
