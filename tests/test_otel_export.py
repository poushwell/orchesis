...
def test_build_payload_resource_attrs() -> None:
    exporter = OTLPSpanExporter(
        OTLPExportConfig(
            enabled=False, resource_attributes={"service.name": "my-svc", "service.version": "1.0"}
        )
    )
    span = _make_span()
    payload = exporter._build_otlp_payload([span])
...
def test_build_payload_events() -> None:
    exporter = OTLPSpanExporter(OTLPExportConfig(enabled=False))
    span = _make_span(events=[{"name": "evt1", "timestamp_ns": 123, "attributes": {"k": "v"}}])
    payload = exporter._build_otlp_payload([span])
...
def test_request_span_genai_attrs() -> None:
    ...
    span = emitter.create_request_span(
        ctx, model="gpt-4o", provider="openai", session_id="s1", agent_id="a1"
    )
...
def test_cache_hit_attrs() -> None:
    span = _make_span(attributes={"orchesis.cache_hit": True, "orchesis.cache_type": "semantic"})
