...
def test_api_forecast_endpoint(tmp_path: Path, monkeypatch) -> None:
    ...
    res1 = client.get(
        "/api/v1/cost-forecast?hours=24", headers={"Authorization": "Bearer test-token"}
    )
    ...
    res2 = client.get(
        "/api/v1/cost-forecast/monthly", headers={"Authorization": "Bearer test-token"}
    )
    assert res2.status_code == 200
    assert "predicted_monthly_cost" in res2.json()

    res3 = client.get(
        "/api/v1/cost-forecast/breakeven?budget=50.0",
