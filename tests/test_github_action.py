...
def _load_action_modules():
    ...
    spec = importlib.util.spec_from_file_location(
        "orchesis_scan_runner", action_dir / "scan_runner.py"
    )
...
def test_config_check_detects_env_secrets_exposure(tmp_path: Path) -> None:
    ...
    p = tmp_path / "mcp.json"
    _write_json(
        p, {"mcpServers": {"x": {"command": "node", "env": {"API_TOKEN": "super-secret-value"}}}}
    )
    findings = checks.run_config_checks(str(p))
...
def test_severity_filter_medium_skips_low_findings(tmp_path: Path) -> None:
    ...
    filtered = [
        item for item in findings if runner.severity_meets_threshold(item.severity, "medium")
    ]
...
def test_severity_filter_critical_only_critical(tmp_path: Path) -> None:
    ...
    findings = runner.run_scan(tmp_path, [mcp], [])
    filtered = [
        item for item in findings if runner.severity_meets_threshold(item.severity, "critical")
    ]
