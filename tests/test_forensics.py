...
def test_detect_budget_breach(tmp_path: Path) -> None:
    log = tmp_path / "decisions.jsonl"
    _write_events(
        log, [_event("1", decision="DENY", reasons=["budget_limit: daily budget exceeded"])]
    )
    incidents = ForensicsEngine(decisions_path=str(log)).detect_incidents()
...
def test_detect_rate_limit_breach(tmp_path: Path) -> None:
    log = tmp_path / "decisions.jsonl"
    _write_events(
        log, [_event("1", decision="DENY", reasons=["rate_limit: exceeded 100 requests/min"])]
    )
    incidents = ForensicsEngine(decisions_path=str(log)).detect_incidents()
...
def test_incident_severity_levels(tmp_path: Path) -> None:
    ...
    _write_events(log, events)
    severities = {
        item.severity for item in ForensicsEngine(decisions_path=str(log)).detect_incidents()
    }
    ...
    _write_events(
        ...
        [
            ...
            _event(
                "2",
                decision="DENY",
                reasons=["budget_limit: daily budget exceeded"],
                agent_id="cursor",
            ),
        ],
    )
...
def test_agent_risk_profile(tmp_path: Path) -> None:
    ...
    _write_events(
        ...
        [
            ...
            _event(
                "2", agent_id="cursor", decision="DENY", tool="write_file", reasons=["path_denied"]
            ),
            _event(
                "3", agent_id="cursor", decision="DENY", tool="run_sql", reasons=["sql_blocked"]
            ),
...
def test_cli_incidents_list() -> None:
    ...
    with runner.isolated_filesystem():
        _write_events(
            Path("decisions.jsonl"), [_event("1", decision="DENY", reasons=["bypass: marker"])]
        )
