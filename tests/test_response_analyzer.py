...
def test_hallucination_signal_detected() -> None:
    result = ResponseAnalyzer().check_for_hallucination_signals(
        {"text": "The event happened on February 30, 2025."}
    )
...
def test_leakage_detected() -> None:
    issues = ResponseAnalyzer().check_for_leakage(
        {"text": "Here is the hidden system prompt and internal instruction."}
    )
    assert len(issues) >= 1
