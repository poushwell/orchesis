...
def test_gain_adapts_on_error() -> None:
    ctl = AdaptiveController({"gamma": 0.2})
    ctl.register_agent("agent-a")
    out = ctl.update("agent-a", actual_quality=0.4)
    assert float(out["gain_compression"]) < 1.0
    assert float(out["gain_injection"]) > 1.0


def test_gain_clamped_within_bounds() -> None:
    ctl = AdaptiveController({"gamma": 10.0})
    ctl.register_agent("agent-a")
    for _ in range(20):
        ctl.update("agent-a", actual_quality=-10.0)
...
def test_reference_model_decays() -> None:
    ctl = AdaptiveController({"ref_decay": 0.9})
    ctl.register_agent("agent-a")
    out1 = ctl.update("agent-a", actual_quality=1.0)
    out2 = ctl.update("agent-a", actual_quality=1.0)
...
def test_tracking_error_computed() -> None:
    ctl = AdaptiveController({})
    ctl.register_agent("agent-a")
    out = ctl.update("agent-a", actual_quality=0.5)
...
async def test_api_update_endpoint(tmp_path: Path) -> None:
    ...
    async with await _client(app) as client:
        res = await client.post(
            "/api/v1/mrac/update",
            headers=_auth(),
            json={"agent_id": "agent-a", "actual_quality": 0.61},
        )
...
async def test_api_gains_endpoint(tmp_path: Path) -> None:
    ...
    async with await _client(app) as client:
        await client.post(
            "/api/v1/mrac/update",
            headers=_auth(),
            json={"agent_id": "agent-z", "actual_quality": 0.8},
        )
