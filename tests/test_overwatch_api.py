...
def _make_app(tmp_path: Path, policy_yaml: str | None = None):
    ...
    policy_path.write_text(
        policy_yaml if isinstance(policy_yaml, str) else _policy_yaml(), encoding="utf-8"
    )
    app = create_api_app(
...
async def test_team_budget_rollup(tmp_path: Path) -> None:
    ...
    async with await _client(app) as client:
        await client.post(
            "/api/v1/overwatch/a1/team", headers=_auth(), json={"team_id": "platform"}
        )
        await client.post(
            "/api/v1/overwatch/a2/team", headers=_auth(), json={"team_id": "platform"}
        )
...
async def test_teams_list_endpoint(tmp_path: Path) -> None:
    ...
    async with await _client(app) as client:
        await client.post(
            "/api/v1/overwatch/a1/team", headers=_auth(), json={"team_id": "research"}
        )
        await client.post(
            "/api/v1/overwatch/a2/team", headers=_auth(), json={"team_id": "engineering"}
        )
