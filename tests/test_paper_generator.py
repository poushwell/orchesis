...
def test_abstract_generated() -> None:
    text = PaperGenerator().generate_abstract()
    assert "Network-Level Context Pipeline" in text
    assert "Zipf" in text
...
def test_full_paper_structure_complete() -> None:
    payload = PaperGenerator().generate_full_paper()
    for key in (
        "title",
        "abstract",
        "introduction",
...
def test_latex_export_creates_files(tmp_path: Path) -> None:
    files = PaperGenerator().export_latex(str(tmp_path / "paper"))
...
def test_markdown_export_creates_files(tmp_path: Path) -> None:
    files = PaperGenerator().export_markdown(str(tmp_path / "paper"))
    names = {Path(path).name for path in files}
...
def test_submission_checklist_items() -> None:
    rows = PaperGenerator().get_submission_checklist()
...
def test_cli_generate_command(tmp_path: Path) -> None:
    ...
    result = runner.invoke(main, ["generate-paper", "--generate", "--output", str(out_dir)])
