...
def _has(
    findings: list, category: str, severity: str | None = None, needle: str | None = None
) -> bool:
...
def test_cve_vulnerable_package(tmp_path: Path) -> None:
    findings = _scan_server(
        tmp_path, {"command": "npx", "args": ["mcp-remote@0.1.15"], "auth": "token"}
    )
...
def test_cve_fixed_version_no_finding(tmp_path: Path) -> None:
    findings = _scan_server(
        tmp_path, {"command": "npx", "args": ["mcp-remote@0.1.17"], "auth": "token"}
    )
...
def test_malicious_package_blocked(tmp_path: Path) -> None:
    findings = _scan_server(
        tmp_path, {"command": "npx", "args": ["mcp-server-free"], "auth": "token"}
    )
...
def test_typosquatting_detected(tmp_path: Path) -> None:
    findings = _scan_server(
        tmp_path, {"command": "npx", "args": ["mcp-server-filesytem"], "auth": "token"}
    )
...
def test_typosquatting_exact_match_no_finding(tmp_path: Path) -> None:
    findings = _scan_server(
        tmp_path, {"command": "npx", "args": ["mcp-server-filesystem"], "auth": "token"}
    )
...
def test_unsafe_npx_y(tmp_path: Path) -> None:
    findings = _scan_server(
        tmp_path, {"command": "npx", "args": ["-y", "mcp-server-fetch"], "auth": "token"}
    )
...
def test_npx_y_with_version_no_finding(tmp_path: Path) -> None:
    findings = _scan_server(
        tmp_path, {"command": "npx", "args": ["-y", "mcp-server-fetch@1.0.0"], "auth": "token"}
    )
...
def test_low_entropy_no_finding(tmp_path: Path) -> None:
    findings = _scan_server(
        tmp_path, {"command": "node", "args": ["normal-argument-123"], "auth": "token"}
    )
...
def test_anthropic_key_detected(tmp_path: Path) -> None:
    findings = _scan_server(
        ...
        {
            "command": "node",
            "args": ["sk-ant-abcdefghijklmnopqrstuvwxyz-abcdefghijklmnopqrstuvwxyz-1234"],
            "auth": "token",
        },
    )
...
def test_privileged_mode(tmp_path: Path) -> None:
    findings = _scan_server(
        tmp_path, {"command": "docker", "args": ["run", "--privileged"], "auth": "token"}
    )
...
def test_docker_socket_mount(tmp_path: Path) -> None:
    findings = _scan_server(
        ...
        {
            "command": "docker",
            "args": ["run", "-v", "/var/run/docker.sock:/var/run/docker.sock"],
            "auth": "token",
        },
    )
...
def test_sensitive_ssh_mount(tmp_path: Path) -> None:
    findings = _scan_server(
        tmp_path,
        {"command": "docker", "args": ["run", "-v", "~/.ssh:/root/.ssh:ro"], "auth": "token"},
    )
...
def test_network_host(tmp_path: Path) -> None:
    findings = _scan_server(
        tmp_path, {"command": "docker", "args": ["run", "--network=host"], "auth": "token"}
    )
...
def test_mount_without_ro(tmp_path: Path) -> None:
    findings = _scan_server(
        tmp_path, {"command": "docker", "args": ["run", "-v", "~/.aws:/root/.aws"], "auth": "token"}
    )
...
def test_running_as_root(tmp_path: Path) -> None:
    findings = _scan_server(
        tmp_path, {"command": "docker", "args": ["run", "alpine:3.20"], "auth": "token"}
    )
...
def test_no_resource_limits(tmp_path: Path) -> None:
    findings = _scan_server(
        ...
        {
            "command": "docker",
            "args": ["run", "--user", "1000:1000", "alpine:3.20"],
            "auth": "token",
        },
    )
...
def test_seccomp_unconfined(tmp_path: Path) -> None:
    findings = _scan_server(
        ...
        {
            "command": "docker",
            "args": ["run", "--security-opt", "seccomp=unconfined"],
            "auth": "token",
        },
    )
...
def test_dangerous_cap_add(tmp_path: Path) -> None:
    findings = _scan_server(
        tmp_path, {"command": "docker", "args": ["run", "--cap-add", "SYS_ADMIN"], "auth": "token"}
    )
...
def test_image_no_digest(tmp_path: Path) -> None:
    findings = _scan_server(
        ...
        {
            "command": "docker",
            "args": ["run", "--user", "1000:1000", "--memory=256m"],
...
def test_clean_config_no_docker_findings(tmp_path: Path) -> None:
    ...
    assert not any(
        item.category == "docker_security" and item.severity in {"critical", "high", "medium"}
        for item in findings
    )
