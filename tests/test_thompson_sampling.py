...
def test_sample_returns_valid_arm() -> None:
    sampler = BanditSampler({"seed": 7, "explore_rate": 0.0})
...
def test_update_shifts_distribution() -> None:
    ...
    assert before in BanditSampler.ARMS
    assert after == "gpt-4o-mini"


def test_best_arm_after_many_updates() -> None:
    sampler = BanditSampler({"seed": 42, "explore_rate": 0.0})
    for _ in range(300):
...
def test_regret_decreases_over_time() -> None:
    sampler = BanditSampler({"seed": 123, "explore_rate": 0.2})
...
def test_per_task_type_different_winners() -> None:
    sampler = BanditSampler({"seed": 9, "explore_rate": 0.0})
    for _ in range(250):
...
def test_proxy_integration() -> None:
    ...
    proxy._thompson_sampler = BanditSampler({"seed": 1, "explore_rate": 0.0})
    ctx = _RequestContext(
    ...
    assert ok is True
    assert ctx.body["model"] in BanditSampler.ARMS
    assert "X-Orchesis-TS-Model" in ctx.session_headers
    assert "bandit_sampler" in ctx.proc_result
