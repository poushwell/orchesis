...
def test_build_record_extracts_cache_info() -> None:
    rec = build_record_from_context(
        SimpleNamespace(proc_result={"cache_hit": True, "cache_type": "exact"})
    )
...
def test_classify_threat_blocked() -> None:
    assert (
        _classify_failure_mode({"blocked": True, "threat_matches": ["x"]}) == "SEC-threat-blocked"
    )
