...
def _record(
    session_id: str = "s1", idx: int = 0, status: int = 200, cost: float = 0.01
) -> SessionRecord:
...
def test_get_stats_fields(tmp_path: Path) -> None:
    ...
    stats = r.get_stats()
    assert (
        "active_sessions" in stats and "total_recorded" in stats and "storage_size_bytes" in stats
    )
...
def test_replayer_dry_run_model_override_no_http(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        replayer,
        "_send_replay",
        lambda body, path="/v1/chat/completions": (_ for _ in ()).throw(RuntimeError("no")),
    )
...
def test_diff_cost_savings_percentage() -> None:
    ...
    replay = SessionReplayer().replay(
        session, ReplayConfig(model_override="gpt-4o-mini", dry_run=True)
    )
    diff = SessionDiffer().diff(session, replay)
