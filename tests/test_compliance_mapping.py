...
def test_llm01_has_full_coverage() -> None:
    assert any(
        m.framework_item.item_id == "LLM01" and m.coverage == "full"
        for m in ORCHESIS_OWASP_MAPPINGS
    )


def test_llm02_has_full_coverage() -> None:
    assert any(
        m.framework_item.item_id == "LLM02" and m.coverage == "full"
        for m in ORCHESIS_OWASP_MAPPINGS
    )
...
def test_map_finding_creates_valid_compliance_finding() -> None:
    engine = ComplianceEngine()
    finding = engine.map_finding(
        "engine", "prompt_injection_scanner", "found injection", Severity.CRITICAL
    )
...
def test_get_coverage_report_returns_correct_structure() -> None:
    ...
    assert {"framework", "total_items", "covered_items", "coverage_percent", "items"} <= set(
        report.keys()
    )
...
def test_get_findings_filters_by_framework() -> None:
    ...
    assert findings
    assert all(
        any(fw == Framework.OWASP_LLM_TOP_10.value for fw, _ in f.framework_mappings)
        for f in findings
    )
...
def test_proxy_compliance_findings_endpoint(tmp_path: Path) -> None:
    proxy, upstream = _make_proxy(tmp_path, "rules: []\n")
    try:
        proxy._compliance_engine.map_finding(
            "engine", "prompt_injection_scanner", "x", Severity.HIGH
        )  # noqa: SLF001
...
def test_session_export_includes_compliance_extensions_when_engine_available(tmp_path: Path) -> None:
