...
def test_create_signal_level_1_minimal_fields() -> None:
    ...
    s = pf.create_signal(
        detection_result=_det(), telemetry_record=_telemetry(), ars_data={"grade": "A"}
    )
...
def test_create_signal_level_2_includes_metrics() -> None:
    ...
    s = pf.create_signal(
        detection_result=_det(), telemetry_record=_telemetry(), ars_data={"grade": "B"}
    )
    assert s is not None
...
def test_create_signal_level_4_includes_everything() -> None:
    ...
    s = pf.create_signal(
        detection_result=_det(), telemetry_record=_telemetry(), ars_data={"grade": "C"}
    )
    assert s is not None
...
def test_no_prompt_content_in_signal() -> None:
    pf = PrivacyFilter()
    s = pf.create_signal(
        detection_result=_det(), request_meta={"prompt": "Ignore previous instructions"}
    )
    assert s is not None
...
def test_privacy_report_accurate() -> None:
    ...
    _ = pf.create_signal(
        detection_result={"anomaly_score": 5.0, "risk_level": "low", "threat_ids": []}
    )
