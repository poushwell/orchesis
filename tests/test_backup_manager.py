...
def _seed_workspace(root: Path) -> None:
    ...
    (root / ".orchesis" / "tenants" / "tenant-a.json").write_text(
        '{"tenant_id":"tenant-a"}\n', encoding="utf-8"
    )
    (root / ".orchesis" / "dna" / "agent-a.json").write_text(
        '{"agent_id":"agent-a"}\n', encoding="utf-8"
    )
...
def test_restore_from_backup(tmp_path: Path) -> None:
    ...
    assert '{"decision":"ALLOW"}' in (tmp_path / ".orchesis" / "decisions.jsonl").read_text(
        encoding="utf-8"
    )
