...
def test_should_inject_below_threshold() -> None:
    ip = ContextInjectionProtocol(
        {"strategy": "reactive", "quality_threshold": 0.7, "max_tokens": 120}
    )
    decision = ip.should_inject(
        {"request_count": 1}, {"quality_score": 0.4, "budget_level": "normal"}
    )
...
def test_no_injection_above_threshold() -> None:
    ...
    decision = ip.should_inject(
        {"request_count": 2}, {"quality_score": 0.9, "budget_level": "normal"}
    )
...
def test_adaptive_strategy_uses_state() -> None:
...
def test_stats_updated() -> None:
    ...
    decision = ip.should_inject(
        {"request_count": 5}, {"quality_score": 0.9, "budget_level": "normal"}
    )
    assert decision["inject"] is True
