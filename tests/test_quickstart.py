...
def test_custom_budget(tmp_path: Path) -> None:
    out = tmp_path / "b.yaml"
    path = QuickstartWizard().run(
        non_interactive=True, preset="generic", budget=42.5, output_path=out
    )
    data = yaml.safe_load(path.read_text(encoding="utf-8"))
...
def test_config_has_required_sections(tmp_path: Path) -> None:
    ...
    data = yaml.safe_load(
        QuickstartWizard()
        .run(non_interactive=True, preset="generic", output_path=out)
...
def test_openclaw_preset_has_semantic_cache_enabled(tmp_path: Path) -> None:
    ...
    data = yaml.safe_load(
        QuickstartWizard()
        .run(non_interactive=True, preset="openclaw", output_path=out)
        .read_text(encoding="utf-8")
    )
    assert data["semantic_cache"]["enabled"] is True
...
def test_all_presets_have_required_fields(tmp_path: Path) -> None:
    for preset in ("openai", "anthropic", "default"):
        ...
        data = yaml.safe_load(
            QuickstartWizard()
            .run(non_interactive=True, preset=preset, output_path=out)
            .read_text(encoding="utf-8")
        )
        assert data["semantic_cache"]["enabled"] is True
        assert data["recording"]["enabled"] is True
