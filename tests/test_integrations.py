...
from orchesis.integrations.slack import (
    SlackIntegration,
...
def test_alert_manager_dispatch_to_telegram(monkeypatch) -> None:  # noqa: ANN001
    sent: list[str] = []
    monkeypatch.setattr(
        TelegramIntegration, "send", lambda self, event: sent.append(event.action) or True
    )
    manager = AlertManager(
        {"alerts": {"telegram": {"enabled": True, "token": "t", "chat_id": "c", "on": ["blocked"]}}}
    )
...
def test_alert_manager_dispatch_to_slack(monkeypatch) -> None:  # noqa: ANN001
    sent: list[str] = []
    monkeypatch.setattr(
        SlackIntegration, "send", lambda self, event: sent.append(event.action) or True
    )
    manager = AlertManager(
        {
            "alerts": {
...
def test_alert_manager_dispatch_to_discord(monkeypatch) -> None:  # noqa: ANN001
    sent: list[str] = []
    monkeypatch.setattr(
        DiscordIntegration, "send", lambda self, event: sent.append(event.action) or True
    )
    manager = AlertManager(
        {
            "alerts": {
...
def test_alert_manager_filter_by_severity(monkeypatch) -> None:  # noqa: ANN001
    sent: list[str] = []
    monkeypatch.setattr(
        SlackIntegration, "send", lambda self, event: sent.append(event.severity) or True
    )
    manager = AlertManager(
        {
            "alerts": {
...
def test_slack_format_budget_message() -> None:
    payload = format_budget_message(
        _event(
            action="BUDGET_EXCEEDED", metadata={"spent": 2.0, "limit": 5.0, "projection_24h": 10.0}
        )
    )
...
def test_webhook_hmac_signature() -> None:
    integration = WebhookIntegration(
        {"enabled": True, "url": "https://example.com/hook", "secret": "s3cr3t"}
    )
