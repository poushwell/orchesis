    ...
    return {
        ...
        "findings": [
            ...
            {"severity": "medium", "description": "Medium issue", "location": "b"},
            {
                ...
                "location": "c",
                "remediation": "Pin package version to X.X.X",
            },
...
def test_severity_filter_excludes_low() -> None:
    if _node_exe() is None:
        return
    result = _run_cli(
        ["--severity", "high", "--format", "json", "--fail-on", "critical"], _sample_findings()
    )
...
def test_output_flag_saves_json(tmp_path: Path) -> None:
    ...
    result = _run_cli(
        ["--output", str(out), "--severity", "medium", "--fail-on", "critical"], _sample_findings()
    )
    assert result.returncode in {0, 1}
    assert out.exists()
...
def test_summary_line_format() -> None:
    ...
    assert (
        ...
        and "high" in result.stdout
        and "medium" in result.stdout
        and "low" in result.stdout
    )
