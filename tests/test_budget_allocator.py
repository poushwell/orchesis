...
def _allocator(total_budget: int = 1000) -> BudgetAllocator:
    return BudgetAllocator({"total_budget": total_budget})
...
def test_api_auction_endpoint(tmp_path: Path, monkeypatch) -> None:
    ...
    client.post(
        "/api/v1/vickrey/bid",
        json={"agent_id": "a", "bid_tokens": 90, "task_priority": "high"},
        headers=headers,
    )
    client.post(
        "/api/v1/vickrey/bid",
        json={"agent_id": "b", "bid_tokens": 80, "task_priority": "medium"},
        headers=headers,
    )
