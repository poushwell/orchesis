...
def test_grade_assigned() -> None:
    cqi = ContextQualityIndex()
    assert (
        cqi.compute_cqs({"iacs": 0.95, "freshness": 0.95, "density": 0.95, "relevance": 0.95})[
            "grade"
        ]
        == "A"
    )
    assert (
        cqi.compute_cqs({"iacs": 0.65, "freshness": 0.65, "density": 0.65, "relevance": 0.65})[
            "grade"
        ]
        == "B"
    )
    assert (
        cqi.compute_cqs({"iacs": 0.45, "freshness": 0.45, "density": 0.45, "relevance": 0.45})[
            "grade"
        ]
        == "C"
    )
    assert (
        cqi.compute_cqs({"iacs": 0.2, "freshness": 0.2, "density": 0.2, "relevance": 0.2})["grade"]
        == "D"
    )
