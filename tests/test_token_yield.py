...
def test_token_yield_computed_correctly() -> None:
    ...
    tracker.record(
        "s1", prompt_tokens=100, completion_tokens=100, cache_hit=False, unique_content_ratio=0.5
    )
...
def test_waste_percent_inverse_of_yield() -> None:
    tracker = TokenYieldTracker()
    tracker.record(
        "s1", prompt_tokens=80, completion_tokens=20, cache_hit=False, unique_content_ratio=0.7
    )
    payload = tracker.get_yield("s1")
...
def test_context_collapse_detected_at_3x() -> None:
    tracker = TokenYieldTracker()
    tracker.record(
        "s1", prompt_tokens=10, completion_tokens=0, cache_hit=False, unique_content_ratio=1.0
    )
    tracker.record(
        "s1", prompt_tokens=31, completion_tokens=0, cache_hit=False, unique_content_ratio=1.0
    )
...
def test_cache_savings_counted() -> None:
    tracker = TokenYieldTracker()
    tracker.record(
        "s1", prompt_tokens=60, completion_tokens=40, cache_hit=True, unique_content_ratio=0.6
    )
    payload = tracker.get_yield("s1")
...
def test_global_stats_aggregated() -> None:
    tracker = TokenYieldTracker()
    tracker.record(
        "a", prompt_tokens=50, completion_tokens=50, cache_hit=False, unique_content_ratio=0.5
    )
    tracker.record(
        "b", prompt_tokens=100, completion_tokens=0, cache_hit=True, unique_content_ratio=0.8
    )
