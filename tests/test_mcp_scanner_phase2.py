...
def test_latest_tag(tmp_path: Path) -> None:
    findings = _scan(
        tmp_path, {"command": "npx", "args": ["mcp-server-fetch@latest"], "auth": "token"}
    )
    assert _has(findings, "version_pinning", "medium")
...
def test_pinned_version_no_finding(tmp_path: Path) -> None:
    findings = _scan(
        tmp_path, {"command": "npx", "args": ["mcp-server-fetch@1.2.3"], "auth": "token"}
    )
...
def test_auto_approve_wildcard(tmp_path: Path) -> None:
    findings = _scan(
        tmp_path, {"command": "node", "args": ["app"], "auth": "token", "autoApprove": ["*"]}
    )
...
def test_auto_approve_true(tmp_path: Path) -> None:
    findings = _scan(
        tmp_path, {"command": "node", "args": ["app"], "auth": "token", "autoApprove": True}
    )
    assert _has(findings, "permissions", "high", "wildcard")
...
def test_auto_approve_small_list_no_finding(tmp_path: Path) -> None:
    findings = _scan(
        tmp_path, {"command": "node", "args": ["app"], "auth": "token", "autoApprove": ["a", "b"]}
    )
...
def test_admin_token_env_key(tmp_path: Path) -> None:
    findings = _scan(
        tmp_path, {"command": "node", "args": ["app"], "auth": "token", "env": {"ADMIN_TOKEN": "x"}}
    )
...
def test_root_key_env(tmp_path: Path) -> None:
    findings = _scan(
        tmp_path, {"command": "node", "args": ["app"], "auth": "token", "env": {"ROOT_KEY": "x"}}
    )
...
def test_normal_env_key_no_finding(tmp_path: Path) -> None:
    findings = _scan(
        tmp_path, {"command": "node", "args": ["app"], "auth": "token", "env": {"API_KEY": "x"}}
    )
...
def test_url_package_install(tmp_path: Path) -> None:
    findings = _scan(
        tmp_path, {"command": "npx", "args": ["https://example.com/pkg.tgz"], "auth": "token"}
    )
...
def test_git_plus_https(tmp_path: Path) -> None:
    findings = _scan(
        tmp_path,
        {"command": "npx", "args": ["git+https://github.com/org/repo.git"], "auth": "token"},
    )
    assert _has(findings, "supply_chain", "high", "URL")
