...
def test_timeline_generated() -> None:
    inspector = RequestInspector()
    inspection = inspector.inspect(
        "req-3", [_event(event_id="req-3", evaluation_order=["parse", "policy", "send"])]
    )
...
def test_api_inspect_endpoint(tmp_path: Path, monkeypatch) -> None:
    ...
    decisions.write_text(
        json.dumps(_event(event_id="req-api"), ensure_ascii=False) + "\n", encoding="utf-8"
    )
    app = create_api_app(policy_path=str(policy), decisions_log=str(decisions))
    client = TestClient(app)
    response = client.get(
        "/api/v1/requests/req-api/inspect", headers={"Authorization": "Bearer test-token"}
    )
...
def test_api_recent_requests(tmp_path: Path, monkeypatch) -> None:
    ...
    decisions = tmp_path / "decisions.jsonl"
    rows = [
        json.dumps(_event(event_id="req-a"), ensure_ascii=False),
        json.dumps(_event(event_id="req-b"), ensure_ascii=False),
    ]
    decisions.write_text("\n".join(rows) + "\n", encoding="utf-8")
    app = create_api_app(policy_path=str(policy), decisions_log=str(decisions))
    client = TestClient(app)
    response = client.get(
        "/api/v1/requests/recent?limit=20", headers={"Authorization": "Bearer test-token"}
    )
    assert response.status_code == 200
    payload = response.json()
