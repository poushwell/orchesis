...
def test_contradiction_detected() -> None:
    engine = ContextTerminationEngine({"enabled": True, "confidence": 0.8})
    messages = [
...
def test_removal_respects_confidence() -> None:
    engine = ContextTerminationEngine({"enabled": True, "confidence": 0.95})
    messages = [
...
def test_proxy_integration() -> None:
    ...
    proxy._context_termination = ContextTerminationEngine({"enabled": True, "confidence": 0.8})
    messages = [
    ...
    ctx = _RequestContext(
        handler=FakeHandler(), body={"messages": messages, "model": "gpt-4o-mini"}
    )
    ok = proxy._phase_context(ctx)
    assert ok is True
    assert "context_termination" in ctx.proc_result
    assert int(ctx.proc_result["context_termination"]["findings_count"]) >= 1
