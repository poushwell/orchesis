...
def test_ack_removes_short_filler() -> None:
    ...
    msgs = [
        _m("user", "a"),
        _m("assistant", "Sure, I'll do that."),
        _m("assistant", "final answer with details"),
    ]
    out, r = c.optimize(msgs)
    assert r.messages_removed >= 1
    assert len(out) < len(msgs)
...
def test_ack_long_message_kept() -> None:
    ...
    out, _ = c.optimize(
        [_m("assistant", long_msg), _m("assistant", "final response is substantial too")]
    )
...
def test_merge_tool_calls_no_merge() -> None:
    c = ContextOptimizer()
    out, r = c.optimize(
        [_m("assistant", "a", [{"name": "x"}]), _m("assistant", "b"), _m("user", "c")]
    )
    assert r.messages_merged == 0
    assert len(out) == 3
...
def test_optimize_normal_conversation() -> None:
    ...
    msgs = [
        _m("system", "policy"),
        _m("user", "hello"),
        _m("assistant", "normal response with details"),
    ]
    _, r = c.optimize(msgs)
...
def test_optimize_long_session() -> None:
    ...
    msgs = [_m("system", "rules")] + [
        _m("user" if i % 2 == 0 else "assistant", "ok") for i in range(30)
    ]
    out, _ = c.optimize(msgs)
    assert len(out) >= 3
...
def test_openclaw_cron_accumulation() -> None:
    ...
    msgs = (
        [_m("system", "policy")]
        + [_m("assistant", "ok")] * 15
        + [_m("user", "final"), _m("assistant", "final answer")]
    )
    out, r = c.optimize(msgs)
    assert len(out) < len(msgs)
    assert r.messages_removed >= 1
...
def test_openclaw_preset_compatible() -> None:
    ...
    out, _ = c.optimize(
        msgs, model="gpt-4o-mini", tools=[{"name": "session_status"}], agent_id="openclaw"
    )
    assert out
