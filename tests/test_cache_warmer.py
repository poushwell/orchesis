...
def test_warming_report_generated() -> None:
    ...
    _ = warmer.warm(
        [{"query": "hello", "frequency": 1, "avg_tokens": 20, "estimated_savings": 0.2}]
    )
...
async def test_api_warm_endpoint(tmp_path: Path) -> None:
    ...
    rows = [_event("common question", tokens=120) for _ in range(4)] + [
        _event("rare question", tokens=50)
    ]
