...
from orchesis.session_export import (
...
def _seed_session(
    recorder: SessionRecorder, session_id: str = "sess-1", *, agent_id: str = "agent-1"
) -> None:
    now = time.time()
    recorder.record(
        SessionRecord(
...
def test_flow_xray_patterns_included(tmp_path: Path) -> None:
    ...
    assert (
        doc["extensions"]["orchesis"]["flow_xray"]["patterns"][0]["pattern_type"] == "token_waste"
    )
...
def test_validate_unknown_version() -> None:
    errors = validate_air(
        {"air": "9.9", "id": "x", "created": "now", "creator": {}, "summary": {}, "turns": []}
    )
...
def test_export_compressed_creates_gzip(tmp_path: Path) -> None:
    recorder = _make_recorder(tmp_path)
    _seed_session(recorder)
    out = export_session_to_air_file(
        "sess-1", str(tmp_path / "compressed"), recorder, compress=True
    )
...
def test_export_endpoint_download_header(tmp_path: Path) -> None:
    ...
    try:
        ...
        code, payload, headers = _get_json(
            proxy._config.port, "/api/sessions/proxy-s2/export?download=true"
        )
        assert code == 200
...
def test_export_endpoint_content_level_param(tmp_path: Path) -> None:
    ...
    try:
        ...
        _, payload, _ = _get_json(
            proxy._config.port, "/api/sessions/proxy-s3/export?content_level=metadata"
        )
