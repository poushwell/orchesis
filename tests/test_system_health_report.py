...
class _StateStub:
    adversarial_tracker = object()
    threat_history = object()
    token_yield = object()
...
def test_security_subsystem_checked() -> None:
    ...
    assert "adversarial_tracker" in security
    assert "threat_history" in security
...
def test_fleet_subsystem_checked() -> None:
    ...
    assert "fleet_consensus" in fleet
    assert "byzantine_detector" in fleet
    assert "context_sync" in fleet
