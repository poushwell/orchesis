...
def test_domain_glob_matching() -> None:
    ...
    assert (
        _decision(policy, "web_search", {"url": "https://api.github.com/repos/x"}).allowed is True
    )
...
def test_command_allowlist_exact_match() -> None:
    policy = {
        "default_action": "deny",
        "capabilities": [
            {"tool": "shell_execute", "allow": {"commands": ["git status", "git log", "ls"]}}
        ],
        "rules": [],
    }
...
def test_capability_can_allow_all_tool_params_when_allow_empty_mapping() -> None:
    policy = {
        "default_action": "deny",
        "capabilities": [{"tool": "web_search", "allow": {}}],
        "rules": [],
    }
    assert (
        _decision(policy, "web_search", {"query": "x", "url": "https://example.com"}).allowed
        is True
    )
