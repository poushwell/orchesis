...
def _event(
    *, session_id: str, cost: float, decision: str = "ALLOW", reasons: list[str] | None = None
) -> dict:
    return {
...
def test_api_endpoints(tmp_path: Path, monkeypatch) -> None:
    ...
    created = client.post(
        ...
        json={"name": "Workflow A", "description": "test"},
        headers=headers,
    )
    ...
    add_one = client.post(
        f"/api/v1/session-groups/{group_id}/sessions", json={"session_id": "s-1"}, headers=headers
    )
    assert add_one.status_code == 200
    add_two = client.post(
        f"/api/v1/session-groups/{group_id}/sessions", json={"session_id": "s-2"}, headers=headers
    )
