...
def _module_results() -> dict[str, dict[str, float]]:
    ...
    for i in range(20):
        entropy.check(
            "agent-a",
            {
                "messages": [{"role": "assistant", "content": f"{_words(90)} {i}"}],
                "timestamp": float(i),
            },
        )

    structural = StructuralPatternDetector(
        {"history_size": 120, "max_pattern_length": 5, "min_occurrences": 3}
    )
    for i in range(50):
        structural.check(
            "agent-a",
            {
                ...
                "model": "gpt-4o-mini",
                "tools": ["read" if i % 2 == 0 else "write", "search"],
            },
        )

    ngram = NgramProfiler(
        {"baseline_messages": 20, "window_size": 10, "min_tokens": 50, "top_k": 200}
    )
    for i in range(25):
    ...
    structural_cold = StructuralPatternDetector(
        {"history_size": 120, "max_pattern_length": 5, "min_occurrences": 3}
    )
    ngram_cold = NgramProfiler(
        {"baseline_messages": 20, "window_size": 10, "min_tokens": 50, "top_k": 200}
    )
    ...
    return {
        "entropy_analyze_message": benchmark(
            lambda: entropy.analyze_message(text_500, role="assistant")
        ),
        "entropy_check_with_baseline": benchmark(
            lambda: entropy.check(
                "agent-a", {"messages": [{"role": "assistant", "content": text_500}]}
            )
        ),
        ...
        "cost_optimizer_optimize": benchmark(
            lambda: cost_optimizer.optimize(request_10["messages"])
        ),
        "context_compression_v2_compress": benchmark(
            lambda: compression_v2.compress(request_10["messages"], budget_tokens=1200)
        ),
        "request_sampler_should_record": benchmark(
            lambda: sampler.should_record({"decision": "ALLOW", "risk_score": 0.2})
        ),
        "context_window_optimizer_optimize": benchmark(
            lambda: context_window_optimizer.optimize_for_model(
                request_10["messages"], "gpt-4o-mini"
            )
        ),
...
def test_perf_quality_control() -> None:
    """QualityController.compute_control < 200 us."""
    from orchesis.quality_control import QualityController

    cc = QualityController()
...
def test_perf_message_consistency() -> None:
    """compute_iacs_full < 500 us for 10 messages."""
    from orchesis.message_consistency import compute_iacs_full

    messages = [
        {"role": "user", "content": f"Message {i} about context management."} for i in range(10)
    ]
    ...
    for a in range(100):
        for i in range(50):
            detector.check(
                f"a{a}",
                {
                    "messages": [{"role": "assistant", "content": f"{_words(60)} {i}"}],
                    "timestamp": float(i),
                },
            )
...
def test_memory_session_risk_1000_sessions() -> None:
    ...
    assert get_memory_bytes(acc.stats) + get_memory_bytes(
        [acc.get_session_state(f"sess{i}") for i in range(1000)]
    ) < _th(5 * 1024 * 1024)
    ...
    for a in range(100):
        ...
        for i in range(20):
            entropy.check(
                aid,
                {
                    "messages": [{"role": "assistant", "content": f"{text} {i}"}],
                    "timestamp": float(i),
                },
            )
...
def _scaling_results() -> dict[str, list[float]]:
    ...
    structural = StructuralPatternDetector(
        {"history_size": 250, "max_pattern_length": 4, "min_occurrences": 3}
    )
    ...
    for size in [100, 500, 1000, 5000]:
        entropy_us.append(
            benchmark(lambda s=size: entropy.analyze_message(_words(s)), iterations=ITERATIONS)[
                "mean_us"
            ]
        )
    ...
    for size in [10, 50, 100, 200]:
        det = StructuralPatternDetector(
            {"history_size": size + 10, "max_pattern_length": 4, "min_occurrences": 3}
        )
        for i in range(size):
            det.check(
                "scale",
                {
                    ...
                    "model": "gpt-4o-mini",
                    "tools": ["read" if i % 2 else "write"],
                },
            )
        structural_us.append(
            benchmark(lambda d=det: d.check("scale", base_req), iterations=ITERATIONS)["mean_us"]
        )

    ngram_us = []
    for size in [100, 500, 1000, 5000]:
        ngram_us.append(
            benchmark(lambda s=size: ngram.build_profile(_words(s)), iterations=ITERATIONS)[
                "mean_us"
            ]
        )
    ...
    for size in [10, 50, 100, 200]:
        ...
        chain_us.append(
            benchmark(lambda m=messages: validate_tool_chain(m), iterations=ITERATIONS)["mean_us"]
        )
