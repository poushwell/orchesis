...
def test_create_experiment() -> None:
    ...
    exp = mgr.create_experiment(
        "Test", [{"name": "control", "weight": 0.5}, {"name": "test", "weight": 0.5}]
    )
...
def test_auto_stop_max_requests() -> None:
    ...
    mgr = ExperimentManager(cfg)
    exp = mgr.create_experiment(
        "X", [{"name": "c", "weight": 0.5}, {"name": "t", "weight": 0.5}], max_requests=10
    )
    mgr.start_experiment(exp.experiment_id)
    for _ in range(6):
...
def test_random_split_distribution() -> None:
    mgr = ExperimentManager()
    exp = mgr.create_experiment(
        "X", [{"name": "a", "weight": 0.5}, {"name": "b", "weight": 0.5}], split_strategy="random"
    )
    mgr.start_experiment(exp.experiment_id)
    counts = {"a": 0, "b": 0}
    for i in range(1000):
...
def test_model_override_applied() -> None:
    ...
    exp = mgr.create_experiment(
        ...
        [
            ...
            {"name": "sonnet", "weight": 0.5, "model_override": "claude-sonnet-4"},
        ],
    )
    mgr.start_experiment(exp.experiment_id)
    found = False
...
def test_no_match_returns_none() -> None:
    mgr = ExperimentManager()
    exp = mgr.create_experiment(
        "X", [{"name": "c", "weight": 1.0}], target_models=["claude-sonnet"]
    )
    mgr.start_experiment(exp.experiment_id)
...
def test_config_overrides_applied() -> None:
    mgr = ExperimentManager()
    exp = mgr.create_experiment(
        "X", [{"name": "c", "weight": 1.0, "config_overrides": {"temperature": 0.7}}]
    )
    mgr.start_experiment(exp.experiment_id)
    a = mgr.assign_variant("s1", "x", "gpt-4o", [])
...
def test_results_insufficient_sample() -> None:
    mgr = ExperimentManager()
    exp = mgr.create_experiment(
        "X", [{"name": "a", "weight": 0.5}, {"name": "b", "weight": 0.5}], min_sample_size=100
    )
    mgr.start_experiment(exp.experiment_id)
    for _ in range(20):
...
def test_results_clear_winner() -> None:
    mgr = ExperimentManager()
    exp = mgr.create_experiment(
        "X", [{"name": "a", "weight": 0.5}, {"name": "b", "weight": 0.5}], min_sample_size=30
    )
    mgr.start_experiment(exp.experiment_id)
    for _ in range(50):
        mgr.record_request(exp.experiment_id, "a", 0.01, 10.0, 50, 1, False, turns=1)
...
def test_variant_comparison_with_outcomes() -> None:
    mgr = ExperimentManager()
    exp = mgr.create_experiment(
        "X", [{"name": "sonnet", "weight": 0.5}, {"name": "opus", "weight": 0.5}]
    )
    mgr.start_experiment(exp.experiment_id)
    for _ in range(40):
...
def test_auto_stop_on_significance() -> None:
    ...
    mgr = ExperimentManager(cfg)
    exp = mgr.create_experiment(
        "X", [{"name": "a", "weight": 0.5}, {"name": "b", "weight": 0.5}], min_sample_size=50
    )
    mgr.start_experiment(exp.experiment_id)
    for _ in range(60):
        mgr.record_request(exp.experiment_id, "a", 0.01, 10.0, 50, 1, False, turns=1)
