...
def test_retry_on_failure(monkeypatch) -> None:
    ...
    emitter = WebhookEmitter(
        WebhookConfig(
            url="https://example.com/hook", retry={"max_retries": 4, "initial_delay": 0.0}
        )
    )
    monkeypatch.setattr(emitter, "_send_payload", _send)
    ok = emitter._send_with_retry(
        "https://example.com/hook", {"x": 1}, WebhookRetryConfig(max_retries=4, initial_delay=0.0)
    )
    assert ok is True
    assert calls["n"] == 3
...
def test_exponential_backoff_delays(monkeypatch) -> None:
    ...
    emitter = WebhookEmitter(
        WebhookConfig(
            url="https://example.com/hook", retry={"max_retries": 3, "initial_delay": 1.0}
        )
    )
    ...
    ok = emitter._send_with_retry(
        ...
        {"x": 1},
        WebhookRetryConfig(
            max_retries=3, initial_delay=1.0, backoff_multiplier=2.0, max_delay=30.0
        ),
    )
    assert ok is False
...
def test_filter_by_event_type() -> None:
    ...
    assert (
        flt.matches({"event_type": "budget_exceeded", "severity": "low", "agent_id": "a"}) is False
    )
...
def test_filter_by_agent_id() -> None:
    ...
    assert (
        flt.matches({"event_type": "threat", "severity": "high", "agent_id": "research_01"}) is True
    )
...
def test_webhook_stats_tracked(monkeypatch) -> None:
    emitter = WebhookEmitter(
        WebhookConfig(url="https://example.com/hook", retry={"max_retries": 0})
    )
...
def test_wildcard_event_matches_all() -> None:
    ...
    assert flt.matches({"event_type": "threat", "severity": "low", "agent_id": "a"}) is True
    assert (
        flt.matches({"event_type": "budget_exceeded", "severity": "critical", "agent_id": "b"})
        is True
    )
