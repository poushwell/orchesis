...
def _decision_event(
    agent_id: str = "agent-1", tool: str = "web_search", decision: str = "ALLOW", cost: float = 0.1
) -> DecisionEvent:
...
def test_profile_builds_correctly(tmp_path: Path) -> None:
    store = SessionFPStore(str(tmp_path / "dna"))
    dna = SessionFP("agent-1")
    dna.observe(
        {
        ...
        {"duration_ms": 120, "cache_hit": True, "decision": "ALLOW"},
    )
...
def test_cost_optimizations_listed(tmp_path: Path) -> None:
    store = SessionFPStore(str(tmp_path / "dna"))
    dna = SessionFP("agent-4")
    for _ in range(3):
        dna.observe(
            {"messages": [{"role": "user", "content": "hello world"}]},
            {"duration_ms": 100, "cache_hit": True, "decision": "ALLOW"},
        )
    dna.compute_baseline()
    store.save(dna)
    decisions_log = tmp_path / "decisions.jsonl"
    events = [
        _decision_event(agent_id="agent-4", cost=0.5),
        _decision_event(agent_id="agent-4", cost=0.4),
    ]
    decisions_log.write_text(
        "\n".join(json.dumps(item.__dict__, ensure_ascii=False) for item in events) + "\n",
        encoding="utf-8",
    )
    profile = AgentIntelligenceProfile().build(
        "agent-4", store, {"score": 0.95}, str(decisions_log)
    )
...
def test_api_endpoint_returns_profile(tmp_path: Path, monkeypatch) -> None:
    ...
    dna_store = SessionFPStore(str(tmp_path / ".orchesis" / "dna"))
    dna = SessionFP("agent-api")
    dna.observe(
        {
    ...
    response = client.get(
        "/api/v1/agents/agent-api/profile", headers={"Authorization": "Bearer test-token"}
    )
