...
def test_nlce_metrics_endpoint(tmp_path: Path, monkeypatch) -> None:
    ...
    response = client.get("/api/v1/pipeline/metrics", headers={"Authorization": "Bearer test-token"})
    assert response.status_code == 200
    payload = response.json()
    assert payload["version"] == "Pipeline v2.0"


def test_confirmed_results_present(tmp_path: Path, monkeypatch) -> None:
    client = _client(tmp_path, monkeypatch)
    payload = client.get(
        "/api/v1/pipeline/metrics", headers={"Authorization": "Bearer test-token"}
    ).json()
...
def test_all_five_theorems_listed(tmp_path: Path, monkeypatch) -> None:
    ...
    assert theorem_ids == {"I1", "I2", "I3", "I4", "I5"}
