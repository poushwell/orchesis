...
def _inc(idx: int, tag: str, vector: str, severity: str = "HIGH") -> dict:
    return {
        ...
        "framework_mappings": {
            "mitre_atlas": ["T0051 Prompt Injection"]
            if "prompt" in tag
            else ["T0001 Reconnaissance"],
...
def test_patterns_analyzed() -> None:
    ...
    report = intel.analyze_patterns(
        [_inc(1, "prompt_injection", "prompt"), _inc(2, "loop", "resource")]
    )
...
def test_clusters_detected() -> None:
    intel = IncidentIntelligence()
    clusters = intel.detect_clusters(
        [_inc(1, "prompt_injection", "prompt"), _inc(2, "prompt_injection", "prompt")]
    )
    assert clusters
...
def test_top_attack_vectors_ranked() -> None:
    ...
    report = intel.analyze_patterns(
        [
            _inc(1, "prompt_injection", "prompt"),
            _inc(2, "prompt_injection", "prompt"),
            _inc(3, "loop", "resource"),
        ]
    )
...
def test_mitre_coverage_computed() -> None:
    intel = IncidentIntelligence()
    coverage = intel.get_mitre_coverage(
        [_inc(1, "prompt_injection", "prompt"), _inc(2, "loop", "resource")]
    )
...
def test_prediction_returned() -> None:
    intel = IncidentIntelligence()
    prediction = intel.predict_next_incident(
        [_inc(1, "prompt_injection", "prompt"), _inc(2, "prompt_injection", "prompt")]
    )
...
def test_emerging_threats_identified() -> None:
    intel = IncidentIntelligence()
    report = intel.analyze_patterns(
        [_inc(1, "novel_tag", "unknown"), _inc(2, "prompt_injection", "prompt")]
    )
