...
from orchesis.vibe_watch import (
    DEFAULT_EXCLUDES,
    DEFAULT_EXTENSIONS,
    AuditResult,
    VibeWatcher,
...
def test_detect_new_file(tmp_path: Path) -> None:
    ...
    assert any(
        event.event_type == "created" and event.filepath.endswith("new.py") for event in events
    )
...
def test_detect_modified_file(tmp_path: Path) -> None:
    ...
    events = watcher._detect_changes(current)  # noqa: SLF001
    assert any(
        event.event_type == "modified" and event.filepath.endswith("m.py") for event in events
    )
...
def test_detect_deleted_file(tmp_path: Path) -> None:
    ...
    events = watcher._detect_changes(current)  # noqa: SLF001
    assert any(
        event.event_type == "deleted" and event.filepath.endswith("gone.py") for event in events
    )
...
def test_nested_directory(tmp_path: Path) -> None:
    ...
    scanned = watcher._scan_files()  # noqa: SLF001
    assert any(
        key.endswith("src\\utils\\x.py") or key.endswith("src/utils/x.py") for key in scanned
    )
...
def test_json_output(tmp_path: Path) -> None:
    ...
    event = WatchEvent(
        event_type="modified", filepath=str((tmp_path / "a.py").resolve()), timestamp=time.time()
    )
    result = AuditResult(
        filepath=event.filepath,
        issues=[{"severity": "high", "message": "x", "line": 1}],
        timestamp=time.time(),
    )
...
def test_callback_mode(tmp_path: Path) -> None:
    ...
    watcher._run_audit = MagicMock(
        return_value=AuditResult(filepath=path, issues=[], timestamp=time.time())
