...
def test_mcp_scanner_no_auth(tmp_path: Path) -> None:
    ...
    path.write_text(
        json.dumps(
            {"mcpServers": {"db-query": {"url": "http://localhost:9000", "tools": ["read_file"]}}}
        ),
        encoding="utf-8",
    )
    report = McpConfigScanner().scan(str(path))
...
def test_cve_detects_vulnerable_mcp_remote(tmp_path: Path) -> None:
    ...
    path.write_text(
        json.dumps(
            {
                "mcpServers": {
                    "pkg": _mcp_base_server({"command": "npx", "args": ["-y", "mcp-remote"]})
                }
            }
        ),
        encoding="utf-8",
...
def test_cve_detects_no_fix_available(tmp_path: Path) -> None:
    ...
    path.write_text(
        json.dumps(
            {
                "mcpServers": {
                    "g": _mcp_base_server({"command": "npx", "args": ["-y", "gemini-mcp-tool"]})
                }
            }
        ),
        encoding="utf-8",
    ...
    report = McpConfigScanner().scan(str(path))
    crit = [
        f for f in report.findings if f.category == "supply_chain_cve" and f.severity == "critical"
    ]
    assert any(
        "no fix" in f.description.lower() or "no safe version" in f.description.lower()
        for f in crit
    )
...
def test_cve_allows_patched_version(tmp_path: Path) -> None:
    ...
    path.write_text(
        json.dumps(
            {
                "mcpServers": {
                    "ok": _mcp_base_server({"command": "npx", "args": ["-y", "mcp-remote@0.1.16"]})
                }
            }
        ),
        encoding="utf-8",
...
def test_typosquatting_catches_close_name(tmp_path: Path) -> None:
    ...
    path.write_text(
        json.dumps(
            {"mcpServers": {"t": _mcp_base_server({"command": "npx", "args": ["-y", "mcp-remte"]})}}
        ),
        encoding="utf-8",
    )
    report = McpConfigScanner().scan(str(path))
...
def test_typosquatting_allows_exact_match(tmp_path: Path) -> None:
    ...
    path.write_text(
        json.dumps(
            {
                "mcpServers": {
                    "e": _mcp_base_server({"command": "npx", "args": ["-y", "mcp-remote@0.1.16"]})
                }
            }
        ),
        encoding="utf-8",
...
def test_registry_unverified_scope(tmp_path: Path) -> None:
    ...
    assert any(
        f.severity == "info"
        and "@evil" in f.description
        and "Unverified npm scope" in f.description
        for f in reg
    )
...
def test_prompt_injection_in_tool_description(tmp_path: Path) -> None:
    ...
    assert inj
    assert any(
        "prompt" in f.description.lower() or "injection" in f.description.lower() for f in inj
    )
...
def test_shadow_server_high_port_localhost(tmp_path: Path) -> None:
    ...
    report = McpConfigScanner().scan(str(path))
    shadow = [
        f for f in report.findings if f.category == "shadow_server" and f.severity == "medium"
    ]
    assert any(
        "high-port" in f.description.lower() or "localhost" in f.description.lower() for f in shadow
    )
...
def test_context_oversharing_cross_service_creds(tmp_path: Path) -> None:
    ...
    report = McpConfigScanner().scan(str(path))
    ctx = [
        f for f in report.findings if f.category == "context_oversharing" and f.severity == "high"
    ]
    assert any(
        "cross-service" in f.description.lower() or "blast" in f.description.lower() for f in ctx
    )
...
def test_context_oversharing_wildcard_permissions(tmp_path: Path) -> None:
    ...
    ctx = [
        f
        for f in report.findings
        if f.category == "context_oversharing" and f.severity == "critical"
    ]
...
def test_cursor_broad_workspace_trust(tmp_path: Path) -> None:
    ...
    report = McpConfigScanner().scan(str(path))
    cur = [
        f for f in report.findings if f.category == "cursor_ide_config" and f.severity == "medium"
    ]
...
def test_cursor_dangerous_skip_permissions(tmp_path: Path) -> None:
    ...
    report = McpConfigScanner().scan(str(path))
    cur = [
        f for f in report.findings if f.category == "cursor_ide_config" and f.severity == "critical"
    ]
...
def test_claude_code_unrestricted_bash(tmp_path: Path) -> None:
    ...
    cc = [
        f
        for f in report.findings
        if f.category == "claude_code_config" and f.severity == "critical"
    ]
...
def test_claude_code_expensive_model_no_budget(tmp_path: Path) -> None:
    ...
    report = McpConfigScanner().scan(str(path))
    cc = [
        f for f in report.findings if f.category == "claude_code_config" and f.severity == "medium"
    ]
...
def test_paperclip_skip_permissions_critical(tmp_path: Path) -> None:
    ...
    report = McpConfigScanner().scan(str(path))
    # Dedup collapses cursor/paperclip/universal skip-permission to one finding (same rule_id).
    assert any(
        f.rule_id == "MCP-DANGEROUSLY-SKIP-PERMISSIONS"
        and f.severity == "critical"
        and "permission" in f.description.lower()
        for f in report.findings
    )
...
def test_paperclip_plaintext_api_key(tmp_path: Path) -> None:
    ...
    assert any(
        "plaintext" in f.description.lower() or "credential" in f.description.lower() for f in pc
    )
...
def test_openclaw_sandbox_disabled_elevated(tmp_path: Path) -> None:
    ...
    report = McpConfigScanner().scan(str(path))
    oc = [
        f for f in report.findings if f.category == "openclaw_config" and f.severity == "critical"
    ]
...
def test_openclaw_auto_approve_tools(tmp_path: Path) -> None:
    ...
    assert any(
        "auto-approve" in f.description.lower() or "approve" in f.description.lower() for f in oc
    )
...
def test_a2a_unsigned_agent_card(tmp_path: Path) -> None:
    ...
    assert any(
        "unsigned" in f.description.lower() or "unverifiable" in f.description.lower() for f in a2a
    )
...
def test_network_external_with_local_access(tmp_path: Path) -> None:
    ...
    report = McpConfigScanner().scan(str(path))
    net = [
        f for f in report.findings if f.category == "network_segmentation" and f.severity == "high"
    ]
    assert any(
        "exfiltration" in f.description.lower() or "local" in f.description.lower() for f in net
    )
...
def test_network_port_collision(tmp_path: Path) -> None:
    ...
    net = [
        f
        for f in report.findings
        if f.category == "network_segmentation" and f.severity == "medium"
    ]
...
def test_network_localhost_dns_rebinding_info(tmp_path: Path) -> None:
    ...
    report = McpConfigScanner().scan(str(path))
    net = [
        f for f in report.findings if f.category == "network_segmentation" and f.severity == "info"
    ]
...
def test_all_findings_have_rule_id(tmp_path: Path) -> None:
    ...
    path.write_text(
        json.dumps(
            {"mcpServers": {"a": {"url": "http://127.0.0.1:9000", "token": "secret-token-ok"}}}
        ),
        encoding="utf-8",
    )
    report = McpConfigScanner().scan(str(path))
    assert report.findings
    assert all(bool(f.rule_id and f.rule_id.strip()) for f in report.findings)


def test_dedup_removes_duplicate_skip_permissions(tmp_path: Path) -> None:
    path = tmp_path / "mcp.json"
    path.write_text(
        json.dumps(
            {
    ...
    report = McpConfigScanner().scan(str(path))
    skip_perm = [f for f in report.findings if f.rule_id == "MCP-DANGEROUSLY-SKIP-PERMISSIONS"]
    assert len(skip_perm) == 1
    assert report.dedup_count >= 2


def test_rule_registry_has_entries() -> None:
    assert McpConfigScanner.get_rule_count() == len(McpConfigScanner.RULE_REGISTRY)
    assert McpConfigScanner.get_rule_count() >= 30
    cve_rules = McpConfigScanner.get_rules_by_category("supply_chain_cve")
    assert any(r.rule_id.startswith("CVE-") for r in cve_rules)


def test_rule_count_matches_emission_sites_approx() -> None:
    path = Path(__file__).resolve().parents[1] / "src" / "orchesis" / "scanner.py"
    text = path.read_text(encoding="utf-8")
    _, _, rest = text.partition("class McpConfigScanner:")
    mcp_section, _, _ = rest.partition("class PolicyScanner:")
    n_scanfinding = mcp_section.count("ScanFinding(")
    assert n_scanfinding >= 110
    assert McpConfigScanner.get_rule_count() <= n_scanfinding


def test_scan_report_has_dedup_stats(tmp_path: Path) -> None:
    path = tmp_path / "mcp.json"
    path.write_text(json.dumps({"mcpServers": {}}), encoding="utf-8")
    report = McpConfigScanner().scan(str(path))
    assert report.rules_checked == McpConfigScanner.get_rule_count()
    assert report.unique_findings == len(report.findings)
...
def test_report_to_json_valid() -> None:
    ...
    assert data["grade"]["score"] == 90
    assert "export_metadata" in data
    assert data["export_metadata"]["orchesis_version"]
    assert data["export_metadata"]["scan_time"] == report.scanned_at
    assert data["findings"]
    assert data["findings"][0]["rule_id"] == "RULE-1"
    assert "dedup_count" in data


def test_report_to_html_contains_findings() -> None:
    report = ScanReport(
        target="/tmp/cfg.json",
        target_type="mcp_config",
        findings=[
            ScanFinding(
                severity="critical",
                category="cat",
                description="Super secret finding text",
                location="mcpServers.s1.command",
                evidence="ev",
                rule_id="R99",
            )
        ],
    ...
    assert "Super secret finding text" in html
    assert "R99" in html
    assert "critical" in html.lower()


def test_report_to_html_self_contained() -> None:
    report = ScanReport(
        target="t.json",
        target_type="mcp_config",
        findings=[],
...
def test_grade_a_plus_no_findings() -> None:
    letter, score = grade_from_findings([])
    assert letter == "A+"
    assert score == 100


def test_grade_f_many_criticals() -> None:
    findings = [
        ScanFinding(severity="critical", category="c", description="d", location="l", evidence="e")
        for _ in range(5)
    ]
    letter, score = grade_from_findings(findings)
    assert letter == "F"
    assert score == 0


def test_grade_calculation_clamps_to_zero() -> None:
    findings = [
        ScanFinding(severity="critical", category="c", description="d", location="l", evidence="e")
        for _ in range(4)
    ]
