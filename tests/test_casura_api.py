...
def test_get_incident_by_id(tmp_path: Path, monkeypatch) -> None:
    ...
    res = client.get(
        f"/api/v1/casura/incidents/{incident_id}", headers={"Authorization": "Bearer test-token"}
    )
    assert res.status_code == 200
...
def test_incident_stats_endpoint(tmp_path: Path, monkeypatch) -> None:
    ...
    _create_incident(client)
    res = client.get(
        "/api/v1/casura/incidents/stats", headers={"Authorization": "Bearer test-token"}
    )
    assert res.status_code == 200
    payload = res.json()
...
def test_intelligence_patterns_endpoint(tmp_path: Path, monkeypatch) -> None:
    ...
    _create_incident(client)
    res = client.get(
        "/api/v1/casura/intelligence/patterns", headers={"Authorization": "Bearer test-token"}
    )
    assert res.status_code == 200
    payload = res.json()
...
def test_mitre_coverage_endpoint(tmp_path: Path, monkeypatch) -> None:
    ...
    _create_incident(client)
    res = client.get(
        "/api/v1/casura/intelligence/mitre-coverage", headers={"Authorization": "Bearer test-token"}
    )
    assert res.status_code == 200
    payload = res.json()
