...
def _post(
    proxy: LLMHTTPProxy, body: dict, headers: dict[str, str] | None = None
) -> tuple[int, dict, dict]:
    req = UrlRequest(
...
def test_normal_chat_session_allowed(tmp_path: Path) -> None:
    ...
    try:
        status, _, _ = _post(
            proxy, {"model": "gpt-4o", "messages": [{"role": "user", "content": "hello"}]}
        )
        assert status == 200
    finally:
        proxy.stop()
    ...
    try:
        for i in range(2):
            status, _, _ = _post(
                ...
                {
                    "model": "gpt-4o",
                    "messages": [{"role": "user", "content": f"Read HEARTBEAT.md run #{i}"}],
                },
            )
            assert status == 200
            time.sleep(1.05)
...
def test_heartbeat_storm_5_in_1min_blocked(tmp_path: Path) -> None:
    ...
    try:
        ...
        blocked = _post(
            proxy,
            {"model": "gpt-4o", "messages": [{"role": "user", "content": "Read HEARTBEAT.md"}]},
        )
...
def test_heartbeat_storm_recovery_after_cooldown(tmp_path: Path) -> None:
    ...
    try:
        ...
        time.sleep(1.05)
        status, _, _ = _post(
            proxy, {"model": "gpt-4o", "messages": [{"role": "user", "content": "new chat"}]}
        )
        assert status == 200
    finally:
        proxy.stop()
...
def test_mixed_chat_and_heartbeat(tmp_path: Path) -> None:
    ...
    try:
        assert (
            _post(proxy, {"model": "gpt-4o", "messages": [{"role": "user", "content": "hello"}]})[0]
            == 200
        )
        assert (
            _post(
...
def test_heartbeat_routed_to_cheap_model(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert _OpenClawUpstreamHandler.last_body.get("model") in {
            "gpt-4o-mini",
...
def test_budget_blocks_after_daily_limit(tmp_path: Path) -> None:
    ...
    try:
        _post(
            proxy,
            {"model": "gpt-4o", "messages": [{"role": "user", "content": "expensive request"}]},
        )
        blocked = _post(
            proxy,
            {"model": "gpt-4o", "messages": [{"role": "user", "content": "expensive request"}]},
...
def test_savings_calculation_accuracy(tmp_path: Path) -> None:
    ...
    try:
        ...
        total = (
            float(data["cache_savings"])
            + float(data["cascade_savings"])
            + float(data["loop_savings"])
        )
...
"""
    ...
    try:
        _post(
            proxy,
            {"model": "gpt-4o", "messages": [{"role": "user", "content": "Read HEARTBEAT.md now"}]},
        )
        assert _OpenClawUpstreamHandler.last_body is not None
    ...
    try:
        ...
        for _ in range(5):
            status, _, _ = _post(
                ...
                {
                    "model": "gpt-4o",
                    "messages": [{"role": "user", "content": "normal repeated content"}],
                },
            )
