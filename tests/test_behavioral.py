...
def test_fingerprint_tracks_tool_distribution() -> None:
    f = BehavioralFingerprint(learning_window=1)
    f.update(
        {"model": "m1", "messages": [], "tools": [{"name": "web_search"}], "estimated_cost": 0.0}
    )
...
def _req(
    prompt_len: int = 40, cost: float = 0.01, model: str = "gpt-4o", tools: list | None = None
) -> dict:
    return {
        "model": model,
...
def test_extract_agent_id_api_key_fallback() -> None:
    rid = extract_agent_id(
        {"headers": {"Authorization": "Bearer TESTTOKEN12345"}, "model": "gpt-4o"}
    )
...
"""
    ...
    try:
        ...
        req = UrlRequest(
            f"http://127.0.0.1:{port}/v1/chat/completions",
            data=json.dumps(
                {"model": "gpt-4o", "messages": [{"role": "user", "content": "x" * 10000}]}
            ).encode("utf-8"),
            headers={
                "Content-Type": "application/json",
                "Authorization": "Bearer x",
