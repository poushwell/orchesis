...
def _urlopen_with_retry(request_or_url, timeout: int = 5, attempts: int = 3):
    """Retry only on transport errors — never on HTTPError.

    Retrying after a failed read could re-send a POST and break stateful demo routes
    (e.g. approve pops an id once; a second POST returns 404).
    """
    last_error = None
...
def test_demo_approve_action() -> None:
    ...
    threading.Thread(target=DemoServer().start, kwargs={"port": port}, daemon=True).start()
    _wait_for_server(port)
    # Defensive: approve is one-shot; ensure id is still pending before POST.
    pending = _get_json(f"http://127.0.0.1:{port}/api/v1/approvals")
    ids = {item.get("approval_id") for item in pending.get("pending", []) if isinstance(item, dict)}
    assert "demo-approval-1" in ids, f"expected demo-approval-1 in pending, got {ids!r}"
    req = UrlRequest(
        f"http://127.0.0.1:{port}/api/v1/approvals/demo-approval-1/approve",
        data=b"{}",
        method="POST",
...
def test_demo_deny_action() -> None:
    ...
    pending = _get_json(f"http://127.0.0.1:{port}/api/v1/approvals")
    ids = {item.get("approval_id") for item in pending.get("pending", []) if isinstance(item, dict)}
    assert "demo-approval-2" in ids, f"expected demo-approval-2 in pending, got {ids!r}"
    req = UrlRequest(
        f"http://127.0.0.1:{port}/api/v1/approvals/demo-approval-2/deny",
        data=b"{}",
        method="POST",
        headers={"Content-Type": "application/json"},
    )
