            ...
            "config_json": json.dumps(
                {
                    "mcpServers": {
                        "x": {
                            "command": "node",
                            "args": ["server.js"],
                            "env": {"OPENAI_API_KEY": "sk-abcdefabcdefabcdefabcdef"},
                        }
                    }
                }
            )
...
def test_scan_config_remote_http_scores_low() -> None:
    result = _handle_scan_config(
        {
            "config_json": json.dumps(
                {"mcpServers": {"r": {"transport": "sse", "url": "http://evil.example/mcp"}}}
            )
        }
    )
    assert result["score"] <= 75
    assert any(
        "HTTP" in item["title"] or "http" in item["description"].lower()
...
def test_scan_config_shell_interpreter_scores_low() -> None:
    result = _handle_scan_config(
        {
            "config_json": json.dumps(
                {"mcpServers": {"sh": {"command": "bash", "args": ["-c", "echo ok"]}}}
            )
        }
    )
...
def test_scan_config_shell_metacharacters_scores_low() -> None:
    result = _handle_scan_config(
        {
            "config_json": json.dumps(
                {"mcpServers": {"x": {"command": "node", "args": ["server.js; rm -rf /"]}}}
            )
        }
    )
    assert result["score"] <= 75
...
def test_scan_config_no_version_pinning_has_medium_finding() -> None:
    result = _handle_scan_config(
        {
            "config_json": json.dumps(
                {"mcpServers": {"x": {"command": "npx", "args": ["mcp-server-fetch"]}}}
            )
        }
    )
...
def test_scan_config_supports_multiple_formats(key: str) -> None:
    result = _handle_scan_config(
        {"config_json": json.dumps({key: {"x": {"command": "node", "args": ["server.js"]}}})}
    )
