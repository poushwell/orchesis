...
def test_critical_gaps_identified() -> None:
    report = ComplianceReportGenerator().generate(
        "agent_x", [{"agent_id": "agent_x", "reasons": []}]
    )
...
async def test_api_endpoint_returns_report(tmp_path: Path) -> None:
    ...
    _write_event(
        decisions_log, agent_id="agent_x", decision="DENY", reasons=["prompt_injection", "audit"]
    )
...
def test_cli_command_runs(tmp_path: Path) -> None:
    ...
    _write_event(
        decisions_path, agent_id="agent_x", decision="DENY", reasons=["prompt_injection", "audit"]
    )
