...
def test_drift_injection_foreign_content() -> None:
    profiler = NgramProfiler(
        {"baseline_messages": 5, "min_tokens": 20, "window_size": 5, "drift_threshold": 0.2}
    )
    base = "status status status status status status status status"
    for _ in range(8):
        profiler.check("a", base)
    has_drift, drift = profiler.check(
        "a", "ZXQxQmFzZTY0QmxvYjEyMzQ1Njc4OWFiY2RlZjAxMjM0NTY3ODlhYmNkZWY="
    )
    assert has_drift
...
def test_drift_model_switch_uniform_shift() -> None:
    ...
    has_drift, drift = profiler.check(
        "a", "therefore furthermore consequently additionally notably"
    )
...
def test_drift_persona_gradual_change() -> None:
    profiler = NgramProfiler(
        {"baseline_messages": 5, "min_tokens": 10, "window_size": 4, "drift_threshold": 0.2}
    )
    for _ in range(8):
...
def test_classify_injection() -> None:
    kind = classify_drift(
        0.7, new_vocab=0.6, missing_vocab=0.1, char_change=0.5, uniform_shift=False, threshold=0.3
    )
...
def test_classify_model_switch() -> None:
    kind = classify_drift(
        0.6, new_vocab=0.2, missing_vocab=0.2, char_change=0.2, uniform_shift=True, threshold=0.3
    )
...
def test_classify_normal() -> None:
    kind = classify_drift(
        0.1, new_vocab=0.0, missing_vocab=0.0, char_change=0.0, uniform_shift=False, threshold=0.3
    )
    assert kind == "normal"
