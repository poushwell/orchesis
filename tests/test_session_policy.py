    ...
    return {
        ...
        "session_policies": {
            "group": {
                ...
                "tool_access": {
                    "mode": "allowlist",
                    "allowed": ["web_search", "read_file", "send_message"],
                },
...
def test_group_session_restricted() -> None:
    decision = evaluate(
        {"tool": "write_file", "params": {}, "cost": 0.0}, _session_policy(), session_type="group"
    )
...
def test_dm_session_normal() -> None:
    decision = evaluate(
        {"tool": "read_file", "params": {}, "cost": 0.0}, _session_policy(), session_type="dm"
    )
...
def test_cli_session_high_trust() -> None:
    decision = evaluate(
        {"tool": "read_file", "params": {}, "cost": 0.0}, _session_policy(), session_type="cli"
    )
    assert decision.allowed is True
...
def test_session_tier_override_lower() -> None:
    ...
    policy = {
        "tool_access": {
            "mode": "tiered",
            "tiers": {"intern": ["read_file"], "operator": {"allowed": ["*"]}},
        },
...
def test_no_session_policy_defaults() -> None:
    ...
    decision = evaluate(
        {"tool": "read_file", "params": {}, "cost": 0.0}, policy, session_type="unknown"
    )
    assert decision.allowed is True
