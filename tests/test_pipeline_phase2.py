...
from orchesis.core.pipeline_engine import AgentState, Phase2_ContextQualityAssessment
...
def test_state_full_3x3_update() -> None:
    phase2 = Phase2_ContextQualityAssessment()
    state = AgentState(
        state_x=[0.5, 1.0, 1.0],
        state_P_full=[1, 0, 0, 0, 1, 0, 0, 0, 1],
        observation_noise=0.2,
    )
    observation = {"cognitive_load": 0.8, "context_quality": 0.7, "coherence": 0.6}
    result = phase2._state_full_update(state, observation)
    assert len(result["x"]) == 3
    assert len(result["P_full"]) == 9
    assert len(result["innovation"]) == 3
    assert float(result["update_magnitude"]) > 0.0
