...
def test_score_perfect_config(tmp_path: Path) -> None:
    (tmp_path / ".gitignore").write_text(".env\n", encoding="utf-8")
    result = OpenClawAuditor().audit_config(
        str(_write_json(tmp_path / "openclaw.json", _clean_config(str(tmp_path / "w"))))
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [
                OpenClawFinding("a", "high", "x", "t", "d", "e", "f", []),
                OpenClawFinding("b", "high", "x", "t", "d", "e", "f", []),
            ],
            "B",
        ),
        (
            [
                OpenClawFinding("a", "critical", "x", "t", "d", "e", "f", []),
                OpenClawFinding("b", "high", "x", "t", "d", "e", "f", []),
                OpenClawFinding("c", "high", "x", "t", "d", "e", "f", []),
            ],
            "D",
        ),
        (
            [
                OpenClawFinding("a", "critical", "x", "t", "d", "e", "f", []),
                OpenClawFinding("b", "critical", "x", "t", "d", "e", "f", []),
            ],
            "D",
        ),
    ],
...
def test_report_text_format(tmp_path: Path) -> None:
    result = OpenClawAuditor().audit_config(
        str(_write_json(tmp_path / "openclaw.json", {"mode": "development"}))
    )
