...
def test_check_config_permissions_world_readable(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda self: [
            self._finding(
                "config_perms", "high", f"{target} is world-readable", "0o644", "chmod 600"
            )
        ],
    )
...
def test_check_config_permissions_owner_only(tmp_path: Path, monkeypatch) -> None:
    ...
    scanner = NetworkExposureScanner()
    monkeypatch.setattr(
        ns_module.NetworkExposureScanner, "check_config_file_permissions", lambda self: []
    )
...
def test_scan_all_aggregates(monkeypatch) -> None:
    scanner = NetworkExposureScanner()
    monkeypatch.setattr(
        scanner, "check_open_ports", lambda: [{"check": "open_port", "severity": "critical"}]
    )
    monkeypatch.setattr(
        scanner, "check_firewall_status", lambda: [{"check": "no_firewall", "severity": "medium"}]
    )
...
""".strip(),
    ...
    with runner.isolated_filesystem():
        ...
        result = runner.invoke(
            main, ["gate", "--policy", "policy.yaml", "--fail-on", "high", "--network"]
        )
