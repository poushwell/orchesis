...
def _assistant_with_calls(ids: list[str], content: str = "calling tools") -> dict:
    return {
        ...
        "content": content,
        "tool_calls": [
            {"id": cid, "type": "function", "function": {"name": "x", "arguments": "{}"}}
            for cid in ids
        ],
    }
...
def test_truncate_never_removes_system_prompt() -> None:
    msgs = [{"role": "system", "content": "important"}] + [
        {"role": "user", "content": "x y z"} for _ in range(20)
    ]
...
def test_truncate_prefers_removing_old_chat_over_tool_chains() -> None:
    msgs = [
        {"role": "user", "content": "old chat " * 20},
        _assistant_with_calls(["a"], "short"),
        _tool_result("a", "r"),
    ]
...
def test_abandoned_chain_stripped() -> None:
    msgs = [_assistant_with_calls(["x"], "text")] + [
...
def test_openclaw_session_new_after_failure() -> None:
    msgs = [_assistant_with_calls(["old"], "x")] + [
        {"role": "user", "content": "new session"} for _ in range(7)
    ]
    out = validate_tool_chain(msgs)
...
def test_openclaw_context_truncation_preserves_active_chain() -> None:
    msgs = [{"role": "system", "content": "rules"}] + [
        {"role": "user", "content": "noise"} for _ in range(20)
    ]
...
def test_malformed_tool_call_no_id() -> None:
    msgs = [
        {"role": "assistant", "content": "x", "tool_calls": [{"function": {"name": "a"}}]},
        _tool_result("a"),
    ]
    out = validate_tool_chain(msgs)
