...
def test_no_keywords_medium_default_model() -> None:
    router = ModelRouter({"default": "gpt-4o"})
    result = router.route(
        "This is a normal request with moderate amount of details and no special triggers"
    )
...
def test_custom_keywords_from_config() -> None:
    router = ModelRouter(
        {"default": "gpt-4o", "low_keywords": ["tinytask"], "high_keywords": ["megatask"]}
    )
...
def test_tool_name_considered_for_simple_hint() -> None:
    router = ModelRouter({"default": "gpt-4o"})
    result = router.route(
        "no keywords here but enough chars to avoid short prompt fallback " * 2,
        tool_name="read_file",
    )
    assert result["complexity"] == "low"
