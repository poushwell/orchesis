...
def test_readme_has_required_sections() -> None:
    ...
    assert (
        ("Supported agents and providers" in text)
        or ("## Works with" in text)
        or ("Works with" in text)
    )
...
def test_readme_has_how_it_works_section() -> None:
    text = Path("README.md").read_text(encoding="utf-8")
    assert "## How it works" in text
    # Functional description, no theoretical claims.
    assert "configurable" in text.lower() or "reliability" in text.lower()
