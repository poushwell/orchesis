...
def test_compression_keeps_high_uci() -> None:
    ...
    messages = [
        low,
        high,
        _msg("user", "rollback strategy for migration again"),
...
def test_proxy_integration() -> None:
    ...
    proxy._uci_compressor = ContentRanker({"enabled": True})
    messages = [_msg("system", "You are helpful")] + [
        _msg("user", "long text " * 80) for _ in range(8)
    ]
    ctx = _RequestContext(
        handler=FakeHandler(),
    ...
    assert ok is True
    assert "content_ranker" in ctx.proc_result
    info = ctx.proc_result["content_ranker"]
