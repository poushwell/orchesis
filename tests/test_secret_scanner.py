...
def test_scan_tool_call() -> None:
    ...
    findings = scanner.scan_tool_call(
        "http_request", {"headers": {"Authorization": "Bearer abcdefghijklmnopqrstuvwxyz123456"}}
    )
    assert findings
...
def test_fuzz_crash_null_high_bytes_round2() -> None:
    """Regression: CI fuzz crash with ] + ec + null + null + 88."""
    scanner = SecretScanner()
    payload = b"]\xec\x00\x00\x88".decode("utf-8", errors="replace")
    result = scanner.scan_text(payload)
    assert isinstance(result, list)


def test_fuzz_crash_high_bytes_round3() -> None:
    from orchesis.contrib.secret_scanner import SecretScanner

    scanner = SecretScanner()
    payload = b"\xec\xff\xff\xff\xec\x00\x00\x88".decode("utf-8", errors="replace")
    result = scanner.scan_text(payload)
    assert isinstance(result, list)


def test_fuzz_crash_iii_equals_round4():
    from orchesis.contrib.secret_scanner import SecretScanner

    scanner = SecretScanner()
    payload = b"iii=\xec\x00\x00\x88".decode("utf-8", errors="replace")
    result = scanner.scan_text(payload)
    assert isinstance(result, list)


def test_fuzz_crash_fe_nulls_round5():
    from orchesis.contrib.secret_scanner import SecretScanner

    scanner = SecretScanner()
    payload = b"\x8c\xfe\xfe\x00\x00\x00".decode("utf-8", errors="replace")
    result = scanner.scan_text(payload)
    assert isinstance(result, list)


def test_fuzz_crash_f5_null_round6():
    from orchesis.contrib.secret_scanner import SecretScanner

    scanner = SecretScanner()
    payload = b"%\xf5\x00\x00\x88".decode("utf-8", errors="replace")
    result = scanner.scan_text(payload)
    assert isinstance(result, list)


def test_fuzz_crash_dash_ec_round7():
    from orchesis.contrib.secret_scanner import SecretScanner

    text = b"-\xec\x00\x00\x88".decode("utf-8", errors="replace")
    result = SecretScanner().scan_text(text)
    assert isinstance(result, list)


def test_fuzz_crash_repeated_0f_control_chars():
    from orchesis.contrib.secret_scanner import SecretScanner

    payload = (b"\x0f" * 45 + b"\x02\x00\x00\x00").decode("utf-8", errors="replace")
    result = SecretScanner().scan_text(payload)
    assert isinstance(result, list)


def test_scanner_caps_input_at_100k():
    from orchesis.contrib.secret_scanner import SecretScanner

    scanner = SecretScanner()
    huge = "sk-proj-" + "A" * 200_000
