...
def test_per_tool_limit_enforcement_two_pass_third_denied() -> None:
    ...
    assert any(
        "rate_limit_exceeded: shell_execute limited to 2/minute" in reason
...
def test_fallback_to_agent_level_limit_when_tool_has_no_specific_limit() -> None:
    policy = {
        ...
        "agents": [
            {"id": "agent_a", "name": "A", "trust_tier": "operator", "rate_limit_per_minute": 2}
        ],
    ...
    assert any(
        "rate_limit: tool 'read_file' exceeded max_requests_per_minute 2" in reason
...
def test_multiple_tools_with_different_limits() -> None:
    ...
    for i in range(2):
        assert evaluate(
            _request("shell_execute"), policy, state=tracker, now=base + timedelta(seconds=i)
        ).allowed
    assert (
        evaluate(
            _request("shell_execute"), policy, state=tracker, now=base + timedelta(seconds=3)
        ).allowed
        is False
    )
    for i in range(3):
        assert evaluate(
            _request("web_search"), policy, state=tracker, now=base + timedelta(seconds=10 + i)
        ).allowed
    assert (
        evaluate(
            _request("web_search"), policy, state=tracker, now=base + timedelta(seconds=20)
        ).allowed
        is False
    )
...
def test_hour_based_per_tool_limit() -> None:
    ...
    assert evaluate(
        _request("send_email"), policy, state=tracker, now=base + timedelta(minutes=10)
    ).allowed
    denied = evaluate(
        _request("send_email"), policy, state=tracker, now=base + timedelta(minutes=20)
    )
    assert denied.allowed is False
...
def test_per_tool_limit_takes_priority_over_agent_rate_limit() -> None:
    policy = {
        ...
        "agents": [
            {"id": "agent_a", "name": "A", "trust_tier": "operator", "rate_limit_per_minute": 1}
        ],
