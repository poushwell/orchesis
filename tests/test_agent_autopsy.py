...
def test_loop_cause_detected() -> None:
    ...
    report = model.perform(
        "sess-1", [_event("sess-1", decision="DENY", reasons=["loop guard triggered"])]
    )
...
def test_budget_cause_detected() -> None:
    model = AgentDiagnostics()
    report = model.perform(
        "sess-1", [_event("sess-1", decision="DENY", reasons=["budget exceeded"])]
    )
...
def test_contributing_factors_found() -> None:
    ...
    assert "high_quality_lock" in factors
...
def test_recommendations_generated() -> None:
    model = AgentDiagnostics()
    report = model.perform(
        "sess-1", [_event("sess-1", decision="DENY", reasons=["budget limit hit"])]
    )
...
def test_severity_scored() -> None:
    model = AgentDiagnostics()
    report = model.perform(
        "sess-1", [_event("sess-1", decision="DENY", reasons=["prompt injection pattern"])]
    )
...
def test_cli_autopsy_command() -> None:
    ...
    with runner.isolated_filesystem():
        _write_events(
            Path("decisions.jsonl"), [_event("sess-1", decision="DENY", reasons=["loop"])]
        )
