...
def test_sse_http_no_tls(tmp_path: Path) -> None:
    findings = _scan_one(
        tmp_path, {"url": "http://example.com/sse", "transport": "sse", "auth": "token"}
    )
...
def test_sse_https_no_finding(tmp_path: Path) -> None:
    findings = _scan_one(
        tmp_path, {"url": "https://example.com/sse", "transport": "sse", "auth": "token"}
    )
...
def test_single_server_no_exfil(tmp_path: Path) -> None:
    findings = _scan_servers(
        tmp_path, {"fs": {"command": "mcp-server-filesystem", "auth": "token"}}
    )
...
def test_shell_server_name(tmp_path: Path) -> None:
    findings = _scan_servers(
        tmp_path, {"shell-access": {"command": "node server.js", "auth": "token"}}
    )
...
def test_sudo_in_args(tmp_path: Path) -> None:
    findings = _scan_one(
        tmp_path, {"command": "bash", "args": ["sudo", "cat", "/etc/hosts"], "auth": "token"}
    )
...
def test_doas_in_args(tmp_path: Path) -> None:
    findings = _scan_one(
        tmp_path, {"command": "bash", "args": ["doas", "cat", "/etc/hosts"], "auth": "token"}
    )
    assert _has(findings, "privilege_escalation", "critical")
...
def test_path_traversal_args(tmp_path: Path) -> None:
    findings = _scan_one(
        tmp_path, {"command": "node", "args": ["../../../etc/passwd"], "auth": "token"}
    )
...
def test_windows_path_traversal(tmp_path: Path) -> None:
    findings = _scan_one(
        tmp_path, {"command": "node", "args": ["..\\..\\windows\\system32"], "auth": "token"}
    )
    assert _has(findings, "path_traversal", "high")
...
def test_cors_present_no_finding(tmp_path: Path) -> None:
    findings = _scan_one(
        tmp_path,
        {
            ...
            "auth": "token",
            "cors": {"allow": ["https://app.example.com"]},
        },
    )
    ...
    findings = _scan_servers(
        tmp_path,
        {
            ...
            "b": {
                "command": "node",
                "auth": "token",
                "env": {"SERVICE_TOKEN": "OtherValue1234567899"},
            },
        },
    )
