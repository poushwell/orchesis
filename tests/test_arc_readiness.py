...
def test_cli_arc_check_exits_0_on_pass(tmp_path: Path) -> None:
    ...
    result = runner.invoke(
        main, ["arc-check", "--agent", "agent-good", "--min-score", "75", "--policy", str(policy)]
    )
...
def test_cli_arc_check_exits_1_on_fail(tmp_path: Path) -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(
        main, ["arc-check", "--agent", "agent-bad", "--min-score", "75", "--policy", str(policy)]
    )
...
def test_api_certify_endpoint(tmp_path: Path, monkeypatch) -> None:
    ...
    response = client.post(
        "/api/v1/arc/certify/agent-good", headers={"Authorization": "Bearer test-token"}
    )
