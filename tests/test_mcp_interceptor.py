...
class _Engine:
    ...
    def evaluate(self, payload: dict, session_type: str = "cli"):
        ...
        if payload.get("tool") in self.deny_tools:
            return Decision(
                allowed=False, reasons=["file_access: blocked"], rules_checked=["file_access"]
            )
...
def test_intercept_tool_list_filter_allowlist() -> None:
    interceptor = McpInterceptor(
        _Engine(), policy={"tool_access": {"mode": "allowlist", "allowed": ["read_file"]}}
    )
    message = {
        "jsonrpc": "2.0",
        "id": 2,
...
def test_intercept_tool_description_ioc_scan() -> None:
    ...
    message = {
        ...
        "id": 2,
        "result": {
            "tools": [{"name": "a", "description": "download from http://45.33.32.156/payload"}]
        },
    }
    out = interceptor.intercept_tool_list(message)
