...
# CI fuzz: invalid UTF-8 bytes in YAML (must not raise UnicodeDecodeError from load_policy).
POLICY_INVALID_UTF8_FUZZ = (
    b"se:: 0\xf89\ni:  0\xf86\n-i: 0\xf86\ni: 0\xf85\ns: 0\xf86\n\n"
    b"s: 0\xf86\ni: ss\ns: 0\xf85\n \ns: 0\xf8_p-"
)
...
def test_pii_detector_pure_binary() -> None:
    ...
    text = binary_blob.decode("utf-8", errors="replace")
    findings = detector.scan_text(text)
    assert isinstance(findings, list)
...
def test_fuzz_crash_policy_repeated_b5_bytes() -> None:
    """Regression: CI fuzz crash with repeated 0xb5 bytes in YAML."""
    from orchesis.config import ConfigError, PolicyError, load_policy

    raw = b'3"\\U' + b"\xb5" * 60 + b"\xc50ru"
...
def test_fuzz_crash_policy_short_f8_byte():
    """Regression: CI fuzz crash with 9-byte YAML containing 0xf8."""
    import os
    import tempfile

    from orchesis.config import load_policy, ConfigError, PolicyError
...
def test_fuzz_crash_policy_yaml_pipe_block():
    import os
    import tempfile
...
def test_load_policy_invalid_utf8_bytes(tmp_path) -> None:
    """CI fuzz bytes: strict UTF-8 read fails; load_policy must surface ConfigError or a dict, never UnicodeDecodeError."""
    from orchesis.config import ConfigError

    load_policy = _load_policy_loader()
    path = tmp_path / "invalid_utf8_fuzz.yaml"
    path.write_bytes(POLICY_INVALID_UTF8_FUZZ)
    with pytest.raises(UnicodeDecodeError):
        path.read_text(encoding="utf-8")
    try:
        result = load_policy(path)
    except UnicodeDecodeError:
        pytest.fail("load_policy must read policy with encoding=utf-8, errors=replace")
    except ConfigError:
        return
    assert isinstance(result, dict)
