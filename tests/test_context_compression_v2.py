...
def test_importance_scoring_keeps_recent() -> None:
    cc = ContextCompressionV2({"algorithm": "importance_scoring", "target_ratio": 0.5})
    context = [
        _msg("user", "old repeated text"),
        _msg("assistant", "latest tool result with error"),
    ]
...
def test_proxy_integration() -> None:
    ...
    proxy._compression_v2 = ContextCompressionV2(
        {"enabled": True, "algorithm": "importance_scoring", "target_ratio": 0.5}
    )
