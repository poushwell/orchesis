...
def test_init_creates_sample_files_and_prints_instruction() -> None:
    ...
    with runner.isolated_filesystem():
        ...
        assert Path(".orchesis/policy.yaml").exists()
        assert Path(".orchesis/data").is_dir()
        assert Path("policy.yaml").exists()
        assert Path("request.json").exists()
        assert "Created .orchesis/policy.yaml" in result.output
        assert "Next: orchesis verify" in result.output
        assert "base_url to http://localhost:8080/v1" in result.output


def test_init_creates_policy_file() -> None:
    runner = CliRunner()
    with runner.isolated_filesystem():
        result = runner.invoke(main, ["init", "--non-interactive", "--minimal"])
        assert result.exit_code == 0
        p = Path(".orchesis/policy.yaml")
        assert p.is_file()
        assert Path(".orchesis/data").is_dir()


def test_init_policy_has_valid_yaml() -> None:
    runner = CliRunner()
    with runner.isolated_filesystem():
        assert runner.invoke(main, ["init", "--non-interactive", "--minimal"]).exit_code == 0
        policy = load_policy(Path(".orchesis/policy.yaml"))
        assert isinstance(policy, dict)
        assert policy["rules"][0]["name"] == "budget_limit"
        assert policy["proxy"]["target"].startswith("https://")


def test_init_force_overwrites() -> None:
    runner = CliRunner()
    with runner.isolated_filesystem():
        Path(".orchesis").mkdir(parents=True)
        Path(".orchesis/policy.yaml").write_text("version: '0.0.0'\nrules: []\n", encoding="utf-8")
        assert runner.invoke(main, ["init", "--non-interactive", "--minimal"]).exit_code != 0
        assert (
            runner.invoke(main, ["init", "--non-interactive", "--minimal", "--force"]).exit_code
            == 0
        )
        text = Path(".orchesis/policy.yaml").read_text(encoding="utf-8")
        assert "proxy:" in text
        assert "version: '0.0.0'" not in text


def test_init_minimal_flag() -> None:
    runner = CliRunner()
    with runner.isolated_filesystem():
        assert runner.invoke(main, ["init", "--non-interactive", "--minimal"]).exit_code == 0
        assert Path(".orchesis/policy.yaml").exists()
        assert not Path("policy.yaml").exists()
        assert not Path("request.json").exists()


def test_init_provider_openai_sets_target() -> None:
    runner = CliRunner()
    with runner.isolated_filesystem():
        assert (
            runner.invoke(
                main, ["init", "--non-interactive", "--minimal", "--provider", "openai"]
            ).exit_code
            == 0
        )
        policy = load_policy(Path(".orchesis/policy.yaml"))
        assert policy["proxy"]["target"] == "https://api.openai.com/v1"


def test_init_provider_anthropic_sets_target() -> None:
    runner = CliRunner()
    with runner.isolated_filesystem():
        assert (
            runner.invoke(
                main, ["init", "--non-interactive", "--minimal", "--provider", "anthropic"]
            ).exit_code
            == 0
        )
        policy = load_policy(Path(".orchesis/policy.yaml"))
        assert policy["proxy"]["target"] == "https://api.anthropic.com/v1"
...
def test_proxy_cli_accepts_preset_flag() -> None:
    runner = CliRunner()
...
def test_proxy_cli_preset_openclaw_merges_loop_detection(monkeypatch) -> None:
    class _FakeProxy:
        last_policy_path = None
        last_config = None
...
""".strip(),
        ...
        monkeypatch.setattr("orchesis.proxy.LLMHTTPProxy", _FakeProxy)
        result = runner.invoke(main, ["proxy", "--preset", "openclaw"])

    assert result.exit_code == 0
    assert _FakeProxy.last_policy_path is not None
    assert _FakeProxy.last_policy_path != "orchesis.yaml"
    merged = load_policy(_FakeProxy.last_policy_path)
    assert merged.get("default_action") == "allow"
    assert merged.get("proxy", {}).get("port") == 8100
    ld = merged.get("loop_detection", {})
    assert isinstance(ld, dict)
    assert ld.get("openclaw_memory_whitelist") is True


def test_proxy_cli_preset_openclaw_has_reset_commands(monkeypatch) -> None:
    class _FakeProxy:
        last_policy_path = None
        last_config = None
...
""".strip(),
    ...
    merged = load_policy(_FakeProxy.last_policy_path)
    ld = merged.get("loop_detection", {})
    assert ld.get("openclaw_reset_commands") == ["/start", "/new", "/reset"]


def test_proxy_cli_preset_paperclip_accepted(monkeypatch) -> None:
    class _FakeProxy:
        last_policy_path = None
        last_config = None
...
""".strip(),
        ...
        monkeypatch.setattr("orchesis.proxy.LLMHTTPProxy", _FakeProxy)
        result = runner.invoke(main, ["proxy", "--preset", "paperclip"])

    assert result.exit_code == 0
    merged = load_policy(_FakeProxy.last_policy_path)
    assert merged.get("paperclip") is not None
...
def test_cli_help_shows_description() -> None:
    ...
    assert result.exit_code == 0
    assert "Runtime Gateway for AI Agents" in result.output
    assert "Block threats" in result.output
...
def test_cli_verify_help_shows_check_count() -> None:
    runner = CliRunner()
    result = runner.invoke(main, ["verify", "--help"])
    assert result.exit_code == 0
    assert str(len(OrchesisVerifier.CHECKS)) in result.output


def test_cli_scan_help_shows_check_count() -> None:
    runner = CliRunner()
