...
class ContextEngine:
    ...
    def optimize(
        ...
        if "trim_tool_results" in active:
            result_messages, tokens_trimmed = self._strategy_trim_tool_results(result_messages)
            if tokens_trimmed > 0:
    ...
    def _strategy_compress_tool_outputs(self, messages: list[dict]) -> tuple[list[dict], int]:
