...
class AgentScorecard:
    ...
    def _compute_metrics(self, agent_id: str, decisions_log: list, period: str) -> dict[str, float]:
        ...
        for row in rows:
            ...
            if isinstance(cache_rate, int | float):
                ...
                cache_count += 1.0
            duration_sum_us += self._as_float(
                row.get("evaluation_duration_us", row.get("evaluation_us", 0.0))
            )
        ...
        cache_percent = self._clamp100(
            (cache_sum / cache_count) * 100.0 if cache_count > 0.0 else 0.0
        )
    ...
    def compute(self, agent_id: str, decisions_log: list, period: str = "7d") -> dict:
        ...
        for item in reversed(history):
            if str(item.get("period", "7d")) == str(period):
                ...
                break
        wow = {
            "overall_score_delta": round(overall - float(previous.get("overall_score", 0.0)), 4)
            if previous
            else 0.0,
            "metrics_delta": {
                key: round(
                    float(metrics.get(key, 0.0))
                    - float((previous or {}).get("metrics", {}).get(key, 0.0)),
                    4,
                )
    ...
    def compute_all(self, decisions_log: list) -> list[dict]:
        ...
        cards = [
            self.compute(agent_id=agent_id, decisions_log=decisions_log, period="7d")
    ...
    def assign_badges(self, scorecard: dict, all_scores: list[dict]) -> list[str]:
        ...
        by_id = {
            str(item.get("agent_id", "")): item for item in all_scores if isinstance(item, dict)
        }
        ...
        best_cost = max(
            (
                float(item.get("metrics", {}).get("cost_efficiency", 0.0))
        ...
        if (
            float(current.get("metrics", {}).get("cost_efficiency", 0.0)) >= best_cost
            and best_cost > 0.0
        ):
            badges.append("💡 Most Efficient")
        best_cache = max(
            (
                float(item.get("metrics", {}).get("cache_hit_rate", 0.0))
                for item in all_scores
                if isinstance(item, dict)
            ),
            default=0.0,
        ...
        if (
            float(current.get("metrics", {}).get("cache_hit_rate", 0.0)) >= best_cache
            and best_cache >= 80.0
        ):
