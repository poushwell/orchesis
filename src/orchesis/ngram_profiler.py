...
def js_divergence(p: dict[str, float], q: dict[str, float], epsilon: float = 1e-12) -> float:
    """Jensen-divergence in [0, 1] with log2."""
...
class NgramProfiler:
    ...
    def __init__(self, config: Optional[dict] = None):
        cfg = config or {}
        self.ngram_sizes = [
            int(n) for n in cfg.get("ngram_sizes", [1, 2, 3]) if int(n) in (1, 2, 3)
        ]
    ...
    def _normalize_counts(self, counts: dict[str, int]) -> dict[str, float]:
        ...
        pairs = sorted(
            ((k, v / total) for k, v in counts.items()), key=lambda kv: kv[1], reverse=True
        )
    ...
    def build_profile(self, text: str) -> NgramProfile:
        ...
        uni = self._normalize_counts(
            self.compute_ngrams(tokens, 1) if 1 in self.ngram_sizes else {}
        )
        bi = self._normalize_counts(self.compute_ngrams(tokens, 2) if 2 in self.ngram_sizes else {})
        tri = self._normalize_counts(
            self.compute_ngrams(tokens, 3) if 3 in self.ngram_sizes else {}
        )
    ...
    def check(self, agent_id: str, text: str) -> tuple[bool, ProfileDrift]:
        ...
        if isinstance(previous_window, NgramProfile) and drift.drift_score >= self.drift_threshold:
            prev_to_current = self.compare_profiles(previous_window, current).drift_score
            if prev_to_current < self.drift_threshold * 0.6 and drift.drift_type not in (
                "injection",
                "model_switch",
            ):
