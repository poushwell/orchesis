...
def _extract_tool_results(
    response_body: dict[str, Any] | None, content_level: str
) -> list[dict[str, Any]]:
...
def _build_turns_from_recording(entries: list[Any], content_level: str) -> list[dict[str, Any]]:
    ...
    for index, raw_entry in enumerate(entries, start=1):
        ...
        if content_level == "full":
            ...
            base_request["messages"] = _strip_messages_for_structure(
        ...
        turn = AIRTurn(
            ...
            tool_results=[]
            if content_level == "metadata"
            else _extract_tool_results(response_body, content_level),
            metadata={
                "provider": entry.provider,
                "error": entry.error,
                "orchesis": _safe_jsonable(
    ...
    for turn in turns:
        ...
        if isinstance(resp, dict):
            ...
            if int(resp.get("status", 0) or 0) >= 400:
                error_count += 1
        for tool in (
            turn.get("tool_results", []) if isinstance(turn.get("tool_results"), list) else []
        ):
...
def _agent_dna_extension(
    behavioral_detector: Any, session_records: list[Any]
) -> dict[str, Any] | None:
    ...
    for item in session_records:
        meta = (
            item.metadata if hasattr(item, "metadata") and isinstance(item.metadata, dict) else {}
        )
