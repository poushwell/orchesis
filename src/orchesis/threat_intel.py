...
BUILT_IN_THREATS: tuple[ThreatSignature, ...] = (
    ...
    ThreatSignature(
        ...
        references=("OWASP ASI-02",),
        content_patterns=(r"(?i)\b(?:DROP\s+TABLE|TRUNCATE\s+TABLE|DELETE\s+FROM\s+\w+\s*$)",),
        owasp_ref="ASI-02",
        default_action=MatchAction.WARN,
    ),
    ThreatSignature(
...
class ThreatMatcher:
    ...
    def _load_custom_signature(self, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            severity = ThreatSeverity.MEDIUM
        content_pats = tuple(str(p) for p in data.get("content_patterns", []) if isinstance(p, str))
        tool_pats = tuple(str(p) for p in data.get("tool_patterns", []) if isinstance(p, str))
        chain_pats = ()
    ...
    def _resolve_action(self, sig: ThreatSignature) -> str:
        action = self._config.severity_actions.get(sig.severity.value, self._config.default_action)
        return str(action)
    ...
    def list_threats(self, category: str = "", severity: str = "") -> list[dict[str, Any]]:
