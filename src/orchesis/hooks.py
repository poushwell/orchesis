...
def evaluate_hook_tool(
    tool_name: str, tool_input: str, config_path: str | None = None
) -> tuple[int, str]:
    path = (
        Path(os.path.expanduser(config_path))
        if isinstance(config_path, str) and config_path.strip()
        else ensure_default_hook_policy()
    )
    if not path.exists():
...
def log_hook_tool(
    tool_name: str, tool_output: str, config_path: str | None = None
) -> tuple[bool, str]:
    path = (
        Path(os.path.expanduser(config_path))
        if isinstance(config_path, str) and config_path.strip()
...
class ClaudeCodeHooks:
    ...
    def install(self) -> HookResult:
        ...
        added = 0
        if not any(
            isinstance(item, dict) and str(item.get("hook", "")).startswith("orchesis hook-check")
            for item in pre_list
        ):
            pre_list.append(dict(self.PRE_HOOK))
            added += 1
        if not any(
            isinstance(item, dict) and str(item.get("hook", "")).startswith("orchesis hook-log")
    ...
    def uninstall(self) -> HookResult:
        ...
        removed = 0
        for key, prefix in (
            ("PreToolUse", "orchesis hook-check"),
            ("PostToolUse", "orchesis hook-log"),
        ):
            rows = hooks.get(key)
    ...
    def status(self) -> dict:
        ...
        if isinstance(pre, list) and any(
            ...
            count += 1
        if isinstance(post, list) and any(
            isinstance(item, dict) and str(item.get("hook", "")).startswith("orchesis hook-log")
            for item in post
        ):
            count += 1
