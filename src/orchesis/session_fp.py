...
class SessionFP:
    ...
    def compute_baseline(self) -> dict:
        ...
        if not self._observations:
            self.baseline = {
                dim: ({} if dim == "topic_distribution" else 0.0) for dim in self.DIMENSIONS
            }
            self._stddevs = {dim: 0.0 for dim in self.DIMENSIONS if dim != "topic_distribution"}
            return dict(self.baseline)

        prompt_lengths = [
            float(item.get("prompt_length", 0.0) or 0.0) for item in self._observations
        ]
    ...
    def load(self, data: dict) -> None:
        ...
        self._observations = (
            [dict(item) for item in observations] if isinstance(observations, list) else []
        )
