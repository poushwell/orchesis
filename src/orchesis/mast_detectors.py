"""Dedicated MAST failure mode detectors."""
...
_SHELL_RE = re.compile(
    r"(?:\brm\s+-rf\b|\bcurl\b.+\|\s*(?:bash|sh)|\bwget\b.+\|\s*(?:bash|sh)|&&|;)"
)
...
class MASTDetectors:
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        self._tool_history: dict[str, deque[tuple[float, str, str]]] = defaultdict(
                    ...
                    out.append(
                        {
                            ...
                            "args": call.get("args", call.get("params", {})),
                        }
                    )
        return out
    ...
    def _check_privilege_escalation(
        self, agent_id: str, request_data: dict[str, Any], context: dict[str, Any]
    ) -> Optional[MASTFinding]:
        _ = agent_id
        ...
        if isinstance(budget, dict):
            ...
            requested = int(
                request_data.get("max_completion_tokens", request_data.get("max_tokens", 0)) or 0
            )
    ...
    def _check_credential_leakage(
        self, agent_id: str, response_data: dict[str, Any], original_request: dict[str, Any]
    ) -> Optional[MASTFinding]:
    ...
    def _check_context_overflow(
        ...
        messages = self._extract_messages(request_data)
        lengths = [
            self._tokens(str(msg.get("content", "") if isinstance(msg, dict) else ""))
            for msg in messages
        ]
        total = sum(lengths)
        ...
        if len(lengths) >= 4:
            grows = sum(
                ...
                if lengths[i] > lengths[i - 1] * 1.5 and lengths[i] > 200
            )
            if grows >= 3:
                return self._mk(
                    "FM-2.3",
    ...
    def _check_output_manipulation(
        self, agent_id: str, response_data: dict[str, Any], original_request: dict[str, Any]
    ) -> Optional[MASTFinding]:
        _ = agent_id
    ...
    def _check_compliance_drift(
        ...
        if isinstance(token_hist, list) and isinstance(token_budget, dict) and token_hist:
            budget_limit = float(
                token_budget.get("daily", token_budget.get("max_tokens", 0)) or 0.0
            )
            if budget_limit > 0:
    ...
    def check_request(
        self, agent_id: str, request_data: dict[str, Any], context: dict | None = None
    ) -> list[MASTFinding]:
        if not self.enabled:
            return []
