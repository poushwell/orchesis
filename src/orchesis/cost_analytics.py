    ...
    def compute(self, decisions_log: list[Any], period_hours: int = 24) -> dict[str, Any]:
        ...
        for item in decisions_log:
            ...
            savings["compression"] += _as_float(
                snapshot.get(
                    "compression_cost_saved_usd", snapshot.get("cost_saved_compression_usd", 0.0)
                )
            )
        ...
        savings["total"] = round(
            savings["cache"] + savings["loop_prevention"] + savings["compression"], 6
        )
        ...
        return {
            ...
            "cost_by_agent": {
                k: round(v, 6)
                for k, v in sorted(agent_cost.items(), key=lambda x: x[1], reverse=True)
            },
