...
@dataclass
class ToolCallCost:
    """Recorded cost line item for one tool invocation (USD, tokens, optional task/model)."""
...
class CostTracker:
    ...
    def record_call(
        ...
        """Append a tool call, update daily/tool/task aggregates, and return the recorded row."""
        if isinstance(cost_override, int | float):
            ...
            cost = (tokens_input / 1000.0 * rates["input"]) + (
                tokens_output / 1000.0 * rates["output"]
            )
        else:
    ...
    def get_daily_total(self, day: str | None = None) -> float:
        """Return total USD spent on ``day`` (ISO date string; default today)."""
    ...
    def check_budget(self, budget_config: dict[str, Any]) -> dict[str, Any]:
        """Compare today's spend against daily and per-tool limits from ``budget_config``."""
        today = date.today().isoformat()
        ...
        result: dict[str, Any] = {
            ...
            "daily_percent": round(
                (daily_spent / daily_budget * 100.0)
                if daily_budget and daily_budget != float("inf")
                else 0.0,
                1,
            ),
    ...
    def reset_daily(self) -> None:
        """Clear today's daily, tool, cascade, and loop savings aggregates for the current date."""
        today = date.today().isoformat()
        with self._lock:
    ...
    def record_cascade_savings(self, original_model: str, actual_model: str, tokens: int) -> float:
        """Estimate USD saved by using ``actual_model`` instead of ``original_model`` for ``tokens`` tokens."""
        ...
        original_cost = (safe_tokens / 1000.0) * (
            (original_rates["input"] + original_rates["output"]) / 2.0
        )
        actual_cost = (safe_tokens / 1000.0) * (
            (actual_rates["input"] + actual_rates["output"]) / 2.0
        )
    ...
    def get_cascade_savings_today(self, day: str | None = None) -> float:
        """Return accumulated cascade model-downgrade savings for the day."""
        safe_day = day or date.today().isoformat()
        with self._lock:
            return float(self._cascade_savings_daily.get(safe_day, 0.0))

    def record_loop_prevented_savings(self, amount_usd: float) -> float:
        """Add USD attributed to loop detection preventing further spend."""
    ...
    def get_loop_prevented_savings_today(self, day: str | None = None) -> float:
        """Return loop-prevention savings total for the day."""
        safe_day = day or date.today().isoformat()
        with self._lock:
    ...
    def estimate_llm_cost(self, model: str, tokens_input: int = 0, tokens_output: int = 0) -> float:
        """Estimate LLM USD cost from configured per-1k token rates (no mutation)."""
        rates = self._model_costs.get(model, self._model_costs["default"])
        in_tokens = max(0, int(tokens_input))
        out_tokens = max(0, int(tokens_output))
        return float(
            (in_tokens / 1000.0 * rates["input"]) + (out_tokens / 1000.0 * rates["output"])
        )

    def to_dict(self) -> dict[str, Any]:
        """Snapshot recent calls and aggregate maps for APIs or persistence."""
        with self._lock:
            return {
