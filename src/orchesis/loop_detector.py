...
class LoopDetector:
    ...
    def __init__(
        ...
        on_detect_cfg = (
            safe_cfg.get("on_detect") if isinstance(safe_cfg.get("on_detect"), dict) else {}
        )
    ...
    def check_request(self, parsed_request: dict[str, Any]) -> LoopDecision:
        ...
        if self._openclaw_memory_whitelist and self._all_tool_calls_are_openclaw_memory_reads(
            safe_tool_calls
        ):
            return LoopDecision("allow", "", 1, 0.0, "none")
        ...
        with self._lock:
            exact_active = self._prune(
                self._exact_patterns.get(exact_hash, []), now, self._exact_window
            )
            exact_active.append(now)
            self._exact_patterns[exact_hash] = exact_active
            exact_count = len(exact_active)

            fuzzy_active = self._prune(
                self._fuzzy_patterns.get(fuzzy_hash, []), now, self._fuzzy_window
            )
            ...
            for table, window in (
                ...
                expired = [
                    key for key, times in table.items() if not self._prune(times, now, window)
                ]
                for key in expired:
                    table.pop(key, None)
            ...
            if exact_count >= self._exact_threshold:
                ...
                saved = (
                    self._estimate_cost_saved(parsed_request, exact_count)
                    if self._track_max_cost_saved
                    else 0.0
                )
                reason = f"Exact loop detected: {exact_count}/{self._exact_threshold}"
                exact_decision = LoopDecision(
                    self._exact_action, reason, exact_count, saved, "exact"
                )

            if fuzzy_count >= self._fuzzy_threshold:
                self._fuzzy_detections += 1
                saved = (
                    self._estimate_cost_saved(parsed_request, fuzzy_count)
                    if self._track_max_cost_saved
                    else 0.0
                )
                reason = f"Fuzzy loop detected: {fuzzy_count}/{self._fuzzy_threshold}"
                fuzzy_decision = LoopDecision(
                    self._fuzzy_action, reason, fuzzy_count, saved, "fuzzy"
                )
...
class ContentLoopDetector:
    ...
    def check(self, content: str, session_id: str = "") -> dict[str, Any]:
        ...
        with self._lock:
            ...
            active = [
                ts for ts in self._history.get(key, []) if (now - ts) <= float(self._window_seconds)
            ]
            active.append(now)
            self._history[key] = active
            count = len(active)
