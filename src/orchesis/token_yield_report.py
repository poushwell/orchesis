    ...
    def generate(self, session_data: list[dict], period: str = "24h") -> dict:
        ...
        if avg_yield < 0.6:
            recommendations.append(
                "Enable stronger context pruning to remove stale prompt history."
            )
        if cache_hit_rate < 0.2:
            recommendations.append("Increase semantic cache coverage to reduce repeat token spend.")
        if collapses > 0:
            recommendations.append(
                "Investigate sessions with context collapse and enforce tighter context budgets."
            )
        if not recommendations:
            recommendations.append(
                "Token Yield is healthy; continue monitoring for drift and periodic regressions."
            )
