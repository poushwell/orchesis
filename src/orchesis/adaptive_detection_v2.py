...
class AdaptiveDetectionV2:
    ...
    def __init__(self, config: dict | None = None):
        ...
        self._ngram = NgramProfiler(
            {"baseline_messages": 5, "window_size": 8, "drift_threshold": 0.3}
        )
        self._weights = {
            "regex": 0.30,
            "structural": 0.20,
    ...
    def detect(self, text: str, context: dict | None = None) -> DetectionResult:
        ...
        self._update_layer_stats(
            "structural", structural_hit or structural_score >= self.structural_threshold
        )
        ...
        entropy_score = max(
            0.0, min(1.0, (entropy_value - 2.5) / max(1e-6, (self.entropy_threshold - 2.5)))
        )
        ...
        new_vocab_ratio = (
            float(details.get("new_vocab_ratio", 0.0)) if isinstance(details, dict) else 0.0
        )
        ...
        session_hit = session_risk >= self.session_risk_threshold or str(
            ctx.get("session_risk_level", "")
        ).lower() in {
            "warn",
            "block",
        }
