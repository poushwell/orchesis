...
class ChannelHealthMonitor:
    ...
    def record_event(self, channel: str, event_type: str, metadata: dict) -> None:
        ...
        with self._lock:
            ...
            if event_type == "inbound":
                ...
                if (
                    int(state.get("outbound_count", 0) or 0) == 1
                    and int(state.get("inbound_count", 0) or 0) == 0
                ):
    ...
    def _check_channel(self, channel: ChannelState) -> list[ChannelAlert]:
        ...
        if severity == "MEDIUM":
            ...
            if (
                isinstance(webhook_last_error, int | float)
                and now - float(webhook_last_error) <= 3600
            ):
                severity = "HIGH"
    ...
    def _recommendation_for(
        ...
        if severity == "HIGH" and channel_type == "whatsapp":
            ...
            hours = (
                (float(session_expiry_ts) - now_ts) / 3600.0
                if isinstance(session_expiry_ts, int | float)
                else 48.0
            )
            return (
    ...
    def get_stats(self) -> dict[str, Any]:
        with self._lock:
            return {
                ...
                "channel_statuses": {
                    ch: info["status"] for ch, info in self._channel_state.items()
                },
            }
