...
class BanditSampler:
        ...
        with self._lock:
            ...
            by_task_out = {
                task: {
                    ...
                    "avg_reward": round(vals["reward_sum"] / vals["count"], 6)
                    if vals["count"] > 0
                    else 0.0,
                }
    ...
    def get_regret(self) -> float:
        ...
        with self._lock:
            if not self._observations:
                return 0.0
            per_task_per_arm: dict[str, dict[str, list[float]]] = defaultdict(
    ...
    def reset_arm(self, arm: str) -> None:
        ...
        with self._lock:
            ...
            self._observations = [
                item for item in self._observations if str(item.get("arm", "")) != model
            ]
