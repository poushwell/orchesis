    ...
    def check_request(self, agent_id: str, request_data: dict[str, Any]) -> BehavioralDecision:
        ...
        if profile.is_learning():
            ...
            return BehavioralDecision(
                action="learning", anomaly_score=0.0, anomalies=[], state="learning"
            )
        now = time.monotonic()
        ...
        current_values = {
            ...
            "prompt_tokens": float(
                BehavioralFingerprint._estimate_prompt_tokens(request_data.get("messages", []))
            ),
        ...
        for dimension, stats in dimensions_map.items():
            ...
            if abs_z >= float(dim_cfg.z_threshold):
                ...
                if self._ACTION_SEVERITY.get(dim_cfg.action, 0) > self._ACTION_SEVERITY.get(
                    highest_action, 0
                ):
