...
class BudgetAdvisor:
    ...
    def analyze(self, decisions_log: list, current_budget: dict) -> dict:
        ...
        for item in decisions_log:
            ...
            loop_saved = _as_float(
                snapshot.get("loop_cost_saved_usd", snapshot.get("cost_saved_loop_usd", 0.0))
            )
            is_loop = bool(snapshot.get("loop_detected")) or (
                "loop" in str(row.get("reasons", "")).lower()
            )
        ...
        daily_limit = _as_float(
            current_budget.get(
                "daily_limit_usd",
                current_budget.get("daily", current_budget.get("budget_daily", 0.0)),
            )
        )
