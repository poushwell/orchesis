...
class ContextCompressionV2:
    ...
    def __init__(self, config: dict | None = None):
        ...
        algo = (
            str(cfg.get("algorithm", "importance_scoring") or "importance_scoring").strip().lower()
        )
    ...
    def compress(self, messages: list[dict], budget_tokens: int) -> dict:
        ...
        with self._lock:
            ...
            self._stats["tokens_saved_total"] = int(self._stats.get("tokens_saved_total", 0)) + int(
                tokens_saved
            )
            self._stats["ratio_sum"] = float(self._stats.get("ratio_sum", 0.0)) + compression_ratio
            self._stats["quality_sum"] = float(self._stats.get("quality_sum", 0.0)) + float(
                quality_score
            )
