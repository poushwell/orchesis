...
class BulkImporter:
    ...
    def _validate_incident(
    ...
    ) -> tuple[Optional[Incident], list[ValidationError]]:
        errors: list[ValidationError] = []
        ...
        if not title:
            errors.append(
                ValidationError(
                    index=index, field="title", message="title is required", incident_id=incident_id
                )
            )
        ...
        incident = Incident(
            ...
            affected_systems=[
                str(item) for item in data.get("affected_systems", []) if isinstance(item, str)
            ],
        )
        return incident, []
    ...
    def import_incidents(self, incidents: list[dict]) -> BulkResult:
        ...
        for index, row in enumerate(incidents):
            incident, row_errors = self._validate_incident(
                row if isinstance(row, dict) else {}, index
            )
            if row_errors:
                failed_count += 1
