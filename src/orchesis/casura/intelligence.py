...
class IncidentIntelligence:
    ...
    def analyze_patterns(self, incidents: list[dict]) -> dict:
        ...
        top_attack_vectors = [
            {"attack_vector": name, "count": count} for name, count in vector_counter.most_common(5)
        ]
        ...
        trend_rows = [
            {"bucket": bucket, "count": count} for bucket, count in sorted(severity_trend.items())
        ]
    ...
    def generate_threat_brief(self, incidents: list[dict]) -> str:
        ...
        total = len(rows)
        top = (
            analysis["top_attack_vectors"][0]["attack_vector"]
            if analysis["top_attack_vectors"]
            else "n/a"
        )
        emerging = (
            ", ".join(analysis["emerging_threats"][:3]) if analysis["emerging_threats"] else "none"
        )
        return (
