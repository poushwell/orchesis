...
<body>
  ...
  <div class="app" id="main-content">
    ...
    <section id="research" class="screen tab-section hidden" role="tabpanel">
      ...
      <div id="pipeline-theorems"></div>
      <div id="pipeline-state"></div>
    </section>
  ...
  <script>
    ...
    function renderResearch({metrics, theorems}) {
      ...
      const resultsRoot = document.getElementById("pipeline-results");
      if(resultsRoot){
        resultsRoot.innerHTML = `
          <div class="panel panel-primary">
            <div class="section-title"><strong>Confirmed pipeline results</strong></div>
      ...
      const theoremRoot = document.getElementById("pipeline-theorems");
      if(theoremRoot){
        const rows = Array.isArray(theoremData.theorems) ? theoremData.theorems : ["I1", "I2", "I3", "I4", "I5"];
        theoremRoot.innerHTML = `
          <div class="panel">
            <div class="section-title"><strong>Pipeline invariants</strong></div>
            <ul>${rows.map((row)=> `<li>${String(row)}</li>`).join("")}</ul>
          </div>
        `;
      }
      const pipelineRoot = document.getElementById("pipeline-state");
      if(pipelineRoot){
    ...
    async function pollResearch() {
      const [metrics, theorems] = await Promise.allSettled([
        ...
        fetch('/api/v1/pipeline/invariants'),
      ]);
      async function toJson(result){
        if(!result || result.status !== "fulfilled"){ return {}; }
        const response = result.value;
