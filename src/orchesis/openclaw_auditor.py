...
class OpenClawAuditor:
    ...
    def audit_instance(self, host: str, port: int = 18789) -> OpenClawAuditResult:
        ...
        try:
            ...
            evidence = (
                f"GET /health -> {status}; Server={server_header or 'n/a'}; bytes={len(body)}"
            )
    ...
    ) -> tuple[dict[str, Any] | None, OpenClawFinding | None]:
    ...
    def _check_hardcoded_secrets(
        self, cfg: dict[str, Any], findings: list[OpenClawFinding]
    ) -> None:
        patterns = (
    ...
    def _check_skill_version_pinning(
        self, cfg: dict[str, Any], findings: list[OpenClawFinding]
    ) -> None:
    ...
    def _has_rate_limit(self, cfg: dict[str, Any]) -> bool:
        rl = (
            cfg.get("rate_limit")
            if isinstance(cfg.get("rate_limit"), dict)
            else cfg.get("rateLimit")
        )
