...
def build_agent_overwatch_snapshot(
    *,
    ...
    for event in events:
        ...
        if dt == today:
            ...
            if event.decision == "DENY":
                ...
                previous_threat_ts = (
                    _parse_ts(str(item["last_threat_at"])) if item["last_threat_at"] else None
                )
    ...
    for item in by_agent.values():
        ...
        budget_remaining = (
            round(budget_limit - spent, 6) if isinstance(budget_limit, float) else None
        )
        ...
        agents.append(
            {
                "id": agent_id,
                "team_id": str(policy.get("team_id"))
                if isinstance(policy.get("team_id"), str)
                else None,
                "status": status,
