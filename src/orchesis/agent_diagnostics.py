...
class AgentDiagnostics:
    ...
    FAILURE_MODES = {
        ...
        "context_poison": "Harmful context detected by context termination",
        "crystal_stale": "Context locked with outdated information",
        "coherence_loss": "Consistency score dropped below threshold",
    ...
    def perform(self, session_id: str, decisions_log: list[dict]) -> dict:
        ...
        session_events = [
            row for row in decisions_log if str(row.get("session_id", "")) == session_id
        ]
    ...
    def _find_contributing_factors(self, events: list[dict]) -> list[str]:
        ...
        for event in events:
            ...
            if float(state.get("psi", 0.0) or 0.0) > 0.85:
                factors.append("high_quality_lock")
            if bool(state.get("slope_alert")):
                factors.append("quality_declining")
