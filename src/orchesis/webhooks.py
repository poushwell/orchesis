...
class WebhookFilter:
    ...
    def __init__(self, config: dict[str, Any]):
        self.events = (
            ...
            if isinstance(config.get("events", ["*"]), list)
            else ["*"]
        )
        self.min_severity = str(config.get("min_severity", "low")).lower()
        self.agent_ids = (
            list(config.get("agent_ids", []))
            if isinstance(config.get("agent_ids", []), list)
            else []
        )
...
class WebhookEmitter(EventEmitter):
    ...
    def _record_delivery_stat(self, success: bool, latency_ms: float) -> None:
        ...
        self._stats["avg_latency_ms"] = (
            (prev_avg * (samples - 1)) + max(0.0, float(latency_ms))
        ) / samples
        self._stats["samples"] = samples

    def _send_with_retry(
        self, url: str, payload: dict[str, Any], config: WebhookRetryConfig
    ) -> bool:
        ...
        for attempt in range(attempts):
            ...
            if attempt < attempts - 1:
                time.sleep(delay)
                delay = min(
                    float(config.max_delay), max(0.0, delay * float(config.backoff_multiplier))
                )
        return False
