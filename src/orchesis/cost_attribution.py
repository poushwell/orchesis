...
class CostAttributionEngine:
    ...
    def __init__(self, config: dict | None = None):
        ...
        self._team_budgets = (
            cfg.get("team_budgets", {}) if isinstance(cfg.get("team_budgets"), dict) else {}
        )
    ...
    def attribute(self, decisions_log: list) -> dict:
        ...
        self._last_daily_team_costs = {
            team: dict(day_costs) for team, day_costs in daily_team_costs.items()
        }
        return result
    ...
    def get_budget_status(self, team: str) -> dict:
        ...
        teams = result.get("by_team", {}) if isinstance(result.get("by_team"), dict) else {}
        used = (
            float(teams.get(name, {}).get("cost", 0.0))
            if isinstance(teams.get(name), dict)
            else 0.0
        )
