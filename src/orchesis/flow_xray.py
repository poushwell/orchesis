...
class FlowAnalyzer:
    ...
    def __init__(self, config: FlowXRayConfig | dict[str, Any] | None = None):
        if isinstance(config, FlowXRayConfig):
            ...
            self._config = FlowXRayConfig(
                ...
                token_waste_stddev_threshold=max(
                    0.5, float(cfg.get("token_waste_stddev_threshold", 2.0))
                ),
                latency_spike_threshold=max(
                    0.1, min(1.0, float(cfg.get("latency_spike_threshold", 0.5)))
                ),
                suspicious_tool_chains=[
                    [str(tool).strip() for tool in chain if str(tool).strip()]
                    for chain in cfg.get(
                        "suspicious_tool_chains", FlowXRayConfig().suspicious_tool_chains
                    )
    ...
    def _hash_messages(messages: Any) -> str:
        try:
            raw = json.dumps(
                messages if isinstance(messages, list) else [], ensure_ascii=False, sort_keys=True
            )
        except Exception:
            raw = ""
    ...
    def _hash_tool_input(tool_name: str, tool_input: Any) -> str:
        try:
            raw = json.dumps(
                tool_input if isinstance(tool_input, dict) else tool_input,
                ensure_ascii=False,
                sort_keys=True,
            )
        except Exception:
    ...
    def record_request(
        self, session_id: str, model: str, messages: list, tools: list[str] | None = None
    ) -> str:
        now = time.monotonic()
        with self._lock:
            ...
            if isinstance(last, str) and last in graph.nodes:
                graph.add_edge(
                    FlowEdge(source_id=last, target_id=node_id, edge_type=EdgeType.SEQUENTIAL)
                )
            ...
            for prev_ts, prev_node, prev_hash in reversed(llm_history):
                ...
                if prev_hash == node.metadata["messages_hash"]:
                    graph.add_edge(
                        FlowEdge(source_id=prev_node, target_id=node_id, edge_type=EdgeType.RETRY)
                    )
                    prev_model = (
                        graph.nodes.get(prev_node).model if prev_node in graph.nodes else ""
                    )
    ...
    def record_response(
        ...
        with self._lock:
            ...
            for call in tool_calls or []:
                ...
                graph.add_edge(
                    FlowEdge(source_id=node_id, target_id=tool_node_id, edge_type=EdgeType.TRIGGERS)
                )
                last = self._last_node_by_session.get(session_id)
                if isinstance(last, str) and last in graph.nodes and last != node_id:
                    graph.add_edge(
                        FlowEdge(
                            source_id=last, target_id=tool_node_id, edge_type=EdgeType.SEQUENTIAL
                        )
                    )
    ...
    def record_tool_result(
        self, session_id: str, tool_node_id: str, result_size: int, status: str = "ok"
    ) -> str:
        now = time.monotonic()
        with self._lock:
            graph = self._get_or_create_graph(session_id, now)
            ...
            graph.add_edge(
                FlowEdge(source_id=tool_node_id, target_id=node_id, edge_type=EdgeType.RESPONDS_TO)
            )
            self._last_node_by_session[session_id] = node_id
            return node_id
    ...
    def list_sessions(self) -> list[dict[str, Any]]:
        with self._lock:
            ...
            return sorted(
                sessions, key=lambda item: float(item.get("start_time", 0.0)), reverse=True
            )
    ...
    def _detect_context_loss(self, graph: FlowGraph) -> list[DetectedPattern]:
        ...
        for node in graph.get_node_sequence():
            ...
            if (
                key in seen
                and (node.timestamp - seen[key]) > self._config.redundancy_window_seconds
            ):
                out.append(
                    DetectedPattern(
    ...
    def _detect_excessive_retries(self, graph: FlowGraph) -> list[DetectedPattern]:
        ...
        while i < len(seq):
            ...
            while j < len(seq):
                nxt = seq[j]
                if (
                    nxt.node_type == NodeType.LLM_CALL
                    and nxt.model == node.model
                    and nxt.status in {"error", "timeout"}
                ):
    ...
    def _detect_token_waste(self, graph: FlowGraph) -> list[DetectedPattern]:
        ...
        for node in llm_nodes:
            if float(node.tokens_in) > threshold and node.tokens_out <= max(
                1, node.tokens_in // 20
            ):
                out.append(
                    DetectedPattern(
    ...
    def _detect_sequential_bottleneck(self, graph: FlowGraph) -> list[DetectedPattern]:
        ...
        for node in tools:
            ...
            prev = run[-1]
            if (
                node.timestamp - prev.timestamp
            ) <= self._config.redundancy_window_seconds and node.tool_name != prev.tool_name:
                run.append(node)
            else:
                if len(run) >= 3:
                    out.append(
    ...
    def _compute_topology(self, graph: FlowGraph) -> FlowTopology:
        ...
        while queue:
            ...
            for target in adj.get(current, []):
                depth_by_node[target] = max(
                    depth_by_node.get(target, 1), depth_by_node.get(current, 1) + 1
                )
                in_degree[target] -= 1
        ...
        critical_path = [
            n.node_id
            for n in sorted(seq, key=lambda n: float(n.latency_ms), reverse=True)[
                : min(5, len(seq))
            ]
        ]
        return FlowTopology(
            ...
            tool_diversity=round(
                (unique_tools / total_tool_calls) if total_tool_calls > 0 else 0.0, 6
            ),
    ...
    def _build_summary(patterns: list[DetectedPattern]) -> FlowSummary:
        ...
        for item in patterns:
            health -= severity_penalty.get(item.severity, 0.05) * max(
                0.2, min(1.0, item.confidence)
            )
    ...
    def analyze_session(self, session_id: str) -> FlowAnalysis | None:
        ...
        patterns.sort(
            key=lambda p: (
                {"critical": 0, "high": 1, "medium": 2, "low": 3, "info": 4}.get(p.severity, 9),
                -p.confidence,
            )
        )
