...
Applied: maintain stable context quality (quality score) within target range
...
class StateBalancer:
    """Negative feedback state_balancer for context quality."""
    ...
    def measure(self, quality_score: float) -> dict:
        """Measure quality score and determine homeostatic response."""
        with self._lock:
            self._history.append(float(quality_score))
            if len(self._history) > 1000:
                self._history = self._history[-1000:]

        deviation = float(quality_score) - self.setpoint
    ...
    def get_equilibrium_stats(self) -> dict:
        with self._lock:
            ...
            in_band = sum(1 for quality_score in self._history if abs(quality_score - self.setpoint) <= self.band)
            return {
                "measurements": len(self._history),
                "avg_quality": round(sum(self._history) / len(self._history), 4),
