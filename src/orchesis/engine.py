...
def _resolve_per_tool_rate_limit(
    *,
    ...
    session_config = (
        ...
        and isinstance(session_policies.get(session_type), dict)
        else None
    )
    base_tool_access = (
        policy.get("tool_access") if isinstance(policy.get("tool_access"), dict) else None
    )
...
def _constraint_denies(
    values: list[str], patterns: list[str], *, normalize_path: bool = False
) -> bool:
...
def _evaluate_capabilities(
    *,
    ...
    for item in matching:
        ...
        deny_domains = [
            str(pattern) for pattern in deny.get("domains", []) if isinstance(pattern, str)
        ]
        deny_commands = [
            str(pattern) for pattern in deny.get("commands", []) if isinstance(pattern, str)
        ]
    ...
    if not matching:
        reasons.append(
            f"capabilities: tool '{tool}' is not explicitly allowed (default_action=deny)"
        )
        return reasons, checked
    ...
    for item in matching:
        ...
        allow_paths = [
            str(pattern) for pattern in allow.get("paths", []) if isinstance(pattern, str)
        ]
        allow_domains = [
            str(pattern) for pattern in allow.get("domains", []) if isinstance(pattern, str)
        ]
        allow_commands = [
            str(pattern) for pattern in allow.get("commands", []) if isinstance(pattern, str)
        ]
        if (
...
def _evaluate_tool_access_control(
    *,
    ...
    session_budget = (
        ...
        and isinstance(session_config.get("budget_per_session"), int | float)
        else None
    )
...
def _check_token_limits(
    *,
    ...
    max_per_call = session_config.get("max_tokens_per_call") or token_limits.get(
        "max_tokens_per_call"
    )
    ...
    max_per_session = session_config.get("max_tokens_per_session") or token_limits.get(
        "max_tokens_per_session"
    )
    if isinstance(max_per_session, int | float) and (session_used + estimated) > int(
        max_per_session
    ):
        reasons.append(
    ...
    *,
        ...
        if denied_paths:
            for raw in paths:
                ...
                if any(normalized.startswith(prefix) for prefix in denied_paths):
                    reasons.append(
                        f"sandbox: filesystem path '{normalized}' denied for session '{session_type}'"
                    )
                    break
        if not reasons:
            allowed_paths = [
        ...
        if not reasons and bool(filesystem.get("deny_hidden_files")):
            for raw in paths:
                if _looks_like_hidden_file(raw):
                    reasons.append(
                        f"sandbox: hidden file access denied for session '{session_type}'"
                    )
                    break
        ...
        if isinstance(network, dict):
            ...
            for raw in values:
                ...
                if deny_ip and _is_ip_address(safe_host):
                    reasons.append(
                        f"sandbox: IP address '{safe_host}' denied for session '{session_type}'"
                    )
                    break
                if denied_domains and any(
                    safe_host == denied or safe_host.endswith(f".{denied}")
                    for denied in denied_domains
                ):
                    reasons.append(
                        f"sandbox: domain '{safe_host}' denied for session '{session_type}'"
                    )
                    break
                if allowed_domains and not any(
                    safe_host == allowed or safe_host.endswith(f".{allowed}")
                    for allowed in allowed_domains
                ):
                    reasons.append(
                        f"sandbox: domain '{safe_host}' not in allowlist for session '{session_type}'"
                    )
                    break
    ...
    if not reasons:
        ...
        if isinstance(execution, dict):
            ...
            subprocess_like = (
                "subprocess" in normalized_tool or "subprocess" in command_text.lower()
            )
            ...
            allowed_commands = [
                item
                for item in execution.get("allowed_commands", [])
                if isinstance(item, str) and item.strip()
            ]
...
def _check_channel_policy(
    *,
    ...
    channel_config = (
        ...
        if isinstance(channel_policies, dict)
        and isinstance(channel_policies.get(normalized_channel), dict)
        else None
    )
    ...
    denied_tools = {
        item
        for item in channel_config.get("denied_tools", [])
        if isinstance(item, str) and item.strip()
    }
    ...
    resolved_tier = min_tier(
        channel_tier if isinstance(channel_tier, str) else None, effective_tier
    )
    ...
    *,
    ...
    if isinstance(daily_budget, int | float):
        spent = (
            ...
            else state.get_agent_budget_spent(agent_id, window_seconds=86400, session_id=session_id)
        )
        if cost is None:
            projected = spent
        elif cost < 0:
...
class PolicyEngine:
    ...
    def policy(self) -> dict[str, Any]:
        """Active policy mapping passed to :func:`evaluate`."""
        return self._policy

    def set_policy(self, policy: dict[str, Any]) -> None:
        """Replace the policy used for subsequent :meth:`evaluate` calls."""
        self._policy = policy
    ...
    def evaluate(
        ...
        *,
        ...
        """Evaluate ``request`` against this engine's policy and shared state."""
        return evaluate(
            request=request,
            policy=self._policy,
            state=self._state,
...
def evaluate(
    ...
    *,
    ...
    tool_access_reasons, tool_access_checked, effective_tier_for_eval = (
        _evaluate_tool_access_control(
            policy=policy,
            request=request,
            state=tracker,
            agent_id=agent_id,
    ...
    policy_tool_costs = (
        policy.get("tool_costs")
        if isinstance(policy, dict) and isinstance(policy.get("tool_costs"), dict)
        else {}
    )
    policy_loop_cfg = (
        policy.get("loop_detection")
        if isinstance(policy, dict) and isinstance(policy.get("loop_detection"), dict)
        else None
    )
    policy_model_routing_cfg = (
        policy.get("model_routing")
        if isinstance(policy, dict) and isinstance(policy.get("model_routing"), dict)
        else None
    )
    current_tool_cost = (
    ...
    if (
        allowed
        and isinstance(policy_loop_cfg, dict)
        and bool(policy_loop_cfg.get("enabled", False))
    ):
        ...
        try:
            ...
            if allowed and bool(budget_status.get("soft_limit_reached", False)):
                ...
                if on_soft == "block":
                    ...
                    if isinstance(policy_model_routing_cfg, dict) and bool(
                        policy_model_routing_cfg.get("enabled", False)
                    ):
            ...
            if (
                allowed
                and isinstance(per_task_budget, int | float)
    ...
    if decision.allowed:
        task_id = (
            safe_params.get("_task_id")
            if isinstance(safe_params, dict) and isinstance(safe_params.get("_task_id"), str)
            else None
        )
        model = (
            safe_params.get("_model")
            if isinstance(safe_params, dict) and isinstance(safe_params.get("_model"), str)
            else None
        )
