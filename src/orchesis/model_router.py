...
class ModelRouter:
    ...
    def _classify(self, prompt: str, tool_name: str | None = None) -> str:
        ...
        if (
            any(token in tool_hint for token in ("search", "read", "write", "notify"))
            and len(prompt_lower) < 150
        ):
    ...
    def get_savings_estimate(self) -> dict[str, Any]:
        ...
        calls_downgraded = sum(1 for item in self._routing_log if item.get("complexity") == "low")
