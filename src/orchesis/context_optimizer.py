...
class ContextOptimizer:
    ...
    def estimate_tokens(
        self, messages: list[dict], tools: list | None = None, model: str = ""
    ) -> int:
    ...
    def _dedup_system_prompt(
        self, messages: list[dict[str, Any]], agent_id: str, result: OptimizationResult
    ) -> list[dict[str, Any]]:
        first_kept = False
        ...
        if agent_id and first_kept:
            first_system = next(
                (m for m in out if str(m.get("role", "")).lower() == "system"), None
            )
            if isinstance(first_system, dict):
                digest = hashlib.sha256(
                    str(first_system.get("content", "")).encode("utf-8", errors="ignore")
                ).hexdigest()
                with self._lock:
    ...
    def _remove_stale_messages(
        self, messages: list[dict[str, Any]], result: OptimizationResult
    ) -> list[dict[str, Any]]:
        ...
        for idx, msg in enumerate(messages):
            ...
            if (
                age >= self.stale_message_age
                and len(content.strip().split()) < self.min_message_length
            ):
                result.messages_removed += 1
                continue
            out.append(msg)
            ...
            result.savings_percent = round(
                max(
                    ...
                    min(
                        100.0,
                        (
                            (result.original_tokens - optimized_tokens)
                            / float(result.original_tokens)
                        )
                        * 100.0,
                    ),
                ),
