...
class SpendRateDetector:
    ...
    @staticmethod
    def _prune_entries(
        entries: list[tuple[float, float]], now: float, window_seconds: int
    ...
    def check(self) -> SpendRateResult:
        now = time.monotonic()
        with self._lock:
            self._events = self._prune_entries(
                self._events, now, max((w.window_seconds for w in self._windows), default=3600)
            )
            for window in self._windows:
            ...
            if len(self._events) >= 4:
                ...
                if long_rate > 0.0 and short_rate > long_rate * self._spike_multiplier:
                    ...
                    return SpendRateResult(
                        ...
                        window_limit=max(
                            0.0, long_rate * self._spike_multiplier * (float(short_window) / 60.0)
                        ),
                        cooldown_until=paused_until,
                    )
