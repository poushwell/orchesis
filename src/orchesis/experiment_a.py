"""Experiment runner for measuring ordering effects on outcomes.
...
class Experiment:
    """Runs reproducible experiments and reports outcome statistics."""

    HYPOTHESIS = "Injection order affects outcome probability"
    KILL_CONDITION = "reject when delta_bar < 0.05"
    ...
    def compute_delta_bar(self) -> dict[str, Any]:
        ...
        status = (
            "REJECTED"
            if delta_bar < 0.05
            else ("CONFIRMED" if delta_bar > 0.10 else "INCONCLUSIVE")
        )
        return {
