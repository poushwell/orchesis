...
class OrchesisVerifier:
    ...
    def run(
        self, policy_path: str = "orchesis.yaml", proxy_url: str = "http://localhost:8090"
    ) -> dict[str, Any]:
        """Run all built-in checklist checks and return pass/fail counts plus per-check results."""
        ...
        return {
            ...
            "checks": results,
            "ready": failed == 0,
            "schema_injection_found": results.get("schema_injection_risk", {}).get("status")
            == "FAIL",
        }
    ...
    def _check_policy_startup_validation(self, policy_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "FAIL",
                "message": f"{policy_path} — policy load failed: {error}",
                "warnings_count": 0,
            }
    ...
    def _check_mcp_config_discovery(self) -> dict[str, Any]:
        ...
        if not paths:
            return {
                "status": "PASS",
                "message": "found 0 MCP config files at: [] — 0 findings (0 critical, 0 high)",
            }
    ...
    def _check_upstream_connectivity(self, policy_path: str) -> dict[str, Any]:
        ...
        if not target:
            return {
                "status": "PASS",
                "message": "no proxy.target in policy — upstream check skipped",
            }
    ...
    def _check_openclaw_compat(self, policy_path: str) -> dict[str, Any]:
        ...
        if not p.exists():
            return {
                "status": "WARN",
                "message": "No policy file - OpenClaw compatibility not checked",
            }
        try:
            cfg = yaml.safe_load(p.read_text(encoding="utf-8")) or {}
        except Exception as error:
        ...
        if isinstance(threat_config, dict):
            ...
            default_action = (
                str(threat_config.get("default_action", "block")).strip().lower() or "block"
            )
    ...
    def format_report(self, result: dict[str, Any]) -> str:
        """Render a human-readable summary from :meth:`run` output."""
        lines = ["\norchesis verify\n" + "-" * 40]
...
class Severity(str, Enum):
    """Outcome level for structured verify checks (ok, warning, critical, info)."""
...
@dataclass
class CheckResult:
    """Structured result from a single programmatic verify check."""
...
@dataclass
class VerifyReport:
    """Collection of :class:`CheckResult` rows from :func:`run_all_checks`."""
...
def check_config_schema_injection(openclaw_config_path: Path | None = None) -> CheckResult:
    """Detect OpenClaw ``includeConfigSchema`` / schema injection misconfiguration (Issue #9828)."""
    candidates = []
...
def check_proxy_connectivity(proxy_host: str = "127.0.0.1", proxy_port: int = 8080) -> CheckResult:
    """Return whether a TCP connection to the Orchesis proxy listen port succeeds."""
    try:
...
def check_openclaw_proxy_routing(openclaw_config_path: Path | None = None) -> CheckResult:
    """Verify OpenClaw JSON config enables routing through the local Orchesis proxy."""
    candidates = []
    if openclaw_config_path is not None:
        candidates.append(openclaw_config_path)
...
def run_all_checks(
    ...
    """Run schema, connectivity, routing, and policy compatibility checks; return a :class:`VerifyReport`."""
    report = VerifyReport()
