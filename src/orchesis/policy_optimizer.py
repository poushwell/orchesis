...
class PolicyOptimizer:
    ...
    def analyze(self, decisions_log: list, current_policy: dict) -> dict:
        ...
        expected_improvements = {
            "cost_reduction_percent": round(
                sum(
                    max(0.0, float(self.estimate_impact(item, decisions_log)["cost_change"]))
            ...
            "performance_delta": round(
                sum(
                    float(self.estimate_impact(item, decisions_log)["performance_change"])
                    for item in suggestions
                ),
                3,
            ),
            "security_delta": round(
                sum(
                    float(self.estimate_impact(item, decisions_log)["security_change"])
                    for item in suggestions
                ),
                3,
            ),
        ...
        risk_assessment = {
            ...
            "requires_review": any(
                item.get("type") in {"rate_limit", "budget"} for item in suggestions
            ),
        }
        return {
    ...
    def estimate_impact(self, suggestion: dict, decisions_log: list) -> dict:
        ...
        if typ == "cache":
            ...
            total = sum(
                self._cost(item) for item in decisions_log if isinstance(decisions_log, list)
            )
