...
class AutoHealer:
    ...
    def get_session_budget(self, session_id: str) -> dict:
        ...
        with self._lock:
            ...
            last = (
                self._session_interventions.get(sid, [])[-1]
                if self._session_interventions.get(sid)
                else None
            )
            return {
    ...
    def _collect_signals(
        self, detection_result: Any, mast_findings: Any, request_data: dict[str, Any]
    ) -> list[str]:
        signals: list[str] = []
    ...
    def _apply_retry_model(
        self, data: dict[str, Any], action: HealingAction, result: HealingResult
    ) -> None:
    ...
    def _apply_rate_limit(self, agent_id: str, action: HealingAction) -> None:
        ...
        self.rate_limiter.set_limit(
            agent_id, max_requests=max_requests, window_seconds=window_seconds
        )
    ...
    def apply(
        self, actions: list[HealingAction], request_data: dict, agent_id: str
        ...
        with self._lock:
            self._session_request_counter[session_id] = (
                int(self._session_request_counter.get(session_id, 0)) + 1
            )
        ...
        for action in actions:
            ...
            if a_type == HealingLevel.L1:
                ...
                self._apply_rate_limit(
                    str(agent_id or "unknown"),
                    HealingAction(
                        "rate_limit",
                        action.reason,
                        {"window_seconds": 60, "max_requests": 10},
                        action.confidence,
                        action.source_detection,
                    ),
                )
        ...
        result.resolution = (
            "healed"
            if (result.messages_modified or result.model_switched or result.context_reset)
            else "mitigated"
        )
        ...
        with self._lock:
            ...
            if len(self._agent_history[str(agent_id or "unknown")]) > 100:
                self._agent_history[str(agent_id or "unknown")] = self._agent_history[
                    str(agent_id or "unknown")
                ][-100:]
        return data, result
    ...
    def heal(
        ...
        if until > time.time():
            blocked = HealingResult(
                actions_taken=[
                    HealingAction(
                        HealingLevel.L6, "circuit open", {"until": until}, 1.0, "circuit_break"
                    )
                ],
        ...
        if self.mode == "monitor":
            ...
            return req, res
        if not actions:
            return req, HealingResult(
                actions_taken=[], original_issue="", resolution="failed", latency_added_ms=0.0
            )
        return self.apply(actions=actions, request_data=req, agent_id=str(agent_id or "unknown"))

    def verify_healing(
        self, agent_id: str, pre_healing_score: float, post_healing_score: float
    ) -> bool:
    ...
    def get_stats(self) -> dict[str, Any]:
        with self._lock:
            ...
            avg_ms = (
                (float(self._stats["total_healing_time_ms"]) / float(attempts))
                if attempts > 0
                else 0.0
            )
            success_rate = (
                (float(self._stats["healed_or_mitigated"]) / float(attempts))
                if attempts > 0
                else 0.0
            )
            return {
