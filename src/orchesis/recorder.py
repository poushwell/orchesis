...
class SessionRecorder:
    ...
    def _list_sessions_from_files(self) -> list[SessionSummary]:
        ...
        for path in self._iter_session_files():
            ...
            models = sorted(
                {item.model for item in records if isinstance(item.model, str) and item.model}
            )
    ...
    def get_session_summary(self, session_id: str) -> SessionSummary:
        ...
        candidate_files = [
            p for p in self._iter_session_files() if self._session_id_from_file(p) == session_id
        ]
        latest_path = (
            max(candidate_files, key=lambda p: p.stat().st_mtime) if candidate_files else Path("")
        )
        total_cost = sum(float(item.cost) for item in records)
        models = sorted(
            {item.model for item in records if isinstance(item.model, str) and item.model}
