"""Quality Control — feedback controller keeping quality score within target range.
...
class QualityController:
    ...
    def get_stats(self) -> dict:
        with self._lock:
            ...
            in_range = sum(
                1 for p in self._psi_history if self.PSI_OPTIMAL_LOW <= p <= self.PSI_OPTIMAL_HIGH
            )
            return {
