...
class ApprovalQueue:
    ...
    def add(
        ...
        with self._lock:
            ...
            if len(self._pending) > self._max_pending:
                oldest = sorted(
                    self._pending.values(), key=lambda item: float(item.get("timestamp", 0.0))
                )[0]
    ...
    def approve(self, request_id: str) -> bool:
        ...
        with self._lock:
            ...
            if len(self._approved) > self._max_pending * 10:
                self._approved = dict(
                    sorted(self._approved.items(), key=lambda kv: kv[1])[-self._max_pending * 5 :]
                )
    ...
    def get_stats(self) -> dict:
        with self._lock:
            avg_wait = (
                (self._total_wait_seconds / float(self._handled)) if self._handled > 0 else 0.0
            )
            return {
    ...
    def evaluate(
        ...
        if self._policy_engine is not None:
            ...
            if eval_result.matched:
                ...
                if action == "approve":
                    if provided_approval_id and self.approval_queue.consume_approved(
        ...
        if self._allowed_legacy:
            if tool in self._allowed_legacy:
                ...
                return ToolDecision(
                    tool_name=tool,
                    action="allow",
                    reason="legacy allowed list",
                    rule_source="legacy_allowed",
                )
            with self._lock:
                self._tool_stats[tool]["block_count"] += 1
