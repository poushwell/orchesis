...
class FleetSelectionModel:
    ...
    def record_interaction(
        self, agent_id: str, cooperative: bool, outcome: float
    ) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            group_cooperators = sum(
                ...
                if self._agents.get(member, {}).get("cooperative", False)
            )
            cooperation_rate = group_cooperators / max(1, len(group_members))

            group_effect = (
                self.cooperation_bonus * cooperation_rate
                if cooperative
                else -self.defection_penalty
            )
    ...
    def get_group_fitness(self, group_id: str) -> dict[str, Any]:
        with self._lock:
            ...
            return {
                ...
                "size": len(members),
                "cooperators": sum(
                    1 for m in members if self._agents.get(m, {}).get("cooperative")
                ),
            }
