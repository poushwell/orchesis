...
class CommunityClient:
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        self._privacy_filter = PrivacyFilter(
            self.privacy_level, min_anomaly_score=self.min_anomaly_score
        )
            ...
            with self._lock:
                with self._connect() as conn:
                    ...
                    count_row = conn.execute(
                        "SELECT COUNT(*) AS c FROM pending_signals WHERE sent_at IS NULL"
                    ).fetchone()
        ...
        try:
            ...
            if 200 <= status < 300:
                ...
                with self._lock:
                    with self._connect() as conn:
                        conn.executemany(
                            "UPDATE pending_signals SET sent_at = ? WHERE id = ?",
    ...
    def start(self) -> None:
        ...
        self._sender_thread = threading.Thread(
            target=self._sender_loop, name="community-sender", daemon=True
        )
        self._receiver_thread = threading.Thread(
            target=self._receiver_loop, name="community-receiver", daemon=True
        )
    ...
    def _init_db(self) -> None:
        try:
            ...
            recovered_path = (
                self._data_dir / f"community-recovered-{int(time.time() * 1000)}.sqlite3"
            )
            try:
    ...
    def _cleanup_old_signals(self) -> None:
        ...
        with self._lock:
            with self._connect() as conn:
                conn.execute(
                    "DELETE FROM pending_signals WHERE sent_at IS NOT NULL AND sent_at < ?",
                    (now - 86400.0,),
                )
                conn.execute(
                    "DELETE FROM pending_signals WHERE sent_at IS NULL AND created_at < ?",
                    (now - 7 * 86400.0,),
                )
                conn.execute(
    ...
    def get_stats(self) -> CommunityStats:
        ...
        with self._lock:
            with self._connect() as conn:
                ...
                cached_row = conn.execute(
                    "SELECT COUNT(*) AS c FROM community_signatures"
                ).fetchone()
        ...
        ratio = (
            (self._signals_sent / float(max(1, received)))
            if received > 0
            else float(self._signals_sent)
        )
