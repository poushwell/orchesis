...
class ForensicsEngine:
    ...
    def detect_incidents(
        ...
        for event in filtered:
            ...
            if event.decision == "DENY":
                ...
                deny_windows[event.agent_id] = [
                    item for item in agent_window if item >= one_minute_ago
                ]
            ...
            if any(
                "lacks capability" in reason or "not in allowed_tools" in reason
            ...
            # 6) unknown agent
            if event.agent_id in {
                "__global__",
                "__unknown__",
                "unknown",
            } or event.agent_id.startswith("unknown"):
                incidents.append(
                    self._new_incident(
                        severity="medium",
                        incident_type="identity_violation",
    ...
    def build_report(
        ...
        if until is not None:
            incidents = [
                item for item in incidents if (_parse_ts(item.timestamp) or end_dt) <= end_dt
            ]
        ...
        if incidents:
            ...
            for item in incidents:
                if item.timeline:
                    first = (
                        _parse_ts(item.timeline[0]["ts"])
                        if isinstance(item.timeline[0], dict)
                        else None
                    )
    ...
    def agent_risk_profile(self, agent_id: str) -> dict[str, Any]:
        ...
        reasons = Counter(reason for item in denied_events for reason in item.reasons).most_common(
            5
        )
        ...
        last_24 = [
            item
            for item in events
            if (_parse_ts(item.timestamp) or now) >= now - timedelta(hours=24)
        ]
        prev_24 = [
            item
            for item in events
            if now - timedelta(hours=48)
            <= (_parse_ts(item.timestamp) or now)
