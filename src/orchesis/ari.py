...
class AgentReadinessIndex:
    ...
    def __init__(
        ...
        self._thresholds = {
            name: {"pass": cfg["pass"], "warn": cfg["warn"]}
    ...
    def evaluate(
        ...
        raw_scores = {
            ...
            "cost_predictability": self._coerce_score(
                components.get("cost_efficiency"), metrics.get("cost_score", 50.0)
            ),
            "task_reliability": self._coerce_score(
                components.get("task_completion"), metrics.get("task_score", 50.0)
            ),
            "loop_safety": self._coerce_score(
                components.get("loop_freedom"), metrics.get("loop_score", 50.0)
            ),
            "latency_profile": self._coerce_score(
                components.get("latency_health"), metrics.get("latency_score", 50.0)
            ),
            "observability": self._coerce_score(
                components.get("context_stability"), metrics.get("obs_score", 50.0)
            ),
        }
        ...
        if ars_result is not None:
            confidence = (
                ars_result.confidence
                if ars_result.confidence in {"high", "medium", "low"}
                else "low"
            )
