            ...
            while True:
                ...
                for pc in pool:
                    if not pc.in_use and not pc.is_idle_expired:
                        ...
                        return pc
                if (
                    len(pool) < self._config.max_connections_per_host
                    and self._total_connections < self._config.max_total_connections
                ):
                ...
                if remaining <= 0:
                    ...
                    if at_host_cap or at_total_cap:
                        self._stats["pool_overflow_count"] = (
                            int(self._stats["pool_overflow_count"]) + 1
                        )
        ...
        with self._lock:
            ...
            return {
                **self._stats,
                ...
                "pools": {
                    f"{host}:{port}": len(pool) for (host, port), pool in self._pools.items()
                },
            }
