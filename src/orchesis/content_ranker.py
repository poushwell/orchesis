...
class ContentRanker:
    ...
    def __init__(self, config: dict | None = None):
        cfg = config or {}
        self.w_contribution = float(cfg.get("w_contribution", 0.35))
    ...
    def compute_uci(self, message: dict, context: list[dict]) -> float:
        ...
        if not isinstance(message, dict):
            return 0.0
        total = max(1e-9, self.w_contribution + self.w_causal + self.w_tig + self.w_zipf)
        try:
        ...
        score = (
            self.w_contribution * self.contribution_score(message, context)
    ...
    def contribution_score(self, message: dict, context: list[dict]) -> float:
    ...
    def compress(self, messages: list[dict], budget_tokens: int) -> dict:
        ...
        scores = [
            self.compute_uci(msg if isinstance(msg, dict) else {}, messages) for msg in messages
        ]
        token_costs = [
            self._estimate_tokens([msg if isinstance(msg, dict) else {}]) for msg in messages
        ]
