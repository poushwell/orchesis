...
def build_tool_registry(policy_path: str | None = None) -> dict[str, dict[str, Any]]:
    ...
    return {
        ...
        "orchesis_check_policy": {
            "description": ("Validate if a tool call would be allowed by current Orchesis policy."),
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        "orchesis_cost_report": {
            "description": ("Get current cost report with totals and tool-level breakdown."),
            "inputSchema": {
                "type": "object",
                "properties": {
...
def _handle_scan_config(params: dict[str, Any]) -> dict[str, Any]:
    ...
    for name, server in servers.items():
        ...
        if (
            not server.get("allowedTools")
            and not server.get("disabledTools")
            and not server.get("tools")
        ):
            score = _append_finding(
                findings,
                score,
    ...
    findings_sorted = sorted(
        findings, key=lambda item: severity_rank.get(str(item.get("severity")), 0), reverse=True
    )
    return {
        "score": score,
        "grade": grade,
