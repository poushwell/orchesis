...
class ThreatFeed:
    ...
    def fetch(self) -> list[dict[str, Any]]:
        ...
        if not isinstance(entries, list):
            return []
        existing = {
            self._signature_key(item) for item in self._signatures if isinstance(item, dict)
        }
    ...
    def apply(self, threat_intel) -> int:  # noqa: ANN001
        ...
        for item in self._signatures:
            ...
            if callable(add_custom_method):
                ...
                applied += 1
                continue
            if hasattr(threat_intel, "_threats") and isinstance(
                getattr(threat_intel, "_threats"), dict
            ):
    ...
    def get_stats(self) -> dict[str, Any]:
        next_update_ts = (
            self._last_updated + max(1, self.update_interval) * 3600 if self._last_updated else 0.0
        )
        return {
            ...
            "next_update": (
                datetime.fromtimestamp(next_update_ts, tz=timezone.utc).isoformat()
                if next_update_ts
                else ""
            ),
    ...
    def get_trending_threats(self, limit: int = 10) -> list[dict]:
        ...
        rows = sorted(
            self._community_feed, key=lambda r: int(r.get("reports", 0) or 0), reverse=True
        )
