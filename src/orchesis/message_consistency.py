"""Discourse coherence metrics for message-consistency metrics."""
...
def compute_entity_continuity(
    messages: list[dict], _preprocessed: list[dict] | None = None
) -> float:
...
def compute_headedness(messages: list[dict], _preprocessed: list[dict] | None = None) -> float:
    ...
    if not prep:
        return 1.0
    structured = sum(
        1 for item in prep if item["has_header"] or item["has_list"] or item["word_count"] > 10
    )
