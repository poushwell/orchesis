...
class AdaptiveDetector:
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        self._actions = {
            key: str(act_cfg.get(key, self._DEFAULT_ACTIONS[key])) for key in self._DEFAULT_ACTIONS
        }
        ...
        block = float(
            sr_cfg.get(
                "block_threshold", sr_cfg.get("escalation_threshold", self._thresholds["high"])
            )
        )

        self._entropy_detector = (
            EntropyDetector(entropy_cfg) if self._detectors_enabled["entropy"] else None
        )
        self._structural_detector = (
            StructuralPatternDetector(structural_cfg)
            if self._detectors_enabled["structural"]
            else None
        )
        self._ngram_profiler = (
            NgramProfiler(ngram_cfg) if self._detectors_enabled["ngram"] else None
        )
        self._session_risk = (
    ...
    def _record_session_signal(
        self, session_id: str, category: str, score_100: float, source: str
    ) -> None:
    ...
    def check(self, agent_id: str, request_data: dict[str, Any]) -> DetectionResult:
        ...
        if self._entropy_detector is not None:
            try:
                ...
                entropy_anom, entropy_score, _ = self._entropy_detector.check(
                    safe_agent, entropy_req
                )
            ...
            try:
                if entropy_score > 0:
                    self._record_session_signal(
                        safe_agent, "entropy_anomaly", entropy_score, "entropy"
                    )
