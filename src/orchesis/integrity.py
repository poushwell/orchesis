...
class IntegrityMonitor:
    ...
    def _iter_files_from_path(self, path: Path) -> list[Path]:
        ...
        if path.is_file():
            if path.name.startswith(".") and path.suffix == "":
                return [path]
            if path.suffix.lower() in _ALLOWED_SUFFIXES or path.name.lower() in {
                "policy.yaml",
                "policy.yml",
            }:
                return [path]
        ...
        for item in path.rglob("*"):
            ...
            if any(part in {".git", "__pycache__", ".pytest_cache"} for part in item.parts):
                continue
            if item.suffix.lower() in _ALLOWED_SUFFIXES or item.name.lower() in {
                "policy.yaml",
                "policy.yml",
            }:
                output.append(item)
    ...
    def init(self, paths: list[str]) -> BaselineReport:
        expanded_paths = [
            Path(item).expanduser() for item in paths if isinstance(item, str) and item.strip()
        ]
            ...
            if new_record["sha256"] != old_sha:
                modified.append(
                    FileChange(
                        ...
                        change_type="hash",
                        old_value=old_sha,
                        new_value=str(new_record["sha256"]),
                    )
                )
                continue
            if new_record["permissions"] != old_perm:
                permission_changed.append(
                    FileChange(
                        ...
                        old_value=old_perm,
                        new_value=str(new_record["permissions"]),
                    )
                )
                continue
            unchanged += 1
    ...
    def update(self, paths: list[str] | None = None) -> BaselineReport:
        ...
        return BaselineReport(
            files_count=len(baseline_files), baseline_path=str(self._baseline_path)
        )
...
def build_integrity_alert_callback(policy: dict[str, Any]) -> Callable[[IntegrityReport], None]:
    ...
    def _callback(report: IntegrityReport) -> None:
        ...
        for item in report.modified:
            severity = (
                "HIGH" if item.path.lower().endswith(("policy.yaml", "policy.yml")) else "MEDIUM"
            )
            messages.append(
                f"[{severity}] Integrity violation: {item.path} modified (hash changed)"
            )
        for item in report.permission_changed:
            ...
            messages.append(
                f"[{severity}] Integrity violation: {item.path} permissions changed {item.old_value}->{item.new_value}"
            )
