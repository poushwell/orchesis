...
class HierarchicalSemanticCache:
    ...
    def get_stats(self) -> dict[str, Any]:
        with self._lock:
            ...
            hit_rate = (
                (float(total_hits) / float(total_lookups) * 100.0) if total_lookups > 0 else 0.0
            )
    ...
    def _evict_expired_tier(tier_dict: OrderedDict[str, CacheEntry], ttl: float) -> int:
        ...
        now = time.time()
        expired = [
            key for key, item in tier_dict.items() if (now - float(item.created_at)) > float(ttl)
        ]
        for key in expired:
    ...
    def lookup(
        ...
        with self._lock:
            ...
            if self.exact_match_only:
                ...
                return CacheLookupResult(hit=False)
            if (
                self._config.cacheable_models
                and model
                and model not in self._config.cacheable_models
            ):
                self._misses += 1
                return CacheLookupResult(hit=False)
