...
class RequestInspector:
    ...
    def inspect(self, request_id: str, decisions_log: list) -> dict:
        ...
        reasons = (
            [str(item) for item in reasons_raw if isinstance(item, str)]
            if isinstance(reasons_raw, list)
            else []
        )
        eval_order_raw = row.get("evaluation_order", [])
        evaluation_order = (
            [str(item) for item in eval_order_raw if isinstance(item, str)]
            if isinstance(eval_order_raw, list)
            else []
        )
        ...
        rules_triggered = (
            [str(item) for item in triggered_raw if isinstance(item, str)]
            if isinstance(triggered_raw, list)
            else []
        )
        run_set = set(evaluation_order)
        blocking_index = (
            max(0, len(evaluation_order) - 1) if decision == "DENY" and evaluation_order else -1
        )
        warn_index = (
            max(0, len(evaluation_order) - 1)
            if decision == "ALLOW" and reasons and evaluation_order
            else -1
        )
