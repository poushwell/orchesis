...
class PaperExporter:
    """Exports research data for academic paper."""
    ...
    CLAIMS = {
        ...
        "fleet_consensus": {"n_star": 16, "exp": 13},
    }
    ...
    def export_experiment_results(self, results_dir: str) -> dict:
        ...
        for exp_id, rows in experiments.items():
            ...
            avg_metric = (
                sum(float(row.get("key_metric", 0.0) or 0.0) for row in completed)
                / float(len(completed))
                if completed
                else 0.0
            )
    ...
    def export_methodology(self) -> str:
        ...
        return (
            "## Methodology\n\n"
            "We evaluate pipeline hypotheses using deterministic offline replay of experiment traces. "
    ...
    def generate_abstract(self) -> str:
        ...
        return (
            "We present a pipeline framework for analyzing language-agent dynamics under constrained processing. "
    ...
    def export_all(self, output_dir: str) -> list[str]:
        ...
        results_path.write_text(
            json.dumps(
                self.export_experiment_results("experiments/results"), ensure_ascii=False, indent=2
            ),
            encoding="utf-8",
        )
