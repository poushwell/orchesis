...
class AntiCollusionDetector:
    ...
    def check_fleet(self) -> CollusionResult:
        ...
        if len(agents) < 2:
            return CollusionResult(
                collusion_detected=False, pairs=[], fleet_risk_score=0.0, recommendation=""
            )
        ...
        logger.warning(
            ...
            extra={
                "component": "anti_collusion",
                "agent_count": len(agents),
                "signal_count": len(all_pairs),
            },
        )
        return CollusionResult(
    ...
    def _check_role_splitting(self, all_actions: list[AgentAction]) -> list[CollusionPair]:
        ...
        try:
            ...
            logger.debug(
                "Skipping role splitting check: %s", exc, extra={"component": "anti_collusion"}
            )
            return []
