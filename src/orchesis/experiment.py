...
class ExperimentManager:
        ...
        **kwargs: Any,
        ...
        with self._lock:
            ...
            exp = Experiment(
                ...
                min_sample_size=int(
                    kwargs.get("min_sample_size", self._config.default_min_sample_size) or 30
                ),
    ...
    def assign_variant(
        ...
        with self._lock:
            active = [e for e in self._experiments.values() if e.status == ExperimentStatus.RUNNING]
            if not active:
                return None
            exp = active[0]
    ...
    def _weighted_choice(
        self, exp: Experiment, session_id: str, agent_id: str
    ) -> Optional[Variant]:
        if not exp.variants:
            return None
    ...
    def get_results(self, experiment_id: str) -> ExperimentResult:
        ...
        if len(exp.variants) >= 2:
            ...
            if n0 >= exp.min_sample_size and n1 >= exp.min_sample_size:
                ...
                if p < (1 - self._config.significance_threshold):
                    ...
                    winner = (
                        v0.name
                        if v0.successes / max(1, n0) > v1.successes / max(1, n1)
                        else v1.name
                    )

        best = max(
            exp.variants, key=lambda v: (v.successes / max(1, v.requests), -v.total_cost_usd)
        )
        if is_sig:
            rec = (
                f"Statistically significant: {winner} is recommended (confidence {confidence:.2%})."
            )
        else:
        ...
        if len(exp.variants) >= 2:
            v0, v1 = exp.variants[0], exp.variants[1]
            diff = (
                (v1.total_cost_usd - v0.total_cost_usd) / max(1, v0.requests + v1.requests) * 1000
            )
    ...
    def get_live_stats(self, experiment_id: str) -> dict:
        ...
        return {
            ...
            "variants": {k: asdict(v) for k, v in result.variants.items()},
