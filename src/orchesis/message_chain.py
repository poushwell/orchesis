...
def _copy_message(msg: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(msg.get("tool_calls"), list):
        copied["tool_calls"] = [
            dict(tc) if isinstance(tc, dict) else tc for tc in msg["tool_calls"]
        ]
    return copied
...
def handle_failed_tool_chain(messages: list[dict]) -> list[dict]:
    ...
    for idx, msg in enumerate(safe):
        ...
        future_result_ids = {
            extract_tool_result_id(item) for item in future if extract_tool_result_id(item)
        }
...
def validate_tool_chain(messages: list[dict]) -> list[dict]:
    ...
    for idx, msg in enumerate(base):
        ...
        found_ids = {
            extract_tool_result_id(base[tidx])
            for tidx in tool_idxs
            if extract_tool_result_id(base[tidx])
        }
...
def truncate_messages_safe(messages: list[dict], max_tokens: int) -> list[dict]:
    ...
    for uidx, unit in enumerate(units):
        idxs = unit["indexes"]
        contains_system = any(
            str(safe[i].get("role", "")).strip().lower() == "system" for i in idxs
        )
        is_latest_group = (
            unit.get("assistant_idx", -1) == latest_assistant and latest_assistant >= 0
        )
