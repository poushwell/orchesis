...
class AgentLifecycleManager:
    ...
    def register(self, agent_id: str, metadata: dict | None = None) -> dict:
        ...
        self._agents[key] = {
            ...
            "since": now,
            "transitions_history": [
                {"from": None, "to": "initializing", "reason": "register", "at": now}
            ],
    ...
    def auto_transition(self, agent_id: str, metrics: dict) -> str | None:
        ...
        if (
            current in {"active", "idle"}
            and error_rate >= 0.2
            and self.transition(key, "degraded", "auto:error_rate")
        ):
            return "degraded"
        if (
            current == "degraded"
            and error_rate <= 0.05
            and self.transition(key, "active", "auto:recovered")
        ):
            return "active"
        if (
            current == "active"
            and request_rate <= 0.0
            and self.transition(key, "idle", "auto:idle")
        ):
            return "idle"
        if (
            current == "idle"
            and request_rate > 0.0
            and self.transition(key, "active", "auto:resume")
        ):
            return "active"
        return None
