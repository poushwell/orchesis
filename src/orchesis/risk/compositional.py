    ...
    def check_compositional_risk(self) -> CompositionRisk:
        ...
        if not sequence:
            return CompositionRisk(
                score=0.0, tool_count=0, unique_tools=0, window_seconds=self.window_seconds
            )
        ...
        for chain in self.chains:
            ...
            for idx in matches:
                ...
                max_severity_score = max(
                    max_severity_score, severity_scores.get(chain.severity, 0.3)
                )
        ...
        if score >= self.score_threshold:
            ...
            recommendation = (
                f"WARN: Detected {len(detected)} tool chain(s) with moderate risk. Monitor closely."
            )
