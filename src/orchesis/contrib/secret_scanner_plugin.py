...
class SecretScannerPlugin:
    ...
    def evaluate(
        ...
        **kwargs: Any,  # noqa: ANN401
        ...
        scanner = (
            SecretScanner(ignore_patterns=ignore) if isinstance(ignore, list) else self._scanner
        )
        reasons: list[str] = []
        ...
        blocking = [
            item for item in findings if _severity_at_least(str(item["severity"]), threshold)
        ]
        if blocking:
            labels = ", ".join(
