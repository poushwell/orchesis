...
class RemoteSkillScanner:
    ...
    def scan_url(self, url: str) -> ScanReport:
        ...
        elif lower_url.endswith("package.json") or (
            '"name"' in stripped and '"version"' in stripped
        ):
    ...
    def scan_npm_package(self, package_name: str) -> ScanReport:
        ...
        latest = (
            payload.get("dist-tags", {}).get("latest")
            if isinstance(payload.get("dist-tags"), dict)
            else None
        )
        versions = payload.get("versions") if isinstance(payload.get("versions"), dict) else {}
        latest_meta = (
            versions.get(latest) if isinstance(latest, str) and isinstance(versions, dict) else None
        )
