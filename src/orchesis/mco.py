...
class MCO:
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        self.initial_exploration_rate = max(
            0.0, min(1.0, float(cfg.get("initial_exploration_rate", 0.3)))
        )
        self.exploration_decay = max(0.8, min(1.0, float(cfg.get("exploration_decay", 0.99))))
        classify_by = cfg.get("classify_by")
        self.classify_by = (
            classify_by
            if isinstance(classify_by, list) and classify_by
            else [
                "estimated_tokens",
                "has_tools",
                "agent_id",
    ...
    def _normalize_cost_latency(self, model_stats: ModelStats) -> tuple[float, float]:
        ...
        norm_cost = (
            0.0 if max_cost <= 0 else min(1.0, max(0.0, float(model_stats.avg_cost_usd) / max_cost))
        )
        norm_latency = (
            0.0
            if max_latency <= 0
            else min(1.0, max(0.0, float(model_stats.avg_latency_ms) / max_latency))
        )
        return norm_cost, norm_latency

    def _apply_objective(
        self, thompson_sample: float, model_stats: ModelStats, objective: str
    ) -> float:
    ...
    def select_model(
        ...
        with self._lock:
            ...
            self._category_selections[category] = (
                int(self._category_selections.get(category, 0)) + 1
            )
    ...
    def compute_quality_score(self, outcome: dict, detection_result: Any = None) -> float:
        ...
        with self._lock:
            if model and model in self._recent_latency_by_model:
                ...
                lat_med = self._median(
                    [x for v in self._recent_latency_by_model.values() for x in v]
                )
        ...
        try:
            ...
            if isinstance(cat_sel, dict):
                self._category_selections = {
                    str(k): int(v) for k, v in cat_sel.items() if isinstance(v, int | float)
                }
        ...
        with self._lock:
            ...
            for model in all_models:
                ...
                output[model] = {
                    ...
                    "avg_latency_ms": round((total_latency / float(total_requests)), 3)
                    if total_requests
                    else 0.0,
                    "avg_cost_usd": round((total_cost / float(total_requests)), 8)
                    if total_requests
                    else 0.0,
