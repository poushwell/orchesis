    ...
    def detect_quorum(self) -> list[dict]:
        ...
        with self._lock:
            ...
            for agents in components:
                ...
                formed_at = self._quorum_contexts.get(quorum_id, {}).get(
                    "formed_at", self._now_iso()
                )
            ...
            self._quorum_contexts = {
                qid: payload for qid, payload in self._quorum_contexts.items() if qid in active_ids
            }
    ...
    def get_shared_context(self, quorum_id: str) -> dict | None:
        ...
        with self._lock:
            ...
            return {
                ...
                "contributors": sorted(list(contributors.keys()))
                if isinstance(contributors, dict)
                else [],
                "context": merged,
            }
