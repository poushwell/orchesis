...
class VibeCodeAuditor:
    ...
    def audit_code(self, code: str, language: str = "python") -> dict[str, Any]:
        ...
        if re.search(r"(API_KEY|SECRET|TOKEN)\s*=\s*['\"]", text, re.IGNORECASE):
            findings.append(
                self._build_finding(
                    "hardcoded_secrets", "critical", "Hardcoded secret-like value found"
                )
            )
        ...
        if re.search(r"subprocess\.(run|Popen)\(.*shell\s*=\s*True", text, re.DOTALL):
            findings.append(
                self._build_finding(
                    "command_injection", "high", "Shell execution with dynamic input"
                )
            )
        if re.search(r"(system:|assistant:|user:)\s*\{?user_input\}?", text, re.IGNORECASE):
            findings.append(
                self._build_finding(
                    "llm_prompt_in_code", "medium", "Prompt assembled from raw user input"
                )
            )
        ...
        if "client.responses.create" in text and not re.search(
            ...
            findings.append(
                self._build_finding(
                    "token_count_ignored", "low", "No token budget handling detected"
                )
            )
        if "client.responses.create" in text and not re.search(
            ...
            findings.append(
                self._build_finding(
                    "hallucination_unchecked", "low", "No hallucination validation detected"
                )
            )
        if re.search(
            r"trust_tier\s*=\s*['\"]?(admin|root)|bypass approval|auto-approve", text, re.IGNORECASE
        ):
            findings.append(
                self._build_finding(
                    "agent_trust_escalation", "high", "Trust escalation pattern detected"
                )
            )
        ...
        return {
            ...
            if not filtered
            else f"{len(filtered)} finding(s) detected",
        }
    ...
    def audit_directory(self, dir_path: str, extensions: list[str] | None = None) -> dict[str, Any]:
        ext_set = (
            {f".{item.lstrip('.').lower()}" for item in extensions}
            if extensions
            else {".py", ".js", ".ts"}
        )
        root = Path(dir_path)
