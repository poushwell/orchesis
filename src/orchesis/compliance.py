...
ORCHESIS_OWASP_MAPPINGS: list[ComplianceMapping] = [
    ...
    ComplianceMapping(
        ...
        "partial",
        "Detects post-injection tool-chain exfiltration patterns.",
    ),
    ComplianceMapping(
        _get_owasp("LLM02"),
        "engine",
        "pii_detector",
        "full",
        "Detects and optionally redacts PII in requests and responses.",
    ),
    ComplianceMapping(
        _get_owasp("LLM02"),
        "engine",
        "secret_scanner",
        "full",
        "Detects API keys, tokens, and secret material.",
    ),
    ComplianceMapping(
        _get_owasp("LLM03"),
        "engine",
        "integrity_monitor",
        "partial",
        "Monitors for unauthorized tool or policy changes.",
    ),
    ComplianceMapping(
        _get_owasp("LLM04"),
        "behavioral",
        "anomaly_detection",
        "detect_only",
        "Detects behavioral drift that may indicate poisoning.",
    ),
    ComplianceMapping(
        _get_owasp("LLM05"),
        "engine",
        "content_filter",
        "partial",
        "Filters dangerous output patterns before agent consumption.",
    ),
    ComplianceMapping(
        _get_owasp("LLM05"),
        "engine",
        "yara_engine",
        "partial",
        "YARA pattern matching for malicious output signatures.",
    ),
    ComplianceMapping(
        _get_owasp("LLM06"),
        "engine",
        "tool_allowlist",
        "full",
        "Policy-based tool allowlist/denylist enforcement.",
    ),
    ComplianceMapping(
        _get_owasp("LLM06"),
        "rate_limiter",
        ...
        "partial",
        "Limits excessive tool invocations.",
    ),
    ComplianceMapping(
        _get_owasp("LLM06"),
        "flow_xray",
        "pattern_detection",
        "partial",
        "Finds excessive agency patterns and dead-end behavior.",
    ),
    ComplianceMapping(
        _get_owasp("LLM07"),
        "engine",
    ...
    ComplianceMapping(
        ...
        "session_recording",
        "detect_only",
        "Enables post-hoc fact-checking and audit.",
    ),
    ComplianceMapping(
        _get_owasp("LLM10"),
        "cost_tracker",
        "budget_enforcement",
        "full",
        "Budget limits and model cascade reduce runaway spend.",
    ),
    ComplianceMapping(
        _get_owasp("LLM10"),
        "circuit_breaker",
    ...
    ComplianceMapping(
        ...
        "loop_detection",
        "full",
        "Detects and blocks infinite agent loops.",
    ),
    ComplianceMapping(
        _get_owasp("LLM10"),
        "rate_limiter",
        "rate_limiting",
        "full",
        "Global and per-tool request throttling.",
    ),
]
...
ORCHESIS_NIST_MAPPINGS: list[ComplianceMapping] = [
    ComplianceMapping(
        ...
        "policy_as_code",
        "full",
        "YAML policy defines controls, budgets, and scanner behavior.",
    ),
    ComplianceMapping(
        _get_nist("GOVERN"),
        "recorder",
        "audit_trail",
        "full",
        "Session recording provides accountability and audit trails.",
    ),
    ComplianceMapping(
        _get_nist("MAP"),
        "flow_xray",
        "topology_analysis",
        "partial",
        "Maps agent flows and highlights risk vectors.",
    ),
    ComplianceMapping(
        _get_nist("MAP"),
        "behavioral",
        "agent_profiling",
        "partial",
        "Agent DNA profiles baseline behavior and drift.",
    ),
    ComplianceMapping(
        _get_nist("MEASURE"),
        "engine",
        "scanning",
        "full",
        "Multiple scanners continuously measure security posture.",
    ),
    ComplianceMapping(
        _get_nist("MEASURE"),
        "cost_tracker",
        "cost_measurement",
        "full",
        "Tracks cost by request/session/tool/model.",
    ),
    ComplianceMapping(
        _get_nist("MANAGE"),
        "circuit_breaker",
        "automated_response",
        "full",
        "Automated mitigation via circuit breaker and guards.",
    ),
    ComplianceMapping(
        _get_nist("MANAGE"),
        "cascade",
        "cost_management",
        "full",
        "Adaptive model cascade manages cost and risk.",
    ),
]
...
FRAMEWORK_CHECKS: dict[str, list[dict[str, str]]] = {
    ...
    "owasp_asi": [
        {
            "id": "ASI-01",
            "requirement": "Agent Authorization & Control",
            "check": "asi_01_authorization_control",
        },
        {"id": "ASI-02", "requirement": "Tool & Function Misuse", "check": "asi_02_tool_misuse"},
        {
            "id": "ASI-03",
            "requirement": "Privilege & Access Escalation",
            "check": "asi_03_privilege_escalation",
        },
        {
            "id": "ASI-04",
            "requirement": "Knowledge & Memory Poisoning",
            "check": "asi_04_memory_poisoning",
        },
        {
            "id": "ASI-05",
            "requirement": "Supply Chain & Dependency Vulnerabilities",
            "check": "asi_05_supply_chain",
        },
        {
            "id": "ASI-06",
            "requirement": "Output Integrity & Handling",
            "check": "asi_06_output_integrity",
        },
        {
            "id": "ASI-07",
            "requirement": "Prompt & Instruction Manipulation",
            "check": "asi_07_prompt_manipulation",
        },
        {
            "id": "ASI-08",
            "requirement": "Goal & Alignment Hijacking",
            "check": "asi_08_goal_alignment",
        },
        {
            "id": "ASI-09",
            "requirement": "Logging, Auditing & Monitoring",
            "check": "asi_09_logging_monitoring",
        },
    ...
    "mitre_atlas": [
        {
            "id": "AML-T0051",
            "requirement": "LLM Prompt Injection",
            "check": "atlas_t0051_prompt_injection",
        },
        {"id": "AML-T0054", "requirement": "LLM Jailbreak", "check": "atlas_t0054_jailbreak"},
        {
            "id": "AML-T0052",
            "requirement": "Verify ML Artifacts",
            "check": "atlas_t0052_verify_artifacts",
        },
        {
            "id": "AML-T0040",
            "requirement": "ML Supply Chain Compromise",
            "check": "atlas_t0040_supply_chain",
        },
        {
            "id": "AML-T0048",
            "requirement": "Exfiltration via ML API",
            "check": "atlas_t0048_exfiltration",
        },
    ...
    "cosai": [
        ...
        {
            "id": "COSAI-SUPPLY-01",
            "requirement": "AI Supply Chain Security",
            "check": "cosai_supply_01",
        },
        {
            "id": "COSAI-RUNTIME-01",
            "requirement": "AI Runtime Protection",
            "check": "cosai_runtime_01",
        },
        {
            "id": "COSAI-MONITOR-01",
            "requirement": "AI Monitoring & Detection",
            "check": "cosai_monitor_01",
        },
    ],
    "csa_maestro": [
    ...
    "nist_ai_100_2": [
        {"id": "NIST-AML-EVASION", "requirement": "Evasion Attacks", "check": "nist_aml_evasion"},
        {
            ...
            "requirement": "Poisoning Attacks",
            "check": "nist_aml_poisoning",
        },
...
class FrameworkCrossReference:
    ...
    def generate_coverage_matrix(self) -> dict[str, Any]:
        all_checks = {ref for refs in self.CROSS_MAP.values() for ref in refs}
        return {
            ...
            "coverage_ratio": (
                len(all_checks) / sum(len(items) for items in FRAMEWORK_CHECKS.values())
            )
            if FRAMEWORK_CHECKS
            else 0.0,
        }
...
class ComplianceEngine:
    ...
    def get_coverage_report(self, framework: Framework | None = None) -> dict[str, Any]:
        ...
        target = framework or (
            self._frameworks[0] if self._frameworks else Framework.OWASP_LLM_TOP_10
        )
    ...
    def get_findings(
        ...
        for finding in reversed(self._findings):
            if severity is not None and finding.severity != severity:
                continue
            if framework is not None and not any(
                framework.value == fw for fw, _ in finding.framework_mappings
            ):
                continue
            if (
                isinstance(item_id, str)
                and item_id
                and not any(item_id == it for _, it in finding.framework_mappings)
            ):
                continue
    ...
    def export_report(self, format: str = "json") -> dict[str, Any] | str:
        ...
        payload = {
            ...
            "coverage": {
                framework.value: self.get_coverage_report(framework)
                for framework in self._frameworks
            },
            "findings": [
                {
    ...
    def export_markdown(self, report: ComplianceReport) -> str:
        ...
        recommendations = [
            check.recommendation for check in report.checks if check.status != "pass"
        ]
        if recommendations:
    ...
    def _check_agents_have_unique_ids(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if not agents:
            return self._result(
                "agents_have_unique_ids",
                "partial",
                "No explicit agent IDs",
                "Define explicit agent identifiers in context_rules",
            )
        ...
        return self._result(
            "agents_have_unique_ids", "pass", f"{len(agents)} unique agents configured", ""
        )
    ...
    def _check_audit_logging_enabled(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        return self._result(
            ...
            "partial",
            "decisions log not found",
            "Enable verify/audit logging in runtime",
        )

    def _check_trust_tiers_defined(self, policy: dict[str, Any]) -> ComplianceCheck:
        tool_access = policy.get("tool_access")
        if (
            isinstance(tool_access, dict)
            and str(tool_access.get("mode", "")).lower() == "allowlist"
        ):
            return self._result(
                "trust_tiers_defined", "pass", "allowlist tool_access mode enabled", ""
            )
        tiers = policy.get("trust_tiers")
        if isinstance(tiers, list) and tiers:
            return self._result(
                "trust_tiers_defined", "pass", f"{len(tiers)} trust tiers defined", ""
            )
        return self._result(
            "trust_tiers_defined",
            "partial",
            "No trust tiers defined",
            "Add trust_tiers section to policy",
        )
    ...
    def _check_rate_limits_defined(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        return self._result(
            "rate_limits_defined", "fail", "No rate_limit rule", "Define rate_limit in policy rules"
        )
    ...
    def _check_budget_limits_defined(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if isinstance(tool_access, dict):
            return self._result(
                "budget_limits_defined", "pass", "tool_access controls configured", ""
            )
        ...
        return self._result(
            "budget_limits_defined",
            "partial",
            "No budget_limit rule",
            "Define budget_limit for human oversight",
        )
    ...
    def _check_denied_operations_defined(self, policy: dict[str, Any]) -> ComplianceCheck:
        for rule in self._rules(policy):
            ...
            if isinstance(denied, list) and denied:
                return self._result(
                    "denied_operations_defined", "pass", "denied SQL operations configured", ""
                )
        return self._result(
            "denied_operations_defined",
            "fail",
            "No denied SQL operations",
            "Add sql_restriction.denied_operations",
        )
    ...
    def _check_write_restrictions_exist(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if isinstance(tool_access, dict):
            return self._result(
                "write_restrictions_exist", "pass", "tool_access controls configured", ""
            )
        for rule in self._rules(policy):
            ...
            if isinstance(denied, list) and denied:
                return self._result(
                    "write_restrictions_exist", "pass", "Denied paths configured", ""
                )
        return self._result(
            "write_restrictions_exist",
            "partial",
            "No denied paths configured",
            "Add file_access denied_paths",
        )
    ...
    def _check_pii_detection_enabled(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        return self._result(
            ...
            "partial",
            "PII detector plugin not enabled",
            "Enable pii_detector plugin in policy.yaml",
        )
    ...
    def _check_secret_scanning_enabled(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        return self._result(
            ...
            "partial",
            "secret_scanner not enabled",
            "Enable secret_scanner plugin",
        )
    ...
    def _check_incident_detection_enabled(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if policy.get("incident_detection") is True:
            return self._result(
                "incident_detection_enabled", "pass", "incident_detection flag enabled", ""
            )
        return self._result(
            "incident_detection_enabled",
            "partial",
            "No incident signal configured",
            "Enable incident detection and incident log",
        )
    ...
    def _check_risk_profiles_available(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if Path(self._incidents_path).exists():
            return self._result(
                "risk_profiles_available",
                "partial",
                "incidents available but risk profile flag absent",
                "Enable risk_profiles in policy",
            )
        return self._result(
            "risk_profiles_available",
            "partial",
            "No risk profile configuration",
            "Enable risk_profiles",
        )
    ...
    def _check_alerts_configured(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if isinstance(alerts, dict):
            ...
            if isinstance(recipients, list) and recipients:
                return self._result(
                    "alerts_configured", "pass", f"{len(recipients)} alert recipients", ""
                )
        return self._result(
            "alerts_configured",
            "partial",
    ...
    def _check_force_sync_available(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        return self._result(
            "force_sync_available",
            "partial",
            "force_sync not configured",
            "Enable sync.force_sync for emergency overrides",
        )
    ...
    def _check_agent_authentication_configured(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if isinstance(auth, dict) and auth.get("required") is True:
            return self._result(
                "agent_authentication_configured", "pass", "authentication.required=true", ""
            )
        ...
        return self._result(
            "agent_authentication_configured",
            "partial",
            "No explicit authentication config",
            "Configure authentication/identity controls",
        )

    def _check_anomaly_detection_enabled(self, policy: dict[str, Any]) -> ComplianceCheck:
        if policy.get("anomaly_detection") is True:
            return self._result(
                "anomaly_detection_enabled", "pass", "anomaly_detection enabled", ""
            )
        return self._result(
            "anomaly_detection_enabled",
            "partial",
            "anomaly_detection not configured",
            "Enable anomaly_detection",
        )
    ...
    def _check_integrity_monitoring(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if age.days <= 7:
            return self._result(
                "integrity_monitoring", "pass", f"Baseline fresh ({age.days}d old)", ""
            )
        return self._result(
            "integrity_monitoring",
            "partial",
    ...
    def _check_decision_explanations_available(self, policy: dict[str, Any]) -> ComplianceCheck:
        if policy.get("decision_explanations") is True:
            return self._result(
                "decision_explanations_available", "pass", "decision_explanations enabled", ""
            )
        # Orchesis core always returns reasons, so this is partially satisfied by design.
        return self._result(
            "decision_explanations_available",
            "partial",
            "Reason strings available by default",
            "Enable decision_explanations metadata",
        )
    ...
    def _check_adversarial_testing_exists(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        return self._result(
            ...
            "partial",
            "No THREAT_MODEL.md found",
            "Add adversarial testing evidence",
        )
    ...
    def _check_metrics_available(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if policy.get("metrics") is True or Path(".orchesis/metrics.json").exists():
            return self._result(
                "metrics_available",
                "pass",
                f"Metrics configuration available; integrity={integrity}",
                "",
            )
        return self._result(
            "metrics_available",
    ...
    def _check_asi_01_authorization_control(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if has_default_tier and has_tool_access and restrictive:
            return self._result(
                "asi_01_authorization_control",
                "pass",
                "trust tiers + tool_access + restrictive default tier",
                "",
            )
        if has_default_tier and has_tool_access:
            return self._result(
                "asi_01_authorization_control",
                "partial",
                "Authorization controls exist but default tier is broad",
                "Set default_trust_tier to intern/assistant",
            )
        return self._result(
            "asi_01_authorization_control",
            "fail",
            "Missing trust tier/tool_access controls",
            "Define default_trust_tier and tool_access",
        )
    ...
    def _check_asi_02_tool_misuse(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if not isinstance(tool_access, dict):
            return self._result(
                "asi_02_tool_misuse",
                "fail",
                "tool_access not configured",
                "Configure tool_access allowlist",
            )
        ...
        has_sandbox_exec = (
            ...
            if isinstance(policy.get("session_policies"), dict)
            else False
        )
        if mode == "allowlist" and (dangerous_denied or has_sandbox_exec):
            return self._result(
                "asi_02_tool_misuse", "pass", "allowlist with dangerous tool controls", ""
            )
        if mode in {"allowlist", "denylist"}:
            return self._result(
                "asi_02_tool_misuse",
                "partial",
                f"{mode} configured with limited misuse controls",
                "Deny dangerous tools and configure sandbox.execution",
            )
        return self._result(
            "asi_02_tool_misuse",
            "fail",
            "tool_access mode missing",
            "Use tool_access mode allowlist",
        )
    ...
    def _check_asi_03_privilege_escalation(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        has_channel = isinstance(policy.get("channel_policies"), dict) and bool(
            policy.get("channel_policies")
        )
        if has_sessions and has_channel:
            return self._result(
                "asi_03_privilege_escalation",
                "pass",
                "session_policies + channel_policies configured",
                "",
            )
        if has_sessions:
            return self._result(
                "asi_03_privilege_escalation",
                "partial",
                "session_policies configured but channel_policies missing",
                "Add channel_policies",
            )
        return self._result(
            "asi_03_privilege_escalation",
            "fail",
            "session_policies missing",
            "Define session_policies with restrictive trust tiers",
        )

    def _check_asi_04_memory_poisoning(self, policy: dict[str, Any]) -> ComplianceCheck:
        scan_cfg = policy.get("scanner")
        has_hidden_scan = isinstance(scan_cfg, dict) and bool(
            scan_cfg.get("detect_hidden_instructions")
        )
        has_ioc = isinstance(policy.get("ioc"), dict) and bool(policy.get("ioc", {}).get("enabled"))
        response_scanning = isinstance(policy.get("proxy"), dict) and bool(
            policy.get("proxy", {}).get("scan_responses")
        )
        if has_hidden_scan and has_ioc and response_scanning:
            return self._result(
                "asi_04_memory_poisoning",
                "pass",
                "hidden instruction + ioc + response scan enabled",
                "",
            )
        if has_ioc or response_scanning:
            return self._result(
                "asi_04_memory_poisoning",
                "partial",
                "partial poisoning defenses configured",
                "Enable scanner hidden-instruction checks and ioc scanning",
            )
        return self._result(
            "asi_04_memory_poisoning",
            "fail",
            "No poisoning controls configured",
            "Enable scanner/ioc/proxy response scanning",
        )

    def _check_asi_05_supply_chain(self, policy: dict[str, Any]) -> ComplianceCheck:
        scanner_cfg = policy.get("scanner")
        has_skill_scan = isinstance(scanner_cfg, dict) and bool(
            scanner_cfg.get("scan_skills_before_install")
        )
        has_remote_scan = isinstance(scanner_cfg, dict) and bool(scanner_cfg.get("remote_scan"))
        has_ioc_supply = isinstance(policy.get("ioc"), dict) and bool(
            policy.get("ioc", {}).get("supply_chain_patterns")
        )
        if has_skill_scan and has_remote_scan and has_ioc_supply:
            return self._result(
                "asi_05_supply_chain",
                "pass",
                "local+remote skill scanning and supply-chain IoC configured",
                "",
            )
        if has_skill_scan or has_remote_scan or has_ioc_supply:
            return self._result(
                "asi_05_supply_chain",
                "partial",
                "Partial supply-chain controls configured",
                "Enable skill pre-install scan, remote scan, and supply-chain IoC patterns",
            )
        return self._result(
            "asi_05_supply_chain",
            "fail",
            "No supply-chain controls configured",
            "Configure scanner and IoC supply-chain checks",
        )
    ...
    def _check_asi_06_output_integrity(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        pii = (
            ...
            and isinstance(proxy.get("pii_scanning"), dict)
            and bool(proxy.get("pii_scanning", {}).get("enabled"))
        )
        sec = (
            isinstance(proxy, dict)
            and isinstance(proxy.get("secret_scanning"), dict)
            and bool(proxy.get("secret_scanning", {}).get("enabled"))
        )
        red = (
            isinstance(proxy, dict)
            and isinstance(proxy.get("response_redaction"), dict)
            and bool(proxy.get("response_redaction", {}).get("enabled"))
        )
        if pii and sec and red:
            return self._result(
                "asi_06_output_integrity",
                "pass",
                "PII + secret scan and response redaction configured",
                "",
            )
        if pii or sec or red:
            return self._result(
                "asi_06_output_integrity",
                "partial",
                "Only subset of output controls configured",
                "Enable proxy pii_scanning, secret_scanning and response_redaction",
            )
        return self._result(
            "asi_06_output_integrity",
            "fail",
            "No output integrity controls configured",
            "Configure proxy output scanning/redaction",
        )

    def _check_asi_07_prompt_manipulation(self, policy: dict[str, Any]) -> ComplianceCheck:
        has_ioc_injection = isinstance(policy.get("ioc"), dict) and bool(
            policy.get("ioc", {}).get("prompt_injection_patterns")
        )
        scanner_cfg = policy.get("scanner")
        has_unicode = isinstance(scanner_cfg, dict) and bool(
            scanner_cfg.get("detect_unicode_tricks")
        )
        has_behavioral_sandbox = (
            any(
                isinstance(cfg, dict)
                and isinstance(cfg.get("sandbox"), dict)
        ...
        if has_ioc_injection and has_unicode and has_behavioral_sandbox:
            return self._result(
                "asi_07_prompt_manipulation",
                "pass",
                "injection+unicode+sandbox controls configured",
                "",
            )
        if has_ioc_injection or has_unicode or has_behavioral_sandbox:
            return self._result(
                "asi_07_prompt_manipulation",
                "partial",
                "Partial prompt-manipulation controls configured",
                "Enable IoC injection patterns, unicode checks, sandbox behavior controls",
            )
        return self._result(
            "asi_07_prompt_manipulation",
            "fail",
            "No prompt-manipulation controls configured",
            "Configure scanner/ioc/sandbox controls",
        )
    ...
    def _check_asi_08_goal_alignment(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if has_budget and has_rate and has_token:
            return self._result(
                "asi_08_goal_alignment", "pass", "budget+rate+token limits configured", ""
            )
        if has_budget and has_rate:
            return self._result(
                "asi_08_goal_alignment",
                "partial",
                "budget/rate configured but token limits missing",
                "Add token_limits",
            )
        return self._result(
            "asi_08_goal_alignment",
            "fail",
            "Missing budget/rate controls",
            "Configure budget_limit and rate_limit",
        )
    ...
    def _check_asi_09_logging_monitoring(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        has_redaction = (
            isinstance(policy.get("logging"), dict)
            and isinstance(policy.get("logging", {}).get("redaction"), dict)
            and bool(policy.get("logging", {}).get("redaction", {}).get("enabled"))
        )
        has_alerts = isinstance(policy.get("alerts"), dict) and bool(policy.get("alerts"))
        if has_logging and has_redaction and has_alerts:
            return self._result(
                "asi_09_logging_monitoring", "pass", "logging+redaction+alerts configured", ""
            )
        if has_logging or has_alerts:
            return self._result(
                "asi_09_logging_monitoring",
                "partial",
                "partial monitoring controls configured",
                "Enable logging.redaction and alerts integrations",
            )
        return self._result(
            "asi_09_logging_monitoring",
            "fail",
            "No monitoring controls configured",
            "Configure logging and alerts",
        )

    def _check_asi_10_multi_agent(self, policy: dict[str, Any]) -> ComplianceCheck:
        agents = policy.get("agents")
        has_agent_tiers = isinstance(agents, list) and any(
            isinstance(item, dict) and isinstance(item.get("trust_tier"), str) for item in agents
        )
        has_sync = isinstance(policy.get("sync"), dict)
        has_channels = isinstance(policy.get("channel_policies"), dict) and bool(
            policy.get("channel_policies")
        )
        if has_agent_tiers and has_sync and has_channels:
            return self._result(
                "asi_10_multi_agent", "pass", "agent tiers + sync + channels configured", ""
            )
        if has_agent_tiers:
            return self._result(
                "asi_10_multi_agent",
                "partial",
                "Agent tiers configured, distributed controls partial",
                "Configure sync and channel_policies",
            )
        return self._result(
            "asi_10_multi_agent",
            "fail",
            "No per-agent trust tier controls",
            "Define agents with trust_tier",
        )

    # MITRE ATLAS
    def _check_atlas_t0051_prompt_injection(self, policy: dict[str, Any]) -> ComplianceCheck:
        has_ioc = isinstance(policy.get("ioc"), dict) and bool(
            policy.get("ioc", {}).get("prompt_injection_patterns")
        )
        has_skill_scan = isinstance(policy.get("scanner"), dict) and bool(
            policy.get("scanner", {}).get("detect_hidden_instructions")
        )
        has_allowlist = (
            isinstance(policy.get("tool_access"), dict)
            and str(policy.get("tool_access", {}).get("mode", "")).lower() == "allowlist"
        )
        if has_ioc and has_skill_scan and has_allowlist:
            return self._result(
                "atlas_t0051_prompt_injection",
                "pass",
                "ioc + skill scanner + allowlist configured",
                "",
            )
        if has_ioc or has_skill_scan or has_allowlist:
            return self._result(
                "atlas_t0051_prompt_injection",
                "partial",
                "Partial prompt injection controls configured",
                "Enable IoC prompt injection patterns, hidden instruction scanner, and allowlist",
            )
        return self._result(
            "atlas_t0051_prompt_injection",
            "fail",
            "No prompt injection controls configured",
            "Configure IoC/scanner/tool_access",
        )

    def _check_atlas_t0054_jailbreak(self, policy: dict[str, Any]) -> ComplianceCheck:
        has_budget = any(self._rule_type(rule) == "budget_limit" for rule in self._rules(policy))
        has_rate = any(self._rule_type(rule) == "rate_limit" for rule in self._rules(policy))
        has_forensics = isinstance(policy.get("incident_detection"), bool) and bool(
            policy.get("incident_detection")
        )
        if has_budget and has_rate and has_forensics:
            return self._result(
                "atlas_t0054_jailbreak", "pass", "budget/rate/forensics controls configured", ""
            )
        if has_budget or has_rate:
            return self._result(
                "atlas_t0054_jailbreak",
                "partial",
                "Only rate/budget controls configured",
                "Enable incident_detection/forensics",
            )
        return self._result(
            "atlas_t0054_jailbreak",
            "fail",
            "No jailbreak controls configured",
            "Configure budget_limit and rate_limit",
        )
    ...
    def _check_atlas_t0052_verify_artifacts(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if has_skill_scan and has_remote and has_ioc_supply:
            return self._result(
                "atlas_t0052_verify_artifacts",
                "pass",
                "artifact verification checks configured",
                "",
            )
        if has_skill_scan or has_remote or has_ioc_supply:
            return self._result(
                "atlas_t0052_verify_artifacts",
                "partial",
                "Partial artifact verification controls configured",
                "Enable scanner.remote_scan and supply-chain IoC patterns",
            )
        return self._result(
            "atlas_t0052_verify_artifacts",
            "fail",
            "No artifact verification controls configured",
            "Configure pre-install and remote scanning",
        )

    def _check_atlas_t0040_supply_chain(self, policy: dict[str, Any]) -> ComplianceCheck:
        has_secret = (
            isinstance(policy.get("proxy"), dict)
            and isinstance(policy.get("proxy", {}).get("secret_scanning"), dict)
            and bool(policy.get("proxy", {}).get("secret_scanning", {}).get("enabled"))
        )
        has_ioc = isinstance(policy.get("ioc"), dict) and bool(policy.get("ioc", {}).get("enabled"))
        has_perm = isinstance(policy.get("network_scanner"), dict) and bool(
            policy.get("network_scanner", {}).get("check_permissions")
        )
        if has_secret and has_ioc and has_perm:
            return self._result(
                "atlas_t0040_supply_chain", "pass", "secret/ioc/permission controls configured", ""
            )
        if has_secret or has_ioc:
            return self._result(
                "atlas_t0040_supply_chain",
                "partial",
                "Partial supply-chain controls configured",
                "Enable network scanner permission checks",
            )
        return self._result(
            "atlas_t0040_supply_chain",
            "fail",
            "No supply-chain compromise controls configured",
            "Enable secret scanning and IoC controls",
        )
    ...
    def _check_atlas_t0048_exfiltration(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        denied_domains = (
            any(
                ...
                and isinstance(
                    cfg.get("sandbox", {}).get("network", {}).get("denied_domains"), list
                )
        ...
        if has_secret and has_pii and denied_domains:
            return self._result(
                "atlas_t0048_exfiltration",
                "pass",
                "secret+pii scanning and denied domains configured",
                "",
            )
        if has_secret or has_pii or denied_domains:
            return self._result(
                "atlas_t0048_exfiltration",
                "partial",
                "Partial exfiltration controls configured",
                "Enable secret/PII scanning and denied_domains",
            )
        return self._result(
            "atlas_t0048_exfiltration",
            "fail",
            "No exfiltration controls configured",
            "Configure scanning and network denied_domains",
        )

    def _check_atlas_t0047_evasion(self, policy: dict[str, Any]) -> ComplianceCheck:
        has_forensics = bool(policy.get("incident_detection")) or bool(
            policy.get("anomaly_detection")
        )
        ...
        if has_forensics and has_multi_rule:
            return self._result(
                "atlas_t0047_evasion",
                "pass",
                "forensics/anomaly + multi-rule policy configured",
                "",
            )
        if has_multi_rule:
            return self._result(
                "atlas_t0047_evasion",
                "partial",
                "Multi-rule policy present but anomaly/forensics limited",
                "Enable incident_detection/anomaly_detection",
            )
        return self._result(
            "atlas_t0047_evasion",
            "fail",
            "Single-rule/no-detection evasion posture",
            "Add multiple rules and anomaly detection",
        )

    # CoSAI
    def _check_cosai_gov_01(self, policy: dict[str, Any]) -> ComplianceCheck:
        has_policy = isinstance(policy, dict) and bool(policy)
        has_version = isinstance(policy.get("version"), str) or isinstance(
        ...
        if has_policy and has_version and has_compliance:
            return self._result(
                "cosai_gov_01", "pass", "policy-as-code + versioning + compliance artifacts", ""
            )
        if has_policy and has_version:
            return self._result(
                "cosai_gov_01",
                "partial",
                "policy/versioning configured but compliance metadata minimal",
                "Add compliance reporting metadata",
            )
        return self._result(
            "cosai_gov_01",
            "fail",
            "Missing governance metadata",
            "Define policy versioning and compliance metadata",
        )
    ...
    def _check_cosai_risk_01(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if has_risk or has_gap:
            return self._result(
                "cosai_risk_01",
                "partial",
                "Partial risk assessment controls configured",
                "Enable network_scanner and compliance gap config",
            )
        return self._result(
            "cosai_risk_01",
            "fail",
            "No explicit AI risk assessment controls configured",
            "Enable risk_profiles and compliance gap analysis",
        )

    def _check_cosai_supply_01(self, policy: dict[str, Any]) -> ComplianceCheck:
        scanner_cfg = policy.get("scanner")
        has_skill = isinstance(scanner_cfg, dict) and bool(
            scanner_cfg.get("scan_skills_before_install")
        )
        has_ioc = isinstance(policy.get("ioc"), dict) and bool(policy.get("ioc", {}).get("enabled"))
        has_remote = isinstance(scanner_cfg, dict) and bool(scanner_cfg.get("remote_scan"))
        if has_skill and has_ioc and has_remote:
            return self._result(
                "cosai_supply_01", "pass", "supply-chain scanner and IoC controls configured", ""
            )
        if has_skill or has_ioc:
            return self._result(
                ...
                "partial",
                "Partial supply-chain controls configured",
                "Enable scanner.remote_scan and ioc.enabled",
            )
        return self._result(
            "cosai_supply_01",
            "fail",
            "No AI supply-chain security controls configured",
            "Configure scanner and ioc controls",
        )
    ...
    def _check_cosai_runtime_01(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if has_runtime and has_tool and has_limits:
            return self._result(
                "cosai_runtime_01", "pass", "runtime/tool access/limits configured", ""
            )
        if has_runtime and has_tool:
            return self._result(
                "cosai_runtime_01",
                "partial",
                "runtime enforcement configured with partial limits",
                "Add token_limits and complete rate/budget controls",
            )
        return self._result(
            "cosai_runtime_01",
            "fail",
            "Runtime protection controls incomplete",
            "Configure rules and tool_access",
        )
    ...
    def _check_cosai_monitor_01(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if has_logging and has_incident and has_alerts:
            return self._result(
                "cosai_monitor_01", "pass", "monitoring/detection/alerts configured", ""
            )
        if has_logging or has_incident:
            return self._result(
                ...
                "partial",
                "Partial monitoring controls configured",
                "Enable alerts integrations",
            )
        return self._result(
            "cosai_monitor_01",
            "fail",
            "No monitoring controls configured",
            "Enable logging + incident_detection + alerts",
        )
    ...
    def _check_maestro_l1(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if has_token or has_budget:
            return self._result(
                "maestro_l1",
                "partial",
                "Only one of token/budget controls configured",
                "Configure token_limits and budget_limit",
            )
        return self._result(
            "maestro_l1",
            "fail",
            "No foundation model resource controls",
            "Add token_limits and budget_limit",
        )
    ...
    def _check_maestro_l2(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if has_pii and has_secret and has_redaction:
            return self._result(
                "maestro_l2", "pass", "PII+secret scans and audit redaction configured", ""
            )
        if has_pii or has_secret or has_redaction:
            return self._result(
                "maestro_l2",
                "partial",
                "Partial data operations controls configured",
                "Enable proxy scanning + logging.redaction",
            )
        return self._result(
            "maestro_l2",
            "fail",
            "No data operations controls configured",
            "Enable PII/secret scanning and redaction",
        )
    ...
    def _check_maestro_l3(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if has_policy_engine and has_agent_tiers and has_plugins:
            return self._result(
                "maestro_l3", "pass", "policy engine + agent tiers + plugin model configured", ""
            )
        if has_policy_engine and has_agent_tiers:
            return self._result(
                "maestro_l3",
                "partial",
                "core agent framework controls configured",
                "Add plugin configuration metadata",
            )
        return self._result(
            "maestro_l3",
            "fail",
            "Agent framework controls incomplete",
            "Configure rules and per-agent tiers",
        )

    def _check_maestro_l4(self, policy: dict[str, Any]) -> ComplianceCheck:
        has_tool_access = isinstance(policy.get("tool_access"), dict)
        has_mcp_scan = isinstance(policy.get("scanner"), dict) and bool(
            policy.get("scanner", {}).get("scan_mcp")
        )
        ...
        if has_tool_access and has_mcp_scan and has_sandbox:
            return self._result(
                "maestro_l4", "pass", "tool access + MCP scan + sandbox configured", ""
            )
        if has_tool_access and has_sandbox:
            return self._result(
                "maestro_l4",
                "partial",
                "tool access and sandbox configured",
                "Enable scanner.scan_mcp",
            )
        return self._result(
            "maestro_l4",
            "fail",
            "Tool/integration security controls incomplete",
            "Configure tool_access, MCP scan, and sandbox",
        )
    ...
    def _check_maestro_l5(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if has_agent_tiers and has_channel:
            return self._result(
                "maestro_l5",
                "partial",
                "channel + multi-agent configured but sync missing",
                "Configure sync for distributed orchestration",
            )
        return self._result(
            "maestro_l5",
            "fail",
            "Orchestration controls incomplete",
            "Configure agents, sync, and channel_policies",
        )
    ...
    def _check_maestro_l6(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if has_exposure and has_perms and has_gate:
            return self._result(
                "maestro_l6", "pass", "deployment scanner + permissions + gate configured", ""
            )
        if has_exposure or has_perms:
            return self._result(
                "maestro_l6",
                "partial",
                "partial deployment controls configured",
                "Enable CI/CD security gate metadata",
            )
        return self._result(
            "maestro_l6",
            "fail",
            "Deployment controls missing",
            "Enable network scanner and gate metadata",
        )

    def _check_maestro_l7(self, policy: dict[str, Any]) -> ComplianceCheck:
        has_marketplace = isinstance(policy.get("marketplace"), dict) and bool(
            policy.get("marketplace")
        )
        has_remote = isinstance(policy.get("scanner"), dict) and bool(
            policy.get("scanner", {}).get("remote_scan")
        )
        has_ioc = isinstance(policy.get("ioc"), dict) and bool(policy.get("ioc", {}).get("enabled"))
        if has_marketplace and has_remote and has_ioc:
            return self._result(
                "maestro_l7", "pass", "ecosystem marketplace + remote scanner + ioc configured", ""
            )
        if has_remote or has_ioc:
            return self._result(
                "maestro_l7",
                "partial",
                "partial ecosystem controls configured",
                "Configure marketplace metadata and IoC database",
            )
        return self._result(
            "maestro_l7",
            "fail",
            "Ecosystem controls missing",
            "Enable marketplace, remote scanner, and ioc",
        )
    ...
    def _check_nist_aml_evasion(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if has_forensics and has_multi_rule:
            return self._result(
                "nist_aml_evasion", "pass", "forensics/anomaly + multi-rule policy present", ""
            )
        if has_multi_rule:
            return self._result(
                "nist_aml_evasion",
                "partial",
                "multi-rule policy present but anomaly detection limited",
                "Enable incident_detection/anomaly_detection",
            )
        return self._result(
            "nist_aml_evasion",
            "fail",
            "evasion controls not configured",
            "Add forensics and anomaly detection controls",
        )

    def _check_nist_aml_poisoning(self, policy: dict[str, Any]) -> ComplianceCheck:
        has_skill = isinstance(policy.get("scanner"), dict) and bool(
            policy.get("scanner", {}).get("scan_skills_before_install")
        )
        has_ioc_supply = isinstance(policy.get("ioc"), dict) and bool(
            policy.get("ioc", {}).get("supply_chain_patterns")
        )
        ...
        if has_skill and has_ioc_supply and has_integrity:
            return self._result(
                "nist_aml_poisoning",
                "pass",
                "skill scan + supply chain IoC + integrity checks configured",
                "",
            )
        if has_skill and has_ioc_supply:
            return self._result(
                "nist_aml_poisoning",
                "partial",
                "poisoning controls configured without integrity_checks metadata",
                "Enable integrity_checks metadata",
            )
        return self._result(
            "nist_aml_poisoning",
            "fail",
            "poisoning controls missing",
            "Configure scanner and supply-chain IoC",
        )
    ...
    def _check_nist_aml_privacy(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        if has_pii or has_secret or has_redaction:
            return self._result(
                "nist_aml_privacy",
                "partial",
                "partial privacy controls configured",
                "Enable PII/secret scanning and redaction",
            )
        return self._result(
            "nist_aml_privacy",
            "fail",
            "privacy controls not configured",
            "Configure proxy scanning and redaction",
        )
    ...
    def _check_nist_aml_misuse(self, policy: dict[str, Any]) -> ComplianceCheck:
        ...
        has_budget = any(self._rule_type(rule) == "budget_limit" for rule in self._rules(policy))
        has_rate = any(self._rule_type(rule) == "rate_limit" for rule in self._rules(policy))
        has_scope = isinstance(policy.get("session_policies"), dict) or isinstance(
            policy.get("channel_policies"), dict
        )
        ...
        if has_tool and (has_budget or has_rate):
            return self._result(
                "nist_aml_misuse",
                "partial",
                "core misuse controls configured",
                "Add session/channel scoping policies",
            )
        return self._result(
            "nist_aml_misuse",
            "fail",
            "misuse controls missing",
            "Configure tool_access, budget/rate, and scope policies",
        )
    ...
    def _result(
        self, check_name: str, status: str, evidence: str, recommendation: str
    ) -> ComplianceCheck:
