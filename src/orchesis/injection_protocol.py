...
class ContextInjectionProtocol:
    ...
    Pipeline component — answers: when to inject, what to inject, how much.
    ...
    INJECTION_STRATEGIES = {
        ...
        "adaptive": "Use recursive state to decide",
    }
    ...
    def inject(self, messages: list[dict], injection: list[dict]) -> dict:
        ...
        extra = (
            [dict(item) for item in injection if isinstance(item, dict)]
    ...
    def get_injection_stats(self) -> dict:
        ...
        total = len(rows)
        avg_tokens = (
            (sum(int(item.get("tokens_injected", 0) or 0) for item in rows) / float(total))
            if total > 0
            else 0.0
        )
        return {
