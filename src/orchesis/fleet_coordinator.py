...
class FleetCoordinator:
    ...
    def _matches_capabilities(self, agent: dict[str, Any], required: list[str]) -> bool:
        if not required:
            return True
        caps = {
            str(item).strip().lower() for item in agent.get("capabilities", []) if str(item).strip()
        }
    ...
    def assign_task(self, task: dict) -> str:
        ...
        required = (
            [str(item).strip() for item in required_raw] if isinstance(required_raw, list) else []
        )
        candidates = [
            agent
            for agent in self._agents.values()
            if str(agent.get("status", "idle")) != "degraded"
            and self._matches_capabilities(agent, required)
        ]
        if not candidates:
            return ""
        candidates.sort(
    ...
    def get_load_distribution(self) -> dict:
        ...
        distribution = {
            agent_id: int(row.get("tasks", 0) or 0) for agent_id, row in self._agents.items()
        }
