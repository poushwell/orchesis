...
class SessionReplayer:
    ...
    def _send_replay(
        self, body: dict[str, Any], path: str = "/v1/chat/completions"
    ...
    def replay(
        ...
        sliced = (
            session[: int(config.max_requests)]
        ...
        for item in sliced:
            ...
            if isinstance(policy, dict):
                ...
                if not decision.allowed:
                    policy_blocked = True
                    policy_block_reason = (
                        decision.reasons[0] if decision.reasons else "blocked_by_policy"
                    )

            if config.dry_run:
                replay_status = 429 if policy_blocked else item.status_code
            else:
                replay_status, replay_resp, replay_error, replay_latency_ms = self._send_replay(
                    request_payload
                )
                if isinstance(replay_resp, dict):
                    ...
                    if isinstance(usage, dict):
                        ...
                        replay_cost = self._estimate_model_cost(
                            replay_model, prompt_tokens, completion_tokens
                        )
        ...
        savings_pct = (
            ((original_cost - replay_cost_total) / original_cost * 100.0)
            if original_cost > 0
            else 0.0
        )
        ...
        summary = ReplayReportSummary(
            ...
            original_errors=sum(
                1 for item in results if item.original_error or item.original_status >= 400
            ),
            replay_errors=sum(
                1 for item in results if item.replay_error or item.replay_status >= 400
            ),
