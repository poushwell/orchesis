"""Experiment runner and results persistence."""
...
class ExperimentRunner:
    """Runs reproducible experiments and saves results."""

    EXPERIMENTS = {
        "exp1": "Session Fingerprint Baseline",
    ...
    def _exp_zipf(self, data: list[dict]) -> dict:
        ...
        tokens = sorted(
            [self._token_value(item) for item in data if isinstance(item, dict)], reverse=True
        )
        ...
        return {
            "zipf_score": round(score, 6),
            "matches_zipf": score >= 0.5,
    ...
    def _exp_baseline(self, data: list[dict]) -> dict:
        """Exp 1: Compute session-fingerprint baseline."""
        dna = SessionFP("experiment")
        for item in data:
    ...
    def run(self, experiment_id: str, data: list[dict]) -> dict:
        ...
        try:
            if exp_id == "exp8":
                ...
                interpretation = (
                    "Token distribution approximates Zipf law."
                    if key_metric >= 0.5
                    else "Token distribution deviates from Zipf law."
                )
            elif exp_id == "exp4":
                results = self._exp_collapse(data)
                key_metric = float(results.get("growth_ratio", 0.0) or 0.0)
                interpretation = (
                    "Context collapse likely detected."
                    if bool(results.get("collapse_detected"))
                    else "No collapse signal detected."
                )
            elif exp_id == "exp1":
                results = self._exp_baseline(data)
                key_metric = float(results.get("baseline", {}).get("error_rate", 0.0) or 0.0)
                interpretation = "Session fingerprint baseline computed."
            else:
