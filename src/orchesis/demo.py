...
DEMO_STATS: dict[str, Any] = {
    ...
    "overwatch_health": "B+",
    "cost_velocity": {
        "current_rate_per_hour": 1.24,
        "projection_24h": 29.76,
        "is_anomalous": False,
    },
    "threat_intel": {
...
class DemoServer:
    ...
    def _handle_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "context_engine": {
                "enabled": True,
                "total_tokens_saved": int(DEMO_STATS["tokens_saved"]),
                "strategy_hits": {"dedup_system_prompt": 321, "remove_stale_messages": 198},
            },
            "context_optimizer": {
                "enabled": True,
    ...
    def _overview(self) -> dict[str, Any]:
        ...
        return {
            ...
            "budget": {
                "limit_usd": 10.0,
                "spent_usd": stats["cost_today"],
                "remaining_usd": max(0.0, 10.0 - float(stats["cost_today"])),
            },
            "recent_events": [
                {
                    "timestamp": now - 50,
                ...
                },
                {
                    ...
                    "type": "warn",
                    "severity": "medium",
                    "description": "Tool approval required for system.run",
                },
            ],
            "cost_timeline": [
                {"timestamp": now - 3600 + i * 300, "cumulative_cost": round(0.8 + i * 0.13, 4)}
                for i in range(12)
            ],
            ...
            "connection_pool": {
                ...
                "hits": 1204,
                "misses": 275,
                "pools": {"api.openai.com": 7},
            },
            "savings": {
                "cache_savings": 20.5,
                "cascade_savings": 11.2,
                "loop_savings": 2.1,
                "total_savings": 33.8,
            },
        }
        ...
        class _Handler(BaseHTTPRequestHandler):
            def do_GET(self) -> None:  # noqa: N802
                            ...
                            "agents": [
                                {
                                    ...
                                    "avg_tokens": 914,
                                    "anomaly_score": 0.08,
                                    "tools_used": ["system.run", "web_fetch"],
                                    "last_seen": "just now",
                                }
                            ]
                        },
                ...
                if path == "/api/tasks/correlations":
                    server._json(
                        self,
                        200,
                        {"insights": ["Tool-approved flows have lower failure rate in demo data."]},
                    )
                    return
                if path == "/api/threats":
                    server._json(
                            ...
                            "findings": [
                                {
                                    "timestamp": "now",
                                    "severity": "medium",
                                    "description": "Demo gap: ASI-10 unsafe plugin design",
                                    "framework_mappings": [["owasp_llm_top10_2025", "ASI-10"]],
                                }
                            ]
                        },
                    )
            ...
            def do_POST(self) -> None:  # noqa: N802
                ...
                if path.startswith("/api/v1/approvals/") and path.endswith("/approve"):
                    request_id = (
                        path.split("/api/v1/approvals/", 1)[1].rsplit("/approve", 1)[0].strip("/")
                    )
                    with server._lock:
                        item = server._pending_approvals.pop(request_id, None)
                        ok = item is not None
                        if ok:
                            server._approval_history.insert(
                                0,
                                {
                                    "status": "approved",
                    ...
                    server._json(
                        self, 200 if ok else 404, {"approved": ok, "approval_id": request_id}
                    )
                    return
                if path.startswith("/api/v1/approvals/") and path.endswith("/deny"):
                    request_id = (
                        path.split("/api/v1/approvals/", 1)[1].rsplit("/deny", 1)[0].strip("/")
                    )
                    with server._lock:
                        item = server._pending_approvals.pop(request_id, None)
                        ok = item is not None
                    ...
                    server._json(
                        self, 200 if ok else 404, {"denied": ok, "approval_id": request_id}
                    )
                    return
                server._json(self, 404, {"error": "not_found"})
