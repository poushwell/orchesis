...
class CredentialInjector:
        ...
        for item in raw_rules:
            ...
            parsed.append(
                InjectionRule(
                    ...
                    if isinstance(item.get("header_name"), str)
                    else None,
                    header_template=item.get("header_template")
                    if isinstance(item.get("header_template"), str)
                    else None,
                    param_name=item.get("param_name")
                    if isinstance(item.get("param_name"), str)
                    else None,
                )
            )
        return parsed
    ...
    def inject(self, tool_call: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
        ...
        output = {
            "tool_name": tool_name,
            "params": dict(
                tool_call.get("params") if isinstance(tool_call.get("params"), dict) else {}
            ),
            "headers": dict(
                tool_call.get("headers") if isinstance(tool_call.get("headers"), dict) else {}
            ),
        }
