...
class IncidentManager:
    ...
    def create(self, threat: dict, agent_id: str) -> dict:
        ...
        row = {
            ...
            "threat_type": str(
                payload.get("type", payload.get("threat_type", "unknown")) or "unknown"
            ),
            "description": str(
                payload.get("description", payload.get("detail", "No description"))
                or "No description"
            ),
            "timeline": [
                {
    ...
    def get_metrics(self) -> dict:
        ...
        total = len(rows)
        open_count = sum(
            1 for row in rows if str(row.get("status", "open")) in {"open", "investigating"}
        )
