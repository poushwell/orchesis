"""arXiv submission validator for paper packages."""
...
class ArxivSubmissionValidator:
    ...
    def validate(self, paper: dict) -> dict:
        ...
        checklist["no_personal_info"] = not any(
            token in serialized for token in ("@gmail.com", "phone:", "telegram:", "passport")
        )
        ...
        if pages < int(self.ARXIV_REQUIREMENTS["min_pages"]):
            warnings.append(
                "Estimated page count below recommended minimum for research submissions."
            )
    ...
    def generate_submission_package(self, paper: dict, output_dir: str) -> dict:
        ...
        category = (
            validation["suggested_categories"][0] if validation["suggested_categories"] else "cs.AI"
        )
        return {
            "files": files,
