...
def paginate(
    items: list[Any], limit: int = 100, offset: int = 0, max_limit: int = 1000
) -> PaginatedResponse:
...
def casura_incident_search(
    ...
    rows = request.app.state.casura_db.search(
        query=query, filters=filters if isinstance(filters, dict) else None
    )
...
async def monitoring_parse_hn(
    ...
    request.app.state.monitoring_opportunities = (
        request.app.state.social_parsers.extract_opportunities(
...
def channels_record_event(
    ...
    if event_type not in {"inbound", "outbound"}:
        raise HTTPException(
            status_code=400, detail={"error": "event_type must be inbound or outbound"}
        )
...
def channels_status(
    ...
    _require_auth(request, authorization)
    payload = request.app.state.channel_monitor.get_channel_status(
...
def vibe_audit_directory_endpoint(
    ...
    extensions = (
        [str(item) for item in ext_raw if str(item).strip()] if isinstance(ext_raw, list) else None
    )
    auditor = VibeCodeAuditor({"severity_threshold": severity.lower()})
    if summary:
