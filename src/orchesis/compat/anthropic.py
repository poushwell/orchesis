...
class AnthropicCompatLayer:
    ...
    ANTHROPIC_MODELS = {
        "claude-opus-4-6": {
            "context": 200000,
            "cost_per_ktok_in": 0.015,
            "cost_per_ktok_out": 0.075,
        },
        "claude-sonnet-4-6": {
            "context": 200000,
            "cost_per_ktok_in": 0.003,
            "cost_per_ktok_out": 0.015,
        },
        "claude-haiku-4-5": {
            "context": 200000,
            "cost_per_ktok_in": 0.00025,
            "cost_per_ktok_out": 0.00125,
        },
    }
    ...
    def _estimate_cost(self, messages: list[dict[str, Any]], model: str) -> float:
        ...
        total_chars = sum(
            len(str(item.get("content", ""))) for item in messages if isinstance(item, dict)
        )
