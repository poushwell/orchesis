...
from orchesis.session_fp import SessionFP
from orchesis.session_fp_store import SessionFPStore
...
from orchesis.fleet_selection import FleetSelectionModel
...
from orchesis.layered_defense import LayeredDefense
...
from orchesis.critical_agent import KeystoneDetector
...
from orchesis.experiment_a import Experiment
...
def create_api_app(
    ...
    persona_cfg = (
        ...
        and isinstance(current_version.policy.get("persona_guardian"), dict)
        else {}
    )
    app.state.persona_guardian = PersonaGuardian(persona_cfg)
    quorum_cfg = (
        current_version.policy.get("fleet_consensus")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("fleet_consensus"), dict)
        else {}
    )
    app.state.quorum_sensor = FleetConsensus(quorum_cfg)
    pid_cfg = (
        current_version.policy.get("pid_controller_v2")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("pid_controller_v2"), dict)
        else {}
    )
    app.state.pid_controller_v2 = PIDControllerV2(pid_cfg)
    state_estimator_cfg = (
        current_version.policy.get("state_estimator")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("state_estimator"), dict)
        else {}
    )
    app.state.state_estimator = StateEstimator(state_estimator_cfg)
    kolmogorov_cfg = (
        current_version.policy.get("content_importance")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("content_importance"), dict)
        else {}
    )
    app.state.content_importance = ContentImportance(kolmogorov_cfg)
    otel_bridge_cfg = (
        current_version.policy.get("otel_bridge")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("otel_bridge"), dict)
        else {}
    )
    app.state.otel_bridge = OpenTelemetryBridge(otel_bridge_cfg)
    vickrey_cfg = (
        current_version.policy.get("budget_allocator")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("budget_allocator"), dict)
        else {}
    )
    app.state.budget_allocator = BudgetAllocator(vickrey_cfg)
    mrac_cfg = (
        current_version.policy.get("adaptive_controller")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("adaptive_controller"), dict)
        else {}
    )
    app.state.adaptive_controller = AdaptiveController(mrac_cfg)
    keystone_cfg = (
        current_version.policy.get("keystone_detector")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("keystone_detector"), dict)
        else {}
    )
    app.state.keystone_detector = KeystoneDetector(keystone_cfg)
    criticality_cfg = (
        current_version.policy.get("quality_control")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("quality_control"), dict)
        else {}
    )
    app.state.quality_controller = QualityController(criticality_cfg)
    immune_cfg = (
        current_version.policy.get("threat_history")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("threat_history"), dict)
        else {}
    )
    app.state.threat_history = ThreatHistory(immune_cfg)
    state_balancer_cfg = (
        current_version.policy.get("state_balancer")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("state_balancer"), dict)
        else {}
    )
    app.state.state_balancer = StateBalancer(state_balancer_cfg)
    threshold_cfg = (
        current_version.policy.get("adaptive_threshold")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("adaptive_threshold"), dict)
        else {}
    )
    ...
    app.state.par_reasoner = FallbackReasoner()
    fleet_selection_cfg = (
        current_version.policy.get("fleet_selection")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("fleet_selection"), dict)
        else {}
    )
    app.state.fleet_selection = FleetSelectionModel(fleet_selection_cfg)
    relevance_cfg = (
        current_version.policy.get("relevance_theory")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("relevance_theory"), dict)
        else {}
    )
    ...
    adversarial_tracker_cfg = (
        current_version.policy.get("adversarial_tracker")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("adversarial_tracker"), dict)
        else {}
    )
    app.state.adversarial_tracker = RedQueenMonitor(adversarial_tracker_cfg)
    double_loop_cfg = (
        current_version.policy.get("adaptive_learning")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("adaptive_learning"), dict)
        else {}
    )
    app.state.double_loop = DoubleLoopLearner(double_loop_cfg)
    complement_cfg = (
        current_version.policy.get("layered_defense")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("layered_defense"), dict)
        else {}
    )
    app.state.layered_defense = LayeredDefense(complement_cfg)
    app.state.agent_diagnostics = AgentDiagnostics()
    app.state.session_forensics = SessionForensics()
    cost_attribution_cfg = (
        current_version.policy.get("cost_attribution")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("cost_attribution"), dict)
        else {}
    )
    ...
    feed_cfg = (
        ...
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("threat_feed"), dict)
        else {}
    )
    app.state.threat_feed = ThreatFeed(feed_cfg)
    app.state.threat_patterns = ThreatPatternLibrary()
    app.state.signature_editor = SignatureEditor(
        str(policy_file.parent / ".orchesis" / "signatures.json")
    )
    app.state.tenant_manager = TenantManager(str(policy_file.parent / ".orchesis" / "tenants"))
    semantic_cfg = (
        current_version.policy.get("semantic_cache")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("semantic_cache"), dict)
        else {}
    )
    ...
    raw_alert_rules = (
        ...
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("alert_rules"), list)
        else []
    )
    ...
    app.state.dna_store = SessionFPStore(str(policy_file.parent / ".orchesis" / "dna"))
    ...
    app.state.context_sync = ContextSyncManager()
    app.state.gossip_protocol = GossipProtocol()
    flywheel_cfg = (
        current_version.policy.get("data_flywheel")
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("data_flywheel"), dict)
        else {}
    )
    ...
    community_cfg = (
        ...
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("community"), dict)
        else {}
    )
    ...
    app.state.experiment_a = Experiment()
    ...
    anomaly_cfg = (
        ...
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("anomaly_alerts"), dict)
        else {}
    )
    ...
    mcp_monitor_cfg = (
        ...
        if isinstance(current_version.policy, dict)
        and isinstance(current_version.policy.get("mcp_monitor"), dict)
        else {}
    )
    ...
    def _sync_decision_emitter(candidate_policy: dict[str, Any]) -> None:
        ...
        decision_log_subscriber_id = event_bus.subscribe(
            JsonlEmitter(decisions_log, redactor=redactor)
        )
    ...
    def _shadow_engine(
        request_payload: dict[str, Any], shadow_policy: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _sync_shadow_mode(candidate_policy: dict[str, Any]) -> None:
        ...
        shadow_policy_ref = str(
            cfg.get("shadow_policy", "shadow_policy.yaml") or "shadow_policy.yaml"
        ).strip()
    ...
    def _sync_data_flywheel(candidate_policy: dict[str, Any]) -> None:
        cfg = (
            ...
            and isinstance(candidate_policy.get("data_flywheel"), dict)
            else {}
        )
    ...
    def _sync_are_from_policy(candidate_policy: dict[str, Any]) -> None:
        ...
        slos = (
            are_cfg.get("slos")
            if isinstance(are_cfg, dict) and isinstance(are_cfg.get("slos"), list)
            else defaults
        )
        loaded = 0
        for row in slos:
        ...
        return {
            ...
            "dimensions": {
                ...
                "observability": {
                    ...
                    "weight": round(
                        (float(observability.weight if observability else 0.0) / 100.0), 4
                    ),
                },
            },
    ...
    def _build_compliance_report(agent_id: str) -> dict[str, Any]:
        ...
        filtered = [
            event for event in events if str(getattr(event, "agent_id", "")) == str(agent_id)
        ]
    ...
    def _build_export_zip() -> bytes:
        ...
        with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as zf:
            ...
            zf.writestr(
                "evidence_record.json", json.dumps(evidence_record, ensure_ascii=False, indent=2)
            )
            zf.writestr(
                "compliance_report.json",
                json.dumps(compliance_report, ensure_ascii=False, indent=2),
            )
    ...
    def _build_context_budget_payload(session_id: str | None = None) -> dict[str, Any]:
        ...
        model_windows = (
            ...
            and isinstance(context_cfg.get("model_context_windows"), dict)
            else {}
        )
        ...
        return {
            ...
            if current_level in {"normal", "L0", "L1", "L2"}
            else "normal",
    ...
    def _build_hourly_cost_points(history_days: int = 7) -> list[dict[str, Any]]:
        ...
        for event in events:
            ...
            buckets[hour_key] = buckets.get(hour_key, 0.0) + float(
                getattr(event, "cost", 0.0) or 0.0
            )
        if not buckets:
            return []
    ...
    def _build_vibe_audit_payload(
        code: str, language: str, severity: str = "low"
    ) -> dict[str, Any]:
    ...
    def _build_search_payload(query: str, limit: int) -> dict[str, Any]:
        ...
        for event in events:
            ...
            if not isinstance(snapshot, dict):
                snapshot = {}
            session_id = _extract_session_id(snapshot) or str(
                snapshot.get("session_id") or "__default__"
            )
        ...
        return {
            "query": q,
            "results": {"agents": agents, "sessions": sessions, "threats": threats},
            "total": total,
        }
    ...
    def _build_rate_limit_status_payload() -> dict[str, Any]:
        ...
        for agent_id in sorted(agents_seen):
            ...
            limit = (
                ...
                and isinstance(identity.rate_limit_per_minute, int)
                and identity.rate_limit_per_minute > 0
                else default_limit
            )
    ...
    def _build_agent_graph_model() -> AgentCollaborationGraph:
        ...
        for event in read_events_from_jsonl(source):
            ...
            interaction_type = (
                ...
                if isinstance(snapshot.get("interaction_type"), str)
                and str(snapshot.get("interaction_type")).strip()
                else "context_share"
            )
            targets: list[str] = []
            for key in (
    ...
    def _record_flow_decision(payload: dict[str, Any], response: dict[str, Any]) -> None:
        ...
        if dna is None:
            dna = SessionFP(agent_id=agent_id)
    ...
    async def trace_headers_middleware(request: Request, call_next: Any) -> Response:
        ...
        if _is_protected_api_request(request):
            client_id = str(
                getattr(request.state, "rate_limit_client_id", "")
                or _client_id_from_request(request)
            )
            limit_status = app.state.api_limiter.check(client_id)
            response.headers["X-RateLimit-Remaining"] = str(
                int(limit_status.get("remaining", 0) or 0)
            )
    ...
    def docs_index() -> Response:
        ...
        for route in app.routes:
            ...
            if not methods:
                continue
            rows.append(f"<tr><td>{', '.join(methods)}</td><td><code>{route.path}</code></td></tr>")
        rows.sort()
        html = (
    ...
    def policy_simulate_impact(
        ...
        if not isinstance(current_policy, dict) or not isinstance(new_policy, dict):
            raise HTTPException(
                status_code=400, detail={"error": "current_policy and new_policy must be objects"}
            )
        if not isinstance(sample_requests, list):
            raise HTTPException(status_code=400, detail={"error": "sample_requests must be a list"})
        return app.state.policy_impact_analyzer.simulate(
            current_policy, new_policy, sample_requests
        )
    ...
    def replay_session_with_policy_endpoint(
        ...
        effective_policy = (
            policy if isinstance(policy, dict) else (body if isinstance(body, dict) else None)
        )
        replayer = SessionReplay(app.state.decisions_log)
    ...
    def session_groups_create(
        ...
        return app.state.session_groups.create_group(
            name=name.strip(), description=str(description or "")
        )
    ...
    def kb_article(
        article_id: str, authorization: str | None = Header(default=None)
    ) -> dict[str, Any]:
        _require_auth(authorization)
    ...
    @app.post("/api/v1/reasoning/observe")
    def par_observe(
        body: dict[str, Any] | None = None,
        authorization: str | None = Header(default=None),
    ) -> dict[str, Any]:
        ...
        return {
            "ok": True,
            "observations": int(app.state.par_reasoner.get_stats().get("observations", 0)),
        }

    @app.post("/api/v1/reasoning/abduce")
    def par_abduce(
        body: dict[str, Any] | None = None,
        authorization: str | None = Header(default=None),
    ) -> dict[str, Any]:
    ...
    @app.get("/api/v1/reasoning/causal-graph")
    ...
    @app.get("/api/v1/reasoning/stats")
    ...
    def immune_expose(
        ...
        return app.state.threat_history.expose(
            threat_pattern=threat_pattern, severity=float(severity)
        )
    ...
    def immune_recall(
        ...
        if not threat_pattern:
            raise HTTPException(status_code=400, detail={"error": "threat_pattern is required"})
        row = app.state.threat_history.recall(threat_pattern)
        if row is None:
    ...
    def immune_stats(authorization: str | None = Header(default=None)) -> dict[str, Any]:
        _require_auth(authorization)
        return app.state.threat_history.get_memory_stats()

    @app.post("/api/v1/state_balancer/measure")
    def state_balancer_measure(
        body: dict[str, Any] | None = None,
        authorization: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _require_auth(authorization)
        ...
        if not isinstance(quality_score, int | float):
            raise HTTPException(status_code=400, detail={"error": "quality_score must be numeric"})
        return app.state.state_balancer.measure(float(quality_score))

    @app.get("/api/v1/state_balancer/stats")
    def state_balancer_stats(authorization: str | None = Header(default=None)) -> dict[str, Any]:
        _require_auth(authorization)
        return app.state.state_balancer.get_equilibrium_stats()
    ...
    def threshold_feedback(
        ...
        if not isinstance(predicted, bool) or not isinstance(actual, bool):
            raise HTTPException(
                status_code=400, detail={"error": "predicted and actual must be boolean"}
            )
        app.state.adaptive_threshold.record_feedback(
            detector=detector, predicted=predicted, actual=actual
        )
    ...
    def threshold_get(
        detector: str, authorization: str | None = Header(default=None)
    ) -> dict[str, Any]:
        _require_auth(authorization)
    ...
    def fleet_selection_register(
        ...
        if not agent_id or not group_id:
            raise HTTPException(
                status_code=400, detail={"error": "agent_id and group_id are required"}
            )
        app.state.fleet_selection.register_agent(agent_id=agent_id, group_id=group_id)
        return {"ok": True, "agent_id": agent_id, "group_id": group_id}

    @app.post("/api/v1/group-selection/interaction")
    def fleet_selection_interaction(
        body: dict[str, Any] | None = None,
        authorization: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _require_auth(authorization)
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail={"error": "outcome must be numeric"}
            ) from error
        result = app.state.fleet_selection.record_interaction(
            agent_id=agent_id,
    ...
    def fleet_selection_group(
        group_id: str,
        authorization: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _require_auth(authorization)
        return app.state.fleet_selection.get_group_fitness(group_id)

    @app.get("/api/v1/group-selection/fittest")
    def fleet_selection_fittest(
        authorization: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _require_auth(authorization)
        row = app.state.fleet_selection.get_fittest_group()
        if row is None:
            return {"group": None}
        return {"group": row}

    @app.get("/api/v1/group-selection/stats")
    def fleet_selection_stats(authorization: str | None = Header(default=None)) -> dict[str, Any]:
        _require_auth(authorization)
        return app.state.fleet_selection.get_stats()
    ...
    def quorum_register_task(
        ...
        if not agent_id or not task_fingerprint:
            raise HTTPException(
                status_code=400, detail={"error": "agent_id and task_fingerprint are required"}
            )
    ...
    def quorum_contribute(
        ...
        if not agent_id or not isinstance(context, dict):
            raise HTTPException(
                status_code=400, detail={"error": "agent_id and context are required"}
            )
        ok = app.state.quorum_sensor.contribute_context(
            quorum_id=quorum_id, agent_id=agent_id, context=context
        )
        if not ok:
            raise HTTPException(status_code=404, detail={"error": "quorum not found"})
    ...
    def pid_update(
        ...
        if not isinstance(current_value, int | float) or not isinstance(setpoint, int | float):
            raise HTTPException(
                status_code=400, detail={"error": "current_value and setpoint must be numbers"}
            )
    ...
    def pid_check_zipf(
        ...
        if not isinstance(values, list):
            raise HTTPException(
                status_code=400, detail={"error": "token_frequencies must be a list"}
            )
    ...
    def state_estimator_update(
        ...
        _require_auth(authorization)
        payload = body if isinstance(body, dict) else {}
        tracker = app.state.state_estimator
    ...
    @app.get("/api/v1/state-estimator/{session_id}/state")
    def state_estimator_state(
        session_id: str,
        authorization: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _require_auth(authorization)
    ...
    @app.get("/api/v1/state-estimator/{session_id}/alert")
    def state_estimator_alert(
        session_id: str,
        authorization: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _require_auth(authorization)
    ...
    @app.get("/api/v1/state-estimator/sessions")
    def state_estimator_sessions(
        authorization: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _require_auth(authorization)
        tracker = app.state.state_estimator
    ...
    @app.post("/api/v1/importance/estimate")
    def kolmogorov_estimate(
        ...
        if isinstance(message, dict):
            return app.state.content_importance.compute_importance(message)
        text = payload.get("text")
        if isinstance(text, str):
            return app.state.content_importance.compute_importance(
                {"role": "user", "content": text}
            )
        raise HTTPException(status_code=400, detail={"error": "message or text is required"})

    @app.post("/api/v1/importance/record-correlation")
    def kolmogorov_record_correlation(
        ...
        if not isinstance(k_score, int | float) or not isinstance(uci_score, int | float):
            raise HTTPException(
                status_code=400, detail={"error": "k_score and uci_score must be numbers"}
            )
        app.state.content_importance.record_correlation(
            float(k_score), float(uci_score), str(decision)
        )
        return app.state.content_importance.get_stats()

    @app.get("/api/v1/importance/stats")
    ...
    @app.get("/api/v1/importance/rho")
    def kolmogorov_rho(
        authorization: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _require_auth(authorization)
        return {"rho": app.state.content_importance.compute_rho()}
    ...
    def pipeline_metrics(
        ...
        return {
            ...
            "uci_stats": app.state.content_importance.get_stats(),
            "pid_warning_level": str(pid_state.get("level", "green")),
        }

    @app.get("/api/v1/pipeline/invariants")
    def impossibility_theorems(
        authorization: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _require_auth(authorization)
        return {
            "theorems": [
                {
                    "id": "I1",
                    "statement": "Per-request scoping for fleet-level metrics has a hard cost trade-off",
                },
                {"id": "I2", "statement": "A compromised agent cannot reliably detect its own compromise"},
                {
                    "id": "I3",
                    "statement": "Single-agent traces cannot fully recover cross-agent causal graphs",
                },
                {
                    "id": "I4",
                    "statement": "Local checks cannot guarantee global coherence under partition",
                },
                {"id": "I5", "statement": "Fallback reasoning triggers when N < 2^k observations"},
            ]
        }
    ...
    def vickrey_bid(
        ...
        if not agent_id or not isinstance(bid_tokens, int | float):
            raise HTTPException(
                status_code=400, detail={"error": "agent_id and bid_tokens are required"}
            )
        return app.state.budget_allocator.submit_bid(
            agent_id=agent_id,
    ...
    def vickrey_allocation(
        ...
        return {
            "agent_id": agent_id,
            "allocation": app.state.budget_allocator.get_allocation(agent_id),
        }
    ...
    def mrac_update(
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail={"error": "actual_quality must be numeric"}
            ) from error
        return app.state.adaptive_controller.update(agent_id=agent_id, actual_quality=actual_quality)

    @app.get("/api/v1/mrac/agents")
    def mrac_agents(authorization: str | None = Header(default=None)) -> dict[str, Any]:
        _require_auth(authorization)
        rows = app.state.adaptive_controller.get_all_agents()
    ...
    def mrac_stats(authorization: str | None = Header(default=None)) -> dict[str, Any]:
        _require_auth(authorization)
        return app.state.adaptive_controller.get_stats()
    ...
    def mrac_gains(
        ...
        _require_auth(authorization)
        return {"agent_id": agent_id, **app.state.adaptive_controller.get_gains(agent_id)}
    ...
    def keystone_record_uci(
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail={"error": "uci_score must be numeric"}
            ) from error
    ...
    def quality_control(
        ...
        return app.state.quality_controller.compute_control(float(psi))
    ...
    def criticality_mrac_update(
        ...
        if not isinstance(psi_actual, int | float) or not isinstance(psi_predicted, int | float):
            raise HTTPException(
                status_code=400, detail={"error": "psi_actual and psi_predicted must be numeric"}
            )
        updated = app.state.quality_controller.mrac_update(
            float(psi_actual), float(psi_predicted)
        )
    ...
    def criticality_stats(authorization: str | None = Header(default=None)) -> dict[str, Any]:
        _require_auth(authorization)
        return app.state.quality_controller.get_stats()
    ...
    def arc_certify(
        ...
        readiness_cfg = (
            policy.get("agent_readiness", {})
            if isinstance(policy.get("agent_readiness"), dict)
            else {}
        )
        metrics_store = (
            readiness_cfg.get("metrics", {})
            if isinstance(readiness_cfg.get("metrics"), dict)
            else {}
        )
        policy_metrics = (
        ...
        return app.state.arc_readiness.certify(
            agent_id=agent_id, metrics=merged_metrics, policy=policy
        )
    ...
    def arc_certificate(
        ...
        matches = [
            item
            for item in app.state.arc_readiness.list_certificates()
    ...
    def compare_record_metric(
        ...
        if not agent_id or not metric or not isinstance(value, int | float):
            raise HTTPException(
                status_code=400, detail={"error": "agent_id, metric, value are required"}
            )
    ...
    def weekly_report_endpoint(
        ...
        data = {
            "security": {
                ...
                "new_sigs": 0,
                "ari": float(app.state.adversarial_tracker.compute_arms_race_index().get("ari", 0.0) or 0.0),
            },
            "cost": {
                "cost": total_cost_usd,
                "savings": 0.0,
            ...
            "competitive": {
                "threats": len(competitive.get("threats", []))
                if isinstance(competitive.get("threats"), list)
                else 0,
                "opportunities": (
    ...
    def adversarial_tracker_attack_endpoint(
        body: dict[str, Any],
        authorization: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _require_auth(authorization)
        app.state.adversarial_tracker.record_attack(body if isinstance(body, dict) else {})
        return {"status": "recorded", "kind": "attack", "stats": app.state.adversarial_tracker.get_stats()}

    @app.post("/api/v1/red-queen/detection")
    def adversarial_tracker_detection_endpoint(
        body: dict[str, Any],
        authorization: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _require_auth(authorization)
        app.state.adversarial_tracker.record_detection(body if isinstance(body, dict) else {})
        return {"status": "recorded", "kind": "detection", "stats": app.state.adversarial_tracker.get_stats()}

    @app.get("/api/v1/red-queen/ari")
    def adversarial_tracker_ari_endpoint(
        authorization: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _require_auth(authorization)
        return app.state.adversarial_tracker.compute_arms_race_index()

    @app.get("/api/v1/red-queen/emerging-patterns")
    def adversarial_tracker_emerging_patterns_endpoint(
        authorization: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _require_auth(authorization)
        patterns = app.state.adversarial_tracker.get_emerging_patterns()
    ...
    def double_loop_adapt_endpoint(
        ...
        if loop == "single":
            rule = str(
                payload.get("rule", "compression_aggressiveness") or "compression_aggressiveness"
            )
        ...
        if loop == "double":
            strategy = str(
                payload.get("new_strategy", "reframe_context_strategy")
                or "reframe_context_strategy"
            )
            rationale = str(
                payload.get("rationale", "error rate above double-loop threshold") or ""
            )
            return {
                "loop": loop,
    ...
    def complement_activate_endpoint(
        ...
        return app.state.layered_defense.activate(float(threat_signal), threat_type)
    ...
    def complement_stats_endpoint(
        ...
        _require_auth(authorization)
        return app.state.layered_defense.get_cascade_stats()
    ...
    def whatsapp_register_session(
        ...
        if not session_id:
            raise HTTPException(status_code=400, detail={"error": "session_id is required"})
        return app.state.whatsapp_expiry.register_session(
    ...
    def predict_history_endpoint(
        ...
        if isinstance(agent_id, str) and agent_id.strip():
            return {
                ...
                "history": app.state.anomaly_predictor.get_predictions_history(agent_id.strip()),
            }
    ...
    def byzantine_observe_endpoint(
        ...
        return {
            ...
            "agent_id": agent_id.strip(),
            "observations": len(
                app.state.byzantine_detector._observations.get(agent_id.strip(), [])
            ),
        }
    ...
    def byzantine_cross_validate_endpoint(
        ...
        return app.state.byzantine_detector.cross_validate(
            agent_a=agent_a, agent_b=agent_b, query=query
        )
    ...
    def raft_append_endpoint(
        ...
        if not agent_id:
            raise HTTPException(status_code=400, detail="agent_id is required")
        return app.state.context_sync.append_entry(key=key, value=value, agent_id=agent_id)
    ...
    def raft_acknowledge_endpoint(
        ...
        ok = app.state.context_sync.acknowledge(agent_id=agent_id, index=idx)
        return {"ok": ok}
    ...
    def raft_divergent_endpoint(
        ...
        _require_auth(authorization)
        return {"agents": app.state.context_sync.get_divergent_agents()}

    @app.get("/api/v1/raft/{agent_id}/context")
    def context_sync_endpoint(
        agent_id: str,
        authorization: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _require_auth(authorization)
        return app.state.context_sync.get_consistent_context(agent_id=agent_id)
    ...
    def raft_sync_endpoint(
        ...
        _require_auth(authorization)
        return app.state.context_sync.sync_agent(agent_id=agent_id)
    ...
    def raft_stats_endpoint(
        ...
        _require_auth(authorization)
        return app.state.context_sync.get_raft_stats()
    ...
    def get_overwatch_teams(authorization: str | None = Header(default=None)) -> dict[str, Any]:
        ...
        return {
            "teams": [
                app.state.agent_policy_store.get_team_summary(team_id) for team_id in team_ids
            ]
        }
    ...
    def cost_attribution_chargebacks(
        ...
        return {
            "period": period,
            "chargebacks": app.state.cost_attribution.get_chargebacks(period=period),
        }
    ...
    def cost_attribution_team(
        ...
        return {
            ...
            "forecast": app.state.cost_attribution.forecast_by_team(
                team, days_ahead=max(1, int(days_ahead))
            ),
        }
    ...
    def get_notifications(
        ...
        if safe_since > 0:
            records = [
                item for item in records if float(item.get("timestamp", 0.0) or 0.0) > safe_since
            ]
    ...
    def lifecycle_register(
        ...
        try:
            return app.state.agent_lifecycle.register(
                agent_id, metadata if isinstance(metadata, dict) else None
            )
    ...
    def get_tenant(
        ...
        resolved = app.state.tenant_manager.resolve_policy(
            tenant_id, app.state.current_version.policy
        )
    ...
    def shadow_status(authorization: str | None = Header(default=None)) -> dict[str, Any]:
        ...
        recommendation = (
            runner.get_recommendation() if isinstance(runner, ShadowModeRunner) else "disabled"
        )
        return {
    ...
    def shadow_enable(authorization: str | None = Header(default=None)) -> dict[str, Any]:
        ...
        runner = getattr(app.state, "shadow_runner", None)
        if not isinstance(runner, ShadowModeRunner):
            app.state.shadow_runner = ShadowModeRunner(
                dict(app.state.current_version.policy), _shadow_engine
            )
        return {"enabled": True}
    ...
    def flow_export(
        ...
        pipeline_phases = [
            {"phase": key, "count": value} for key, value in sorted(phase_counts.items())
        ]
        ...
        duration_ms = round(
            sum(float(item.get("duration_ms", 0.0) or 0.0) for item in decisions), 6
        )
    ...
    def _evaluate_payload(body: dict[str, Any], trace: TraceContext) -> dict[str, Any]:
        ...
        session_type = (
            session_type if isinstance(session_type, str) and session_type.strip() else "cli"
        )
        ...
        completion_tokens_raw = payload.get(
            "completion_tokens", payload_context.get("completion_tokens", 0)
        )
    ...
    def _authenticate_request(request: Request, eval_payload: dict[str, Any]) -> tuple[bool, str]:
        ...
        if authenticator is None:
            return True, ""
        allowed, agent_id, reason = authenticator.authenticate_request(
            eval_payload, dict(request.headers)
        )
        if not allowed:
            if mode == "enforce":
                raise HTTPException(
                    status_code=401, detail={"error": "unauthorized", "reason": reason}
                )
            if mode == "log":
    ...
    def _load_autopsy_decisions_rows() -> list[dict[str, Any]]:
        ...
        for line in source.read_text(encoding="utf-8").splitlines():
            ...
            if not isinstance(state, dict):
                state = {}
            session_id = str(row.get("session_id") or state.get("session_id") or "__global__")
    ...
    def autopsy_perform_endpoint(
        ...
        report = app.state.agent_diagnostics.perform(session_id=session_id, decisions_log=rows)
        if "error" in report:
    ...
    def autopsy_get_endpoint(
        ...
        _require_auth(authorization)
        if session_id == "list":
            rows = app.state.agent_diagnostics.list_recent(limit=20)
            return {"autopsies": rows, "count": len(rows)}
        existing = app.state.agent_diagnostics.get(session_id)
    ...
    def autopsy_list_endpoint(
        ...
        _require_auth(authorization)
        rows = app.state.agent_diagnostics.list_recent(limit=max(1, min(500, int(limit))))
        return {"autopsies": rows, "count": len(rows)}
    ...
    def session_forensics_analyze_endpoint(
        ...
        events = [
            row
            for row in _load_autopsy_decisions_rows()
    ...
    def incidents_update_status(
        ...
        ok = app.state.incident_manager.update_status(
            incident_id=incident_id, status=status, note=note
        )
        if not ok:
    ...
    @app.post("/api/v1/experiment/trial")
    def experiment_record_trial(
        body: dict[str, Any] | None = None,
        authorization: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _require_auth(authorization)
        ...
        if order not in {"security_first", "task_first"}:
            raise HTTPException(
                status_code=400, detail={"error": "order must be security_first or task_first"}
            )
        security_score = float(payload.get("security_score", 0.0))
        task_score = float(payload.get("task_score", 0.0))
        trial = app.state.experiment_a.record_trial(
            order=order, security_score=security_score, task_score=task_score
        )
        return {"ok": True, "trial": trial}

    @app.get("/api/v1/experiment/results")
    def experiment_results(authorization: str | None = Header(default=None)) -> dict[str, Any]:
        _require_auth(authorization)
        return app.state.experiment_a.compute_delta_bar()

    @app.get("/api/v1/experiment/stats")
    def experiment_stats(authorization: str | None = Header(default=None)) -> dict[str, Any]:
        _require_auth(authorization)
        return app.state.experiment_a.get_stats()
