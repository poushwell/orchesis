...
def _load_yaml(text: str) -> Any:
    ...
    try:
        ...
        raise ImportError(
            "YAML config requires pyyaml. Install with: pip install orchesis[yaml]"
        ) from None
...
def _normalize_policy_paths(policy: dict[str, Any]) -> None:
    ...
    if isinstance(tool_access, dict):
        ...
        allowed = (
            set(tool_access.get("allowed", []))
            if isinstance(tool_access.get("allowed"), list)
            else set()
        )
        denied = (
            set(tool_access.get("denied", []))
            if isinstance(tool_access.get("denied"), list)
            else set()
        )
...
def _normalize_cost_controls(policy: dict[str, Any]) -> None:
    ...
    spend_rate["spike_multiplier"] = (
        float(spike_multiplier)
        if _is_number(spike_multiplier) and float(spike_multiplier) > 0
        else 5.0
    )
    pause_seconds = spend_rate.get("pause_seconds", 300)
    spend_rate["pause_seconds"] = (
        int(pause_seconds) if _is_number(pause_seconds) and int(pause_seconds) > 0 else 300
    )
    ...
    loop_detection["warn_threshold"] = (
        int(warn_threshold) if isinstance(warn_threshold, int | float) else 5
    )
    loop_detection["block_threshold"] = (
        int(block_threshold) if isinstance(block_threshold, int | float) else 10
    )
    loop_detection["window_seconds"] = (
        float(window_seconds) if isinstance(window_seconds, int | float) else 300.0
    )
...
def _normalize_proxy_config(policy: dict[str, Any]) -> None:
    ...
    max_accumulated = streaming_cfg.get(
        "max_accumulated_events", DEFAULT_STREAMING_MAX_ACCUMULATED_EVENTS
    )
...
def _normalize_kill_switch(policy: dict[str, Any]) -> None:
    ...
    if raw is None:
        policy["kill_switch"] = {
            "enabled": False,
            "auto_triggers": {},
            "resume_token": DEFAULT_RESUME_TOKEN,
        }
        return
    ...
    auto["cost_multiplier"] = (
        float(cost_multiplier)
        if _is_number(cost_multiplier) and float(cost_multiplier) > 0
        else 5.0
    )
    auto["secrets_threshold"] = (
        int(secrets_threshold)
        if _is_number(secrets_threshold) and int(secrets_threshold) > 0
        else 3
    )
    auto["loops_threshold"] = (
        int(loops_threshold) if _is_number(loops_threshold) and int(loops_threshold) > 0 else 5
    )
...
def _normalize_loop_detection(policy: dict[str, Any]) -> None:
    ...
    raw["window_seconds"] = float(
        max(raw["exact"]["window_seconds"], raw["fuzzy"]["window_seconds"])
    )
...
def _normalize_behavioral_fingerprint(policy: dict[str, Any]) -> None:
    ...
    for dim_name, default in DEFAULT_DIMENSIONS.items():
        ...
        if not _is_number(z_threshold) or float(z_threshold) <= 0:
            raise PolicyError(
                f"behavioral_fingerprint.dimensions.{dim_name}.z_threshold must be > 0"
            )
        if action not in {"warn", "block", "log"}:
            raise PolicyError(
                f"behavioral_fingerprint.dimensions.{dim_name}.action must be warn|block|log"
            )
    ...
    for index, chain in enumerate(chains):
        ...
        for item in chain:
            if not isinstance(item, str) or not item.strip():
                raise PolicyError(
                    f"flow_xray.suspicious_tool_chains[{index}] entries must be non-empty strings"
                )
...
def _normalize_experiments(policy: dict[str, Any]) -> None:
    ...
    raw["default_min_sample_size"] = (
        int(raw.get("default_min_sample_size", 30))
        if _is_number(raw.get("default_min_sample_size", 30))
        else 30
    )
    raw["auto_stop_on_significance"] = bool(raw.get("auto_stop_on_significance", True))
    raw["significance_threshold"] = (
        float(raw.get("significance_threshold", 0.95))
        if _is_number(raw.get("significance_threshold", 0.95))
        else 0.95
    )
...
def _normalize_task_tracking(policy: dict[str, Any]) -> None:
    ...
    raw["idle_timeout_seconds"] = (
        float(raw.get("idle_timeout_seconds", 300))
        if _is_number(raw.get("idle_timeout_seconds", 300))
        else 300.0
    )
    raw["min_turns_for_success"] = (
        int(raw.get("min_turns_for_success", 1))
        if _is_number(raw.get("min_turns_for_success", 1))
        else 1
    )
    raw["consecutive_errors_threshold"] = (
        int(raw.get("consecutive_errors_threshold", 3))
        if _is_number(raw.get("consecutive_errors_threshold", 3))
        else 3
    )
...
def _normalize_threat_intel(policy: dict[str, Any]) -> None:
    ...
    raw["custom_signatures"] = (
        [c for c in custom if isinstance(c, dict)] if isinstance(custom, list) else []
    )
...
def _normalize_alerts(policy: dict[str, Any]) -> None:
    ...
    if isinstance(notify_on_raw, list):
        notify_on = [
            item.strip() for item in notify_on_raw if isinstance(item, str) and item.strip()
        ]
    else:
...
def _normalize_semantic_cache(policy: dict[str, Any]) -> None:
    ...
    raw["cacheable_models"] = (
        [str(m) for m in cacheable if m] if isinstance(cacheable, list) else []
    )
...
def _normalize_capability_constraints(raw: Any, *, key: str, index: int, section: str) -> list[str]:
    ...
    for item in raw:
        if not isinstance(item, str) or not item.strip():
            raise PolicyError(
                f"capabilities[{index}].{section}.{key} entries must be non-empty strings"
            )
...
def _normalize_capability_rule(raw: Any, *, index: int, section: str) -> dict[str, list[str]]:
    ...
    for key, value in raw.items():
        ...
        normalized[key] = _normalize_capability_constraints(
            value, key=key, index=index, section=section
        )
    return normalized
    ...
    if isinstance(value, dict):
        ...
        for key, inner in value.items():
            ...
            if kl in {"resume_token", "bot_token"} or _SENSITIVE_KEY_FRAGMENT.search(ks):
                ...
                out[key] = {
                    hk: "***" if isinstance(hv, str) else _redact_config(hv)
    ...
    *,
    ...
    if bool(kill.get("enabled", False)):
        ...
        if token == DEFAULT_RESUME_TOKEN:
            warnings.append(
                "Default resume token in use — set a strong token in policy for production"
            )
        elif len(token) < 16:
            warnings.append(
    ...
    def __init__(
        ...
        *,
        ...
        self._check_interval_s: float = (
            _resolve_config_check_interval()
