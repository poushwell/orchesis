...
class AABBBenchmark:
    ...
    def run_category(self, agent_id: str, category: str) -> dict:
        ...
        for metric in metrics:
            ...
            if ok:
                ...
                failed += 1
        category_score = (
            round(sum(item["score"] for item in checks) / len(checks), 2) if checks else 0.0
        )
        return {
            "agent_id": aid,
            "category": cat,
    ...
    def compare_agents(self, agent_a: str, agent_b: str) -> dict:
        ...
        with self._lock:
            for row in self._results.values():
                ...
                if agent_id == aid and (
                    best_a is None
                    or row.get("overall_score", 0.0) > best_a.get("overall_score", 0.0)
                ):
                    best_a = row
                if agent_id == bid and (
                    best_b is None
                    or row.get("overall_score", 0.0) > best_b.get("overall_score", 0.0)
                ):
                    best_b = row
        ...
        cat_a = (
            best_a.get("category_scores", {})
            if isinstance(best_a, dict) and isinstance(best_a.get("category_scores"), dict)
            else {}
        )
        cat_b = (
            best_b.get("category_scores", {})
            if isinstance(best_b, dict) and isinstance(best_b.get("category_scores"), dict)
            else {}
        )
        all_cats = sorted(set(cat_a.keys()) | set(cat_b.keys()))
        category_diff = {
            cat: round(float(cat_a.get(cat, 0.0)) - float(cat_b.get(cat, 0.0)), 2)
            for cat in all_cats
        }
    ...
    def get_benchmark_stats(self) -> dict:
        with self._lock:
            ...
            total_runs = len(rows)
            unique_agents = len(
                {str(row.get("agent_id", "unknown")) for row in rows if isinstance(row, dict)}
            )
