...
class ResponseAnalyzer:
    ...
    def analyze(self, response: dict) -> dict:
        ...
        if hallucination.get("suspicious"):
            issues.append(
                {"type": "hallucination_signals", "signals": hallucination.get("signals", [])}
            )
        ...
        word_count = (
            len([part for part in re.split(r"\s+", text.strip()) if part]) if text.strip() else 0
        )
    ...
    def _sentiment(text: str) -> str:
        lowered = text.lower()
        positive = sum(
            word in lowered for word in ("great", "good", "success", "happy", "excellent")
        )
