...
def persona_baseline(
    ...
    files = (
        [str(item) for item in identity_files if isinstance(item, str) and item.strip()]
...
def persona_cron_event(
    ...
    event = request.app.state.persona_guardian.record_cron_event(
        cron_expression=cron_expression, source=source
    )
    alert = request.app.state.persona_guardian.check_zenity_pattern()
...
def alert_rules_evaluate(
    ...
    metric_values = (
        ...
        else {"deny_rate": 0.0, "events_per_minute": 0.0}
    )
