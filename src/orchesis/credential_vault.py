...
class FileVault(CredentialVault):
    ...
    def _get_or_create_machine_key(self) -> str:
        ...
        print(
            "[Orchesis] Set ORCHESIS_VAULT_PASSPHRASE env var for explicit control.",
            file=sys.stderr,
        )
        print(
            "[Orchesis] Keep this file secure - losing it means losing vault access.",
            file=sys.stderr,
        )
        return key
    ...
    def _derive_key(self, salt: bytes) -> bytes:
        return hashlib.pbkdf2_hmac(
            "sha256", self._passphrase.encode("utf-8"), salt, 120_000, dklen=32
        )
    ...
    def _save(self, values: dict[str, str]) -> None:
        ...
        salt = os.urandom(16)
        key = self._derive_key(salt)
        plaintext = json.dumps({"credentials": values}, ensure_ascii=False, sort_keys=True).encode(
            "utf-8"
        )
...
def build_vault_from_policy(
    policy: dict[str, Any], *, passphrase: str | None = None
) -> CredentialVault:
