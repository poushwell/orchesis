...
class CascadeClassifier:
    ...
    def classify(
        self, parsed_request: ParsedRequest, context: dict[str, Any] | None = None
    ) -> CascadeLevel:
...
class CascadeRouter:
    ...
    def _level_cfg(self, level: CascadeLevel) -> dict[str, Any]:
        return (
            self._levels.get(self._level_name(level), {}) if isinstance(self._levels, dict) else {}
        )
    ...
    def route(
        ...
        if not self._enabled:
            ...
            return CascadeDecision(
                model=model, max_tokens=max_tokens, cascade_level=CascadeLevel.SIMPLE
            )
        ...
        max_tokens = int(
            level_cfg.get("max_tokens", parsed_request.raw_body.get("max_tokens", 0) or 0)
        )
    ...
    def should_escalate(
        self, status_code: int, parsed_response: ParsedResponse | None = None
    ) -> bool:
        if not self._enabled:
            return False
        ...
        if bool(self._auto_escalate.get("on_error", True)) and status_code >= 400:
            return True
        if bool(self._auto_escalate.get("on_low_confidence", True)) and isinstance(
