...
class ContextStrategyRouter:
    ...
    def classify(self, messages: list[dict], tools_used: list[str]) -> str:
        ...
        msg_text = " ".join(
            str(item.get("content", "")).lower() for item in messages if isinstance(item, dict)
        )
        ...
        if strategy == "summarize_old":
            ...
            if dropped > 0:
                summary = [
                    {
                        "role": "assistant",
                        "content": f"Summary: {dropped} older messages condensed.",
                    }
                ]
        ...
        if strategy == "hierarchical":
            ...
            if middle_count > 0:
                summary = [
                    {
                        "role": "assistant",
                        "content": f"Plan summary: {middle_count} detailed messages compressed.",
                    }
                ]
