...
class FastPathEvaluator:
    ...
    def evaluate(
        ...
        agent_id = str(
            headers.get("x-orchesis-agent-id") or headers.get("x-openclaw-session-id") or ""
        )
        ...
        skip = [phase for phase in SKIPPABLE_PHASES.get(trust, []) if phase not in MANDATORY_PHASES]
