    ...
    def start(self) -> None:
        ...
        with self._lock:
            ...
            self._thread = threading.Thread(
                target=_worker, name="orchesis-mcp-monitor", daemon=True
            )
            self._thread.start()
        ...
        with self._lock:
            for path in self._paths:
                ...
                if previous_hash is None and current_hash is not None:
                    changes.append(
                        self._build_change(
                            path=path, change_type="added", details="MCP config file appeared"
                        )
                    )
                elif previous_hash is not None and current_hash is None:
                    changes.append(
                        self._build_change(
                            path=path, change_type="removed", details="MCP config file removed"
                        )
                    )
                elif (
                    ...
                    changes.append(
                        self._build_change(
                            ...
                            details="MCP config file content changed",
                        )
                    )
    ...
    def get_alerts(self, since: float | None = None) -> list[dict[str, Any]]:
        ...
        with self._lock:
            ...
            return [
                ...
                if float(item.get("timestamp", 0.0)) >= float(since)
            ]
