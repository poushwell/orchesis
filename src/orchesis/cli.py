...
from orchesis.credential_vault import (
    CredentialNotFoundError,
    EnvVault,
    FileVault,
    build_vault_from_policy,
)
...
from orchesis.experiment_runner import ExperimentRunner
...
CLI_VERIFY_PREFLIGHT_CHECKS = len(OrchesisVerifier.CHECKS)
# MCP JSON scanning — keep aligned with README “MCP checks” badge / marketing copy.
CLI_SCAN_MCP_CHECK_COUNT = 52

_POLICY_PATH_HINT = (
    "Create a starter policy with: orchesis init\n"
    "Or pass an existing file, e.g. orchesis proxy --config ./policy.yaml"
)
...
def _dump_policy_to_tempfile(policy: dict[str, Any]) -> str:
    fd, path = tempfile.mkstemp(suffix=".yaml", prefix="orchesis-preset-", text=True)
    try:
...
def _merge_base_policy_with_preset(base_policy: dict[str, Any], preset_name: str) -> dict[str, Any]:
    """Deep-merge preset over base; preset values win (see orchesis.config._deep_merge)."""
    from orchesis.openclaw_presets import get_named_preset

    overlay = copy.deepcopy(get_named_preset(preset_name))
    return _deep_merge(copy.deepcopy(base_policy), overlay)
...
@click.group(
    ...
    help=(
        ...
        "Block threats, cut token waste, monitor your fleet. "
        "Use `orchesis proxy` to sit between agents and LLM APIs, "
        "or `orchesis verify` before you go live."
    ),
)
@click.version_option(version=__version__, prog_name="orchesis")
@click.pass_context
...
@main.group("hooks", help="Install or remove Claude Code integration hooks.")
...
@main.command("hook-check", help="[internal] Check tool call against policy.")
...
def hook_check_command(tool_name: str, tool_input: str, config_path: str | None) -> None:
    ...
    code, output = evaluate_hook_tool(
        tool_name=tool_name, tool_input=tool_input, config_path=config_path
    )
    if output:
        click.echo(output)
...
@main.command("hook-log", help="[internal] Log tool call result.")
@click.option("--tool", "tool_name", required=True)
...
def hook_log_command(tool_name: str, tool_output: str, config_path: str | None) -> None:
    ...
    ok, log_path = log_hook_tool(
        tool_name=tool_name, tool_output=tool_output, config_path=config_path
    )
...
def _server_dependency_error() -> click.ClickException:
    return click.ClickException(
        "HTTP/API features need extra packages. Install with: pip install orchesis[server] "
        "(or pip install orchesis[all] for YAML + HTTP + server)."
    )
...
def _load_auth_stack(
    credentials_file: str, mode: str = "enforce"
) -> tuple[CredentialStore, AgentAuthenticator]:
...
@main.group("auth", help="Register, list, revoke, or rotate HMAC credentials for agents.")
def auth_group() -> None:
...
def auth_list(credentials_file: str) -> None:
    ...
    for row in rows:
        ...
        try:
            created_str = datetime.fromtimestamp(float(created_at), tz=timezone.utc).strftime(
                "%Y-%m-%d %H:%M"
            )
        except Exception:
            created_str = "unknown"
        if isinstance(last_used, int | float) and float(last_used) > 0:
            last_used_str = datetime.fromtimestamp(float(last_used), tz=timezone.utc).strftime(
                "%Y-%m-%d %H:%M"
            )
        else:
            last_used_str = "never"
...
def auth_revoke(agent_id: str, credentials_file: str) -> None:
    ...
    if not authenticator.revoke(agent_id):
        raise click.ClickException(
            f"Unknown agent: {agent_id}. List registered agents with: orchesis auth list"
        )
    store.save(authenticator.credentials)
...
def auth_rotate(agent_id: str, credentials_file: str) -> None:
    ...
    if cred is None:
        raise click.ClickException(
            f"Unknown agent: {agent_id}. Register first with: orchesis auth register {agent_id}"
        )
    store.save(authenticator.credentials)
...
@main.command("quickstart", help="Interactive setup wizard for a working orchesis.yaml config.")
@click.option(
    "--preset",
...
@main.command("new", help="Scaffold a new Orchesis project from template.")
...
@main.command("doctor", help="Run environment and project diagnostics.")
...
def doctor(config_path: str, attempt_fix: bool, json_output: bool) -> None:
    ...
    def _add_check(
        name: str, ok: bool, severity: str, detail: str, fix_suggestion: str = ""
    ) -> None:
        checks.append(
            {
                "name": name,
    ...
    if policy_file.exists():
        try:
            ...
            _add_check("config_exists", True, "ERROR", str(policy_file))
            _add_check(
                "config_valid", False, "ERROR", str(error), "Fix YAML syntax and required fields."
            )
            _add_check(
                "policy_validate",
                False,
                "ERROR",
                str(error),
                "Fix YAML syntax and required fields.",
            )
    else:
        if attempt_fix:
    ...
    _add_check(
        ...
        "WARNING",
        proxy_detail,
        "Start proxy: orchesis proxy --config <path>",
    )

    api_ok, api_detail = _http_responding("http://127.0.0.1:8090/health")
    _add_check(
        "api_running", api_ok, "WARNING", api_detail, "Start API: orchesis serve --policy <path>"
    )
    ...
    if isinstance(loaded_policy, dict):
        ...
        recording_cfg = {}
    semantic_enabled = (
        bool(semantic_cache_cfg.get("enabled")) if isinstance(semantic_cache_cfg, dict) else False
    )
    loop_enabled = bool(loop_cfg.get("enabled")) if isinstance(loop_cfg, dict) else False
    recording_enabled = (
        bool(recording_cfg.get("enabled")) if isinstance(recording_cfg, dict) else False
    )
    _add_check(
        "semantic_cache_enabled",
        semantic_enabled,
        "INFO",
        "enabled" if semantic_enabled else "disabled",
    )
    _add_check(
        "loop_detection_enabled", loop_enabled, "INFO", "enabled" if loop_enabled else "disabled"
    )
    _add_check(
        "recording_enabled",
        recording_enabled,
        "INFO",
        "enabled" if recording_enabled else "disabled",
    )
    ...
    warning_failures = [
        item for item in ordered_checks if item["severity"] == "WARNING" and not item["ok"]
    ]
    ...
    if json_output:
        payload = {
            ...
            "summary": {
                "errors": sum(
                    1 for item in ordered_checks if item["severity"] == "ERROR" and not item["ok"]
                ),
...
@main.command(
    help=(
        "Interactive project setup wizard: writes orchesis.yaml, policy.yaml, and request.json "
...
@click.option("--non-interactive", is_flag=True, default=False)
@click.option("--dir", "--output-dir", "project_dir", type=click.Path(), default=".")
@click.option("--budget", "budget_override", default=None)
@click.option(
    "--provider",
    type=click.Choice(["openai", "anthropic", "google", "other"]),
    default=None,
    help="Upstream LLM API for proxy.target in .orchesis/policy.yaml (non-interactive default: openai).",
)
@click.option(
    "--force",
    is_flag=True,
    default=False,
    help="Overwrite existing .orchesis/policy.yaml if present.",
)
@click.option(
    "--minimal",
    is_flag=True,
    default=False,
    help="Write only a minimal .orchesis/policy.yaml (no root policy.yaml / request.json).",
)
def init(
    preset: str | None,
    non_interactive: bool,
...
"""
    ...
    defaults = {
        "openclaw": {"budget": "50", "semantic_cache": True, "recording": True, "team": ""},
        "claude": {"budget": "50", "semantic_cache": True, "recording": True, "team": ""},
        "codex": {"budget": "50", "semantic_cache": True, "recording": True, "team": ""},
        "other": {"budget": "50", "semantic_cache": True, "recording": True, "team": ""},
    }
    ...
    if is_effectively_non_interactive:
        ...
        team_name = click.prompt(
            "Your team name (optional)", default="", show_default=False
        ).strip()
    ...
    elif is_effectively_non_interactive:
        provider_key = "openai"
    else:
        provider_key = click.prompt(
            "Which LLM provider are you routing through the proxy?",
            type=click.Choice(["openai", "anthropic", "google", "other"]),
            default="openai",
        )
    ...
    orch_dir = target_dir / ".orchesis"
    orch_dir.mkdir(parents=True, exist_ok=True)
    (orch_dir / "data").mkdir(parents=True, exist_ok=True)
    policy_orch = orch_dir / "policy.yaml"
    if policy_orch.exists() and not force:
        raise click.ClickException(f"{policy_orch} already exists. Use --force to overwrite.")

    upstream = provider_targets.get(provider_key, provider_targets["other"])
    daily_policy: str | float = "unlimited" if budget_unlimited else float(daily_budget or 50.0)

    if minimal:
        minimal_policy = f"""# Minimal Orchesis policy (orchesis init --minimal)
# proxy.target — your LLM vendor API; the proxy forwards requests here.
# proxy.listen_port — informational; run: orchesis proxy --config orchesis.yaml (port from CLI/config).

proxy:
  target: {upstream}
  listen_port: 8080

# budgets.daily — soft/hard limits are enforced in the pipeline (see budgets.on_hard_limit in full policies).
budgets:
  daily: {daily_policy}

# Loop / repetition detection (exact + fuzzy hashes of tool+prompt patterns).
loop_detection:
  enabled: true
  block_threshold: 5

# Threat / injection-style pattern matching (IOC + signature hooks).
threat_intel:
  enabled: true
...
rules:
  - name: budget_limit
    max_cost_per_call: 0.50
"""
        policy_orch.write_text(minimal_policy, encoding="utf-8")
    else:
        starter_head = f"""# Orchesis starter policy (orchesis init)
# -----------------------------------------------------------------------------
# proxy.target — upstream LLM API base URL for forwarded requests.
# proxy.listen_port — document the port you will bind (default 8080 with orchesis proxy).
# Point your agent at the proxy, e.g. base_url: http://localhost:8080/v1
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# budgets.daily — daily spend cap (USD-style float). Use "unlimited" in YAML if needed.
# -----------------------------------------------------------------------------
budgets:
  daily: {daily_policy}

# -----------------------------------------------------------------------------
# loop_detection — mitigates runaway tool/prompt loops.
# -----------------------------------------------------------------------------
loop_detection:
  enabled: true
  block_threshold: 5
...
# -----------------------------------------------------------------------------
# threat_intel — enables threat-signature matching (injection / IOC style).
# -----------------------------------------------------------------------------
threat_intel:
  enabled: true

"""
        policy_orch.write_text(starter_head + legacy_policy_content, encoding="utf-8")
        (target_dir / "policy.yaml").write_text(
            policy_orch.read_text(encoding="utf-8"), encoding="utf-8"
        )
        (target_dir / "request.json").write_text(legacy_request_content, encoding="utf-8")

    click.echo(f"Created {policy_orch.as_posix()}")
    click.echo("Next: orchesis verify")
    click.echo("Then: change your agent's base_url to http://localhost:8080/v1")
    if not minimal:
        click.echo(
            "Also wrote policy.yaml and request.json in the project root for `orchesis verify request.json`."
        )

    click.echo("")
    click.echo("✓ orchesis.yaml created")
    click.echo("✓ .orchesis/ directory ready (policy + data/)")
    click.echo("")
    click.echo("More next steps:")
...
@main.command(help="Generate Ed25519 keypair for signed decision logs.")
def keygen() -> None:
...
@main.command(
    help=(
        f"Run {CLI_VERIFY_PREFLIGHT_CHECKS} pre-flight security checks on your setup "
        "(proxy reachability, policy validity, MCP configs, OpenClaw routing, budgets, and more). "
        "With a request JSON path, evaluate that request against policy instead."
    ),
)
...
def verify(
    ...
    """Run pre-flight checks (see --help summary).

    Without a request file, runs the full verifier suite. With request.json, evaluates
    allow/deny for that tool call against policy.
    """
...
@main.command(
    help="Validate a policy YAML/JSON file and optional Policy-as-Code / framework rules.",
)
...
def validate(
    ...
    if not isinstance(resolved_policy, str):
        raise click.ClickException(
            "One of --config or --policy is required. "
            "Example: orchesis validate --policy policy.yaml"
        )
    try:
...
@click.option(
    "--format", "output_format", type=click.Choice(["markdown", "text"]), default="markdown"
)
...
def serve_scanner_command(
    port: int, host: str, policy_path: str | None, allow_file_access: bool
) -> None:
...
def credentials_set(alias: str, use_env_vault: bool, vault_path: str) -> None:
    ...
    if use_env_vault:
        ...
        return
    value = click.prompt(
        f"Credential value for {safe_alias}", hide_input=True, confirmation_prompt=True
    )
...
@main.command(help="List registered agent identities from policy.")
@click.option("--policy", "policy_path", type=click.Path(exists=True), required=True)
...
@main.command(help="Show policy version history.")
@click.option("--policy", "policy_path", type=click.Path(exists=True), required=True)
...
@main.command(help="Rollback to previous policy version.")
@click.option("--policy", "policy_path", type=click.Path(exists=True), required=True)
...
@main.command(
    help=(
        "Start the Orchesis production API server (FastAPI + Uvicorn in legacy mode; "
        "orchesis.serve when using default TLS/worker options)."
    ),
)
...
@click.option(
    "--policy", "policy_path", type=click.Path(), default=None, help="Path to orchesis.yaml policy"
)
...
@click.option(
    "--preset",
    type=click.Choice(["openclaw", "paperclip"]),
    default=None,
    help="Apply named preset configuration",
)
def serve(
    host: str,
    port: int,
    ...
    """Start the API server (see --help for modes and options)."""
    use_legacy_mode = (
        bool(cors_value)
        or bool(api_token)
    ...
    if not use_legacy_mode:
        ...
        policy_for_config = policy_path
        if preset is not None:
            base_path = str(Path(policy_path or "policy.yaml").expanduser())
            base_pol: dict[str, Any] = {}
            if Path(base_path).is_file():
                try:
                    base_pol = load_policy(base_path)
                except (ValueError, YAMLError, OSError) as error:
                    raise click.ClickException(f"Failed to load policy: {error}") from error
            try:
                merged_serve = _merge_base_policy_with_preset(base_pol, preset)
            except ValueError as exc:
                raise click.ClickException(str(exc)) from exc
            policy_for_config = _dump_policy_to_tempfile(merged_serve)
            OPERATIONS_LOG.info(f"Applied preset: {preset}")
            _LOGGER.debug(
                "loop_detection after preset merge: %s",
                json.dumps(merged_serve.get("loop_detection", {}), ensure_ascii=False, default=str),
            )
    ...
    if preset is not None:
        try:
            merged_legacy = _merge_base_policy_with_preset(policy, preset)
        except ValueError as exc:
            raise click.ClickException(str(exc)) from exc
        effective_policy_path = _dump_policy_to_tempfile(merged_legacy)
        policy = merged_legacy
        OPERATIONS_LOG.info(f"Applied preset: {preset}")
        _LOGGER.debug(
            "loop_detection after preset merge: %s",
            json.dumps(merged_legacy.get("loop_detection", {}), ensure_ascii=False, default=str),
        )
    ...
    policy_api_cfg = (
        policy.get("api")
        if isinstance(policy, dict) and isinstance(policy.get("api"), dict)
        else {}
    )
    policy_token = (
        policy_api_cfg.get("token") if isinstance(policy_api_cfg.get("token"), str) else None
    )
    token_to_use = (
        api_token.strip()
        if isinstance(api_token, str) and api_token.strip()
        else (
            policy_token.strip() if isinstance(policy_token, str) and policy_token.strip() else ""
        )
    )
    if not token_to_use:
        token_to_use = (
            f"orchesis-{hashlib.sha256(str(time.time()).encode('utf-8')).hexdigest()[:6]}"
        )
    ...
    click.echo(
        "Endpoints: /health, /docs, /api/v1/policy, /api/v1/agents, /api/v1/evaluate, /api/v1/status"
    )
...
@main.command(
    "proxy",
    help=(
        "Run the stdlib LLM HTTP proxy between your agents and OpenAI/Anthropic-compatible "
        "upstream APIs (set agent base URL to this host/port)."
    ),
)
...
def proxy_command(
    ...
    """Run the LLM HTTP proxy (see --help)."""
    ...
    effective_policy_path = (
        config_path if isinstance(config_path, str) and config_path.strip() else policy_path
    )
    if isinstance(effective_policy_path, str) and effective_policy_path.strip():
        ...
        if not candidate.exists():
            raise click.ClickException(
                f"Policy file not found at {candidate}.\n{_POLICY_PATH_HINT}"
            )
    ...
    working_policy: dict[str, Any] = copy.deepcopy(policy) if isinstance(policy, dict) else {}
    if preset is not None:
        try:
            working_policy = _merge_base_policy_with_preset(working_policy, preset)
        except ValueError as exc:
            raise click.ClickException(str(exc)) from exc
        safe_policy_path = _dump_policy_to_tempfile(working_policy)
        OPERATIONS_LOG.info(f"Applied preset: {preset}")
        _LOGGER.debug(
            "loop_detection after preset merge: %s",
            json.dumps(working_policy.get("loop_detection", {}), ensure_ascii=False, default=str),
        )

    proxy_policy_cfg = working_policy.get("proxy", {})
...
@main.command("demo", help="Launch local proxy + API services for a full demo.")
...
@main.command("launch", help="Launch a local agent through Orchesis proxy.")
...
def launch_command(agent: str, config_path: str | None) -> None:
    ...
    if not isinstance(agent_cmd_raw, list):
        raise click.ClickException(
            f"Unsupported agent launch mode: {agent}. "
            f"Supported: {', '.join(sorted(AGENT_COMMANDS))}."
        )
...
def status_command(json_output: bool, watch_mode: bool) -> None:
    ...
    def _status_snapshot() -> dict[str, Any]:
        ...
        loops_today = int(
            proxy_stats.get("loops_detected_today", proxy_stats.get("loops_detected", 0)) or 0
        )
        active_agents = int(
            proxy_stats.get("active_agents", proxy_stats.get("agents_discovered", 0)) or 0
        )
    ...
    def _render_text(snapshot: dict[str, Any]) -> None:
        ...
        if snapshot["api_running"]:
            click.echo(
                f"API          {api_mark} running  :8090   uptime {snapshot['api_uptime_human']}"
            )
        else:
...
@main.command(
    "dashboard",
    help=(
        "Open the real-time monitoring dashboard in your browser "
        "(default URL port 8081, or proxy UI with --proxy-dashboard)."
    ),
)
@click.option(
    "--port",
...
def dashboard_command(port: int, proxy_dashboard: bool, no_browser: bool) -> None:
    """Open the monitoring dashboard (see --help)."""
    ...
    click.echo(
        "Tip: with `orchesis proxy` / LLM proxy on :8080, use --proxy-dashboard for the embedded UI."
    )
    if not no_browser:
        webbrowser.open(url)
    raise SystemExit(0)
...
@main.command("kill", help="Trigger emergency kill switch on running proxy.")
...
@main.command("resume", help="Resume proxy after kill switch activation.")
@click.option("--port", type=int, default=8100)
...
@main.command("reload", help="Reload policy in a running proxy instance.")
...
def migrate_command(
    config_path: str, target_version: str | None, dry_run_mode: bool, no_backup: bool
) -> None:
    ...
    result = (
        migrator.dry_run(policy, resolved_target)
        if dry_run_mode
        else migrator.migrate(policy, resolved_target)
    )
    ...
    path.write_text(
        yaml.safe_dump(migrated_policy, sort_keys=False, allow_unicode=True), encoding="utf-8"
    )
...
def backup_command(
    ...
    selected = (
        int(list_mode)
        + int(isinstance(restore_path, str) and bool(restore_path.strip()))
        + int(isinstance(verify_path, str) and bool(verify_path.strip()))
    )
    if selected > 1:
...
@main.command("nodes", help="List connected enforcement nodes from control plane.")
...
@main.command("sync", help="Run one-shot policy synchronization against control plane.")
...
@main.command("plugins", help="List registered plugins.")
@click.option("--policy", "policy_path", type=click.Path(exists=True), default=None)
...
@main.command(help="Run synthetic adversarial fuzzing against a policy.")
@click.option("--policy", "policy_path", type=click.Path(exists=True), required=True)
...
@main.command(help="Run prebuilt adversarial scenarios.")
@click.option("--policy", "policy_path", type=click.Path(exists=True), required=True)
...
@main.command(help="Run mutation engine against policy.")
@click.option("--policy", "policy_path", type=click.Path(exists=True), required=True)
@click.option("--count", "count", type=int, default=1000)
@click.option("--seed", "seed", type=int, default=42)
...
@main.command(help="Run formal invariant checks.")
@click.option("--policy", "policy_path", type=click.Path(exists=True), required=True)
...
@main.command("drift", help="Run state drift detection over current state and decision log.")
@click.option("--policy", "policy_path", type=click.Path(exists=True), required=True)
...
@main.command("torture", help="Run sustained concurrent stress test and report throughput/latency.")
@click.option("--policy", "policy_path", type=click.Path(exists=True), required=True)
...
@main.command(help="Manage regression corpus entries and generated tests.")
...
@main.command(
    "scan",
    help=(
        "Scan MCP configs, skills, and policy files for security issues. "
        f"MCP JSON scanning includes {CLI_SCAN_MCP_CHECK_COUNT} check categories (see README). "
        "Use --mcp to scan discovered configs, or pass a PATH."
    ),
)
...
@click.option(
    "--format",
    "output_format",
    type=click.Choice(["text", "json", "html", "md"]),
    default="text",
)
@click.option(
    "--output",
    ...
    default=None,
    help="Write scan output to this file instead of stdout (for --format json or html).",
)
...
def scan_command(
    ...
    """Static security scan (see --help)."""
    if output_path is not None and output_format not in ("json", "html"):
        raise click.ClickException("--output is only supported with --format json or html.")
    reports: list[ScanReport] = []
    network_findings: list[dict[str, Any]] = []
    machine_fmt = output_format in ("json", "html")
    discovery_err = machine_fmt
    if scan_network:
        network_findings = NetworkExposureScanner().scan_all()
    if scan_mcp_configs:
        discovered = discover_mcp_configs()
        click.echo("Discovered MCP configs:", err=discovery_err)
        for cfg in discovered:
            click.echo(f"  {cfg}", err=discovery_err)
        for cfg in discovered:
        ...
        if not target.exists():
            raise click.ClickException(
                f"Path not found: {target}. Fix the path, or use --mcp / --network for other scan modes."
            )
        reports = scan_path(target)
    elif not scan_network:
        raise click.ClickException(
            "Nothing to scan: pass PATH (file or directory), use --mcp for discovered MCP configs, "
            "or add --network for local exposure checks."
        )
    ...
    if yara_path is not None:
        ...
        for report in reports:
            ...
            for match in yara_matches:
                ...
                description = str(
                    match.meta.get("description", f"YARA rule matched: {match.rule_name}")
                )
                location = (
                ...
                evidence = (
                    match.matched_strings[0].matched_text
                    if match.matched_strings
                    else match.rule_name
                )
    ...
    if use_llm_judge:
        ...
        if llm_config is None:
            click.echo(
                "Warning: --llm-judge requested but ORCHESIS_LLM_API_KEY is not set; skipping LLM analysis."
            )
        else:
            judge = LLMJudge(
    ...
    if output_format == "json":
        text = reports_to_json(
            reports,
            llm_findings_by_target=llm_findings_by_target,
            network_findings=network_findings if network_findings else None,
            indent=2,
        )
        if output_path is not None:
    ...
    if output_format == "html":
        text = reports_to_html(
            reports, network_findings=network_findings if network_findings else None
        )
        if output_path is not None:
            output_path.parent.mkdir(parents=True, exist_ok=True)
            output_path.write_text(text, encoding="utf-8")
...
@main.command(
    "benchmark-scanner", help="Benchmark sequential regex vs Aho-Corasick prefilter scanner."
)
...
def integrity_init(
    paths: tuple[str, ...], auto_discover: bool, baseline_path: str, extra_paths: tuple[str, ...]
) -> None:
...
@click.option("--baseline", "baseline_path", default=".orchesis/integrity.json")
def integrity_watch(
    interval: int, alert_enabled: bool, policy_path: str, baseline_path: str
) -> None:
    monitor = IntegrityMonitor(baseline_path=baseline_path)
    alert_callback = None
    if alert_enabled:
...
def scan_remote_command(target: str | None, output_format: str, batch_file: str | None) -> None:
    ...
    if output_format == "json":
        click.echo(
            json.dumps([report_to_dict(item) for item in reports], ensure_ascii=False, indent=2)
        )
        return
...
def gate(
    ...
    if not policy_file.exists():
        click.echo(
            f"Gate error: policy file not found at {policy_file}.\n"
            f"  {_POLICY_PATH_HINT.replace(chr(10), ' ')}"
        )
        raise SystemExit(2)
    ...
    try:
        ...
        click.echo(
            f"Gate error: could not load policy ({policy_file}): {error}\n"
            "  Fix YAML/JSON syntax, install pyyaml for .yaml files (pip install orchesis[yaml]), "
            "then re-run."
        )
        raise SystemExit(2)
    ...
    if scan_dir:
        ...
        if not target.exists():
            click.echo(
                f"Gate error: scan directory not found: {target}. "
                "Create it or pass --scan-dir with an existing path."
            )
            raise SystemExit(2)
...
@main.command(help="Audit the decision log: list entries, stats, signatures, or export.")
...
@main.command("export")
@click.option(
    "--format", "output_format", type=click.Choice(["json", "csv", "jsonl"]), default="json"
)
@click.option("--output", "output_path", type=click.Path(), default=None)
@click.option("--agent", "agent_id", default=None)
@click.option("--session", "session_id", default=None)
...
@main.command("evidence", help="Export Evidence Record for one session.")
...
@click.option(
    "--decisions-log", "decisions_log_path", type=click.Path(), default=str(DEFAULT_DECISIONS_PATH)
)
def autopsy_command(
    session_id: str | None,
    output_format: str,
    ...
    if source.exists():
        for line in source.read_text(encoding="utf-8").splitlines():
            ...
            rows.append(
                {
                    "session_id": str(
                        entry.get("session_id") or state.get("session_id") or "__global__"
                    ),
    ...
    if list_recent:
        ...
        if output_format == "json":
            content = json.dumps(
                {"sessions": listing, "count": len(listing)}, ensure_ascii=False, indent=2
            )
        else:
    ...
    report = AgentDiagnostics().perform(session_id=session_id.strip(), decisions_log=rows)
    if "error" in report:
...
def incidents_report(since: str | None, output_format: str, output_path: str | None) -> None:
    ...
    content = (
        engine.export_markdown(report) if output_format == "md" else engine.export_json(report)
    )
    if output_path is not None:
        target = Path(output_path)
        target.parent.mkdir(parents=True, exist_ok=True)
...
def incidents_risk(agent_id: str) -> None:
    ...
    profile = ForensicsEngine(decisions_path=str(DEFAULT_DECISIONS_PATH)).agent_risk_profile(
        agent_id
    )
    ...
    top_tools = (
        ", ".join(f"{name} ({count})" for name, count in profile["top_denied_tools"][:2]) or "none"
    )
...
def incidents_timeline(agent_id: str | None, incident_id: str | None, last_n: int) -> None:
    ...
    for event in events:
        ts = (
            event["ts"][11:19]
            if isinstance(event["ts"], str) and len(event["ts"]) >= 19
            else event["ts"]
        )
...
@main.command(help="Replay structured decision logs or re-evaluate a session against policy.")
...
@main.command(help="Show decision timeline for a specific agent.")
@click.option("--agent", "agent_id", required=True)
...
@main.command("reliability-report", help="Generate reliability report.")
@click.option("--format", "output_format", type=click.Choice(["md", "json"]), default="md")
...
@click.option(
    "--decisions", "decisions_path", default=str(DEFAULT_DECISIONS_PATH), show_default=True
)
@click.option("--output", "output_path", type=click.Path(), default=None)
def token_yield_report_command(
    ...
    rendered = (
        ...
        else generator.export_text(report)
    )
...
@click.option(
    ...
    default=None,
    help="Input JSON/JSONL data file",
)
...
def experiment_command(
    ...
    """Run experiments or inspect results."""
...
def bootstrap_ci_cmd(
    ...
    from orchesis.stats.bootstrap_ci import BootstrapCI, DEFAULT_TOOL_FREQUENCIES
    ...
    if data_file is not None:
        ...
        click.echo("Using built-in tool frequency data (N=41)")
        data = list(DEFAULT_TOOL_FREQUENCIES)
...
@main.command("export-paper-data")
...
def export_paper_data_command(output_dir: str, section: str | None) -> None:
    """Export paper-ready data sections."""
...
@main.command("generate-paper")
@click.option(
    "--generate", "generate_all", is_flag=True, default=False, help="Generate full paper package"
)
@click.option(
    "--section",
    "section",
...
@click.option(
    "--checklist", "show_checklist", is_flag=True, default=False, help="Show arXiv checklist"
)
@click.option(
    "--export-latex", "export_latex", is_flag=True, default=False, help="Export LaTeX package"
)
@click.option("--output", "output_dir", default="paper", show_default=True, help="Output directory")
...
def _load_paper_from_dir(paper_dir: str) -> dict[str, Any]:
    ...
    if refs.exists():
        lines = [
            line.strip() for line in refs.read_text(encoding="utf-8").splitlines() if line.strip()
        ]
...
@main.command("arxiv-validate", help="Validate paper package for arXiv submission.")
...
@main.command("arxiv-package", help="Generate arXiv submission package files.")
@click.option("--paper", "paper_dir", type=click.Path(exists=True, file_okay=False), required=True)
...
@click.option(
    "--file", "file_path", type=click.Path(exists=True), default=None, help="Single file to audit"
)
@click.option(
    "--dir",
    "dir_path",
...
@click.option(
    "--ext", "extensions_csv", default=None, help="Comma-separated file extensions (e.g. py,js,ts)"
)
@click.option(
    "--severity",
    "severity_threshold",
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Summary mode for directory audits",
)
@click.option(
    "--badge", "badge_output", is_flag=True, default=False, help="Print compact badge text"
)
@click.option(
    "--format",
    "output_format",
...
@click.option(
    "--output", "output_path", type=click.Path(), default=None, help="Write report to file"
)
...
@click.option(
    "--list-cases", "list_cases", is_flag=True, default=False, help="List all benchmark cases"
)
@click.option(
    "--export", "export_path", default=None, type=click.Path(), help="Export results to JSON/CSV"
)
def benchmark_command(
    ...
    def _load_report(path: str) -> BenchmarkReport:
        ...
        return BenchmarkReport(
            ...
            failed=int(
                payload.get(
                    "failed", max(0, len(results) - sum(1 for item in results if item.passed))
                )
            ),
            pass_rate=float(
                payload.get(
            ...
            if isinstance(payload.get("by_category"), dict)
            else {},
            by_severity=payload.get("by_severity", {})
            if isinstance(payload.get("by_severity"), dict)
            else {},
    ...
    def _run_cases_with_progress(
        suite: BenchmarkSuite, selected: list[BenchmarkCase]
    ) -> BenchmarkReport:
        total = len(selected)
        ...
        avg_latency = (
            (sum(item.latency_ms for item in results) / float(len(results))) if results else 0.0
        )
        ...
        by_severity = suite._aggregate(  # noqa: SLF001
            ...
            lambda result: (
                case_by_id.get(
                    result.case_id, BenchmarkCase("", "", "", "", {}, "allow", "low", [], "")
                ).severity
            ),
        )
        return BenchmarkReport(
    ...
    if list_cases:
        ...
        for case in ORCHESIS_BENCHMARK_V1:
            click.echo(
                f"{case.id:<7}  {case.category:<11}  {case.subcategory:<23}  {case.description}"
            )
        return
...
@main.command("update")
@click.option(
    "--check", "check_only", is_flag=True, default=False, help="Check only; do not install"
)
@click.option(
    "--install", "do_install", is_flag=True, default=False, help="Install update automatically"
)
@click.option(
    "--channel", type=click.Choice(["stable", "beta"], case_sensitive=False), default="stable"
)
...
@click.option(
    "--decisions", "decisions_path", type=click.Path(), default=str(DEFAULT_DECISIONS_PATH)
)
def publish_command(
    period_days: int,
    output_path: str | None,
...
def spec_command(
    ...
    selected = (
        int(generate_doc)
        + int(owasp_alignment)
        + int(eu_ai_act_alignment)
        + int(show_version)
        + int(isinstance(validate_path, str) and bool(validate_path.strip()))
    )
    if selected == 0:
        raise click.ClickException(
...
@main.command("diff", help="Compare two policy versions and print change summary.")
...
@main.command("readiness", help="Compute Agent Readiness Index (ARI) for an agent.")
@click.option("--agent", "agent_id", required=True)
...
def readiness_command(agent_id: str, config_path: str) -> None:
    ...
    for dim in result.dimensions:
        if dim.status == "pass":
            ...
            label = "FAIL"
        click.echo(
            f"  {color}{mark}{reset} {dim.name:<20} {dim.score:>5.1f}  {color}{label}{reset}"
        )
...
@main.command("ari-check", help="CI/CD gate for Agent Readiness Index.")
@click.option("--agent", "agent_id", required=True)
...
@main.command("arc-check", help="CI/CD gate for ARC readiness certification.")
@click.option("--agent", "agent_id", required=True)
@click.option("--min-score", type=int, default=75, show_default=True)
...
def arc_check_command(agent_id: str, min_score: int, policy_path: str) -> None:
    ...
    if Path(policy_path).exists():
        try:
            ...
            policy = {}
    readiness_cfg = (
        policy.get("agent_readiness", {}) if isinstance(policy.get("agent_readiness"), dict) else {}
    )
    metrics_store = (
        readiness_cfg.get("metrics", {}) if isinstance(readiness_cfg.get("metrics"), dict) else {}
    )
    metrics = (
        metrics_store.get(agent_id, {}) if isinstance(metrics_store.get(agent_id), dict) else {}
    )
...
@main.command("arc-certify", help="Issue ARC certificate for an agent when threshold is met.")
@click.option("--agent", "agent_id", required=True)
@click.option("--policy", "policy_path", default="orchesis.yaml", show_default=True)
...
@main.command("arc-list", help="List ARC certificates issued in this command run.")
@click.option("--policy", "policy_path", default="orchesis.yaml", show_default=True)
...
@main.command("channels", help="Show channel health status.")
@click.option("--format", "fmt", default="text", type=click.Choice(["text", "json"]))
...
@main.command("are-report", help="Show ARE (Agent Reliability Engineering) report.")
@click.option("--format", "fmt", default="text", type=click.Choice(["text", "json"]))
...
@main.command("vibe-watch", help="Watch directory for file changes and run Vibe Code Audit.")
...
def aabb_command(
    ...
    if bool(show_leaderboard):
        click.echo(
            json.dumps({"leaderboard": bench.get_leaderboard()}, ensure_ascii=False, indent=2)
        )
        return
...
def threat_feed_command(
    ...
    if config_file.exists():
        try:
            ...
            policy = {}
    cfg = (
        policy.get("threat_feed", {})
        if isinstance(policy, dict) and isinstance(policy.get("threat_feed"), dict)
        else {}
    )
    feed = ThreatFeed(cfg)
...
@click.option(
    "--format", "output_format", type=click.Choice(["md", "json", "text"]), default="text"
)
@click.option("--output", "output_path", type=click.Path(), default=None)
def compliance_command(
    ...
    if isinstance(agent_id, str) and agent_id.strip():
        ...
        events = read_events_from_jsonl(source) if source.exists() else []
        filtered = [
            event for event in events if str(getattr(event, "agent_id", "")) == agent_id.strip()
        ]
...
@main.command("comply", help="Run EU AI Act compliance check on fleet.")
@click.option("--format", "fmt", default="text", type=click.Choice(["text", "json"]))
...
def _render_single_compliance_text(report: Any) -> str:
    ...
    for check in report.checks:
        symbol = (
            ...
            else ("PARTIAL" if check.status == "partial" else "FAIL")
        )
