...
def verify(request_path: str, policy_path: str, should_sign: bool) -> None:
    ...
    decision = evaluate(request, policy, state=state_tracker)
    ...
    append_decision(decision, request, Path("decisions.jsonl"), signature=signature)
...
def audit(since: int | None, limit: int, verify_signatures: bool) -> None:
    """Audit decision log."""
    decisions = read_decisions(Path("decisions.jsonl"))
