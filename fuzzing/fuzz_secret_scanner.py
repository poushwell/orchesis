...
def TestOneInput(data: bytes) -> None:
    ...
    # Part C: raw fuzz bytes as lossy UTF-8 (matches CI harness; tests real unicode edge cases).
    try:
        fuzz_text = data.decode("utf-8", errors="replace")
        _ = scanner.scan_text(fuzz_text)
    except (ValueError, TypeError, KeyError, AttributeError, IndexError,
            OverflowError, ArithmeticError, OSError):
        return
